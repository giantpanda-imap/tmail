...
#include "linkage.c"
    ...
    if (--argc < 1)
        _exit(fail("usage: tmail [-D] user[+folder]", EX_USAGE));
    /* process all flags */
    while (argc && (*(s = *++argv)) == '-')
    {
        ...
        case 'I': /* inbox specifier */
            ...
            if (argc--)
                ...
                _exit(fail("missing argument to -I", EX_USAGE));
            break;
        ...
        case 'r': /* flag giving return path */
            ...
            if (argc--)
                ...
                _exit(fail("missing argument to -f or -r", EX_USAGE));
            break;
        ...
        case 'p': /* precedence for quota */
            if (s[2] && ((s[2] == '-') || isdigit(s[2])))
                precedence = atol(s + 2);
            else if (argc-- && ((*(s = *++argv) == '-') || isdigit(*s)))
                precedence = atol(s);
            else
                _exit(fail("missing argument to -p", EX_USAGE));
            break;
    ...
    if (!argc)
        ret = fail("no recipients", EX_USAGE);
    else if (!(f = tmpfile()))
        ret = fail("can't make temp file", EX_TEMPFAIL);
    else
    { /* build delivery headers */
        if (sender)
            fprintf(f, "Return-Path: <%s>\015\012", sender);
        /* start Received line: */
        ...
        if (ruid && !((pwd = getpwnam("daemon")) && (ruid == pwd->pw_uid)))
        {
            ...
            {
                if (pwd)
                    sprintf(tmp, "user %.80s", pwd->pw_name);
                else
                    sprintf(tmp, "UID %ld", (long)ruid);
            ...
            if (pwd)
                ...
                fprintf(f, "UID %ld", (long)ruid);
            fputs(")", f);
        }
        /* write "for" if single recipient */
        if (argc == 1)
            fprintf(f, " for %s", *argv);
        fputs("; ", f);
        rfc822_date(tmp);
        fputs(tmp, f);
        fputs("\015\012", f);
        ...
        if (!fgets(tmp, MAILTMPLEN - 1, stdin) || !(s = strchr(tmp, '\n')) ||
            (s == tmp) || s[1])
            _exit(fail("bad first message line", EX_USAGE));
        if (s[-1] == '\015')
        { /* nuke leading "From " line */
        ...
        if (ferror(f))
            ret = fail("error writing temp file", EX_TEMPFAIL);
        else if (!msglen)
            ret = fail("empty message", EX_TEMPFAIL);
        /* single delivery */
        else if (argc == 1)
            ret = deliver(f, msglen, *argv);
        else
            do
            { /* multiple delivery uses daughter forks */
                if ((pid = fork()) < 0)
                    ret = fail(strerror(errno), EX_OSERR);
                else if (pid)
                { /* mother process */
                    grim_pid_reap_status(pid, NIL, (void *)status);
                    /* normal termination? */
                    if (!ret)
                        ret = (status & 0xff) ? EX_SOFTWARE : (status & 0xff00) >> 8;
                }
                /* daughter process */
                else
                    _exit(deliver(f, msglen, *argv));
            } while (--argc && *argv++);
...
int deliver(FILE *f, unsigned long msglen, char *user)
{
    ...
    { /* non-INBOX name */
        ...
        default: /* other names, try to deliver if not INBOX */
            if (!strstr(mailbox, "..") && !strstr(mailbox, "//") &&
                ...
                !deliver_safely(NIL, &st, mailbox, path, duid, tmp))
                return NIL;
        case '%':
        case '*': /* wildcards not valid */
                   ...
                   ((inbox[4] == 'X') || (inbox[4] == 'x')) && !inbox[5]))
    {
        ...
        { /* -I successfully resolved a path? */
            ...
            /* now attempt delivery */
                ...
                return deliver_safely(ds, &st, inbox, path, duid, tmp);
        }
    }
    ...
    if (mailboxfile(path, mailbox = "INBOX") && !path[0])
    {
        ...
        if (strcmp(ds->dtb->name, "dummy"))
            return (ibxpath(ds, &mailbox, path) && !lstat(path, &sbuf)) ? deliver_safely(ds, &st, mailbox, path, duid, tmp) : fail("unable to resolve INBOX path", EX_CANTCREAT);
    ...
    /* black box, must create, get create proto */
        ...
        ds = default_proto(NIL);
    else
    { /* black box, existing file */
...
long ibxpath(MAILSTREAM *ds, char **mailbox, char *path)
{
    ...
    else if (ds->dtb->flags & DR_NAMESPACE)
        return NIL;
    /* INBOX in home directory */
    else
        ret = ((mailboxfile(path, "&&&&&") == path) &&
               (s = strstr(path, "&&&&&")) && strcpy(s, "INBOX"))
                  ? T
                  : NIL;
    if (ret)
                   ...
                   uid_t uid, char *tmp)
{
    ...
    if (i)
        return i; /* give up now if delivery unsafe */
    /* directory, not file */
    if ((sbuf.st_mode & S_IFMT) == S_IFDIR)
    {
        if (sbuf.st_mode & 0001)
        { /* listable directories may be worrisome */
    ...
    { /* file, not directory */
        if (sbuf.st_nlink != 1)
        { /* multiple links may be worrisome */
            sprintf(tmp, "WARNING: multiple links to file %.80s", path);
            mm_log(tmp, WARN);
        }
        if (sbuf.st_mode & 0111)
        { /* executable files may be worrisome */
            sprintf(tmp, "WARNING: file %.80s is executable", path);
            mm_log(tmp, WARN);
        }
    }
    if (sbuf.st_mode & 0002)
    { /* public-write files may be worrisome */
    ...
    if (!tmail_quota(st, path, uid, tmp, sender, precedence))
        return fail(tmp, -1);
    /* so far, so good */
    ...
    /* check file type */
        ...
        case S_IFREG: /* file is unsafe if setuid */
            if (sbuf->st_mode & S_ISUID)
                strcat(tmp, "setuid file");
            /* or setgid */
            else if (sbuf->st_mode & S_ISGID)
                strcat(tmp, "setgid file");
            else
                return NIL; /* otherwise safe */
            break;
        case S_IFCHR:
            strcat(tmp, "character special");
            break;
        case S_IFBLK:
            strcat(tmp, "block special");
            break;
        case S_IFLNK:
            strcat(tmp, "symbolic link");
            break;
#if defined(S_IFSOCK)
        case S_IFSOCK:
            strcat(tmp, "socket");
...
char *getusername(char *s, char **t)
{
    ...
    {                   /* have a mailbox specifier? */
        *(*t)++ = '\0'; /* yes, tie off user name */
        ...
        if (strlen(*t) > NETMAXMBX)
            return NIL;
        /* user+ and user+INBOX same as user */
        if (!**t || !compare_cstring((unsigned char *)*t, "INBOX"))
            *t = NIL;
    }
...
{
    if (trycreate)
        mm_dlog(string); /* debug logging only if trycreate in effect */
    else
    { /* ordinary logging */
        fprintf(stderr, "%s\n", string);
        switch (errflg)
