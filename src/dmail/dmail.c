...
#include "linkage.c"
    ...
    for (--argc; argc && (*(s = *++argv)) == '-'; argc--)
        ...
        case 'r': /* flag giving return path */
            ...
            if (argc--)
                ...
                _exit(fail("missing argument to -r", EX_USAGE));
            break;
        case 'k':
            if (keywords)
                _exit(fail("duplicate -k", EX_USAGE));
            if (argc--)
                keywords = cpystr(*++argv);
            else
                _exit(fail("missing argument to -k", EX_USAGE));
            break;
        case 'p':
    ...
    if (argc > 1)
        _exit(fail("too many recipients", EX_USAGE));
    else if (!(f = tmpfile()))
        _exit(fail("can't make temp file", EX_TEMPFAIL));
    ...
    /* single delivery */
        ...
        ret = deliver(f, msglen, argc ? *argv : myusername());
...
int deliver(FILE *f, unsigned long msglen, char *user)
{
    ...
    { /* non-INBOX name */
        ...
        default: /* other names, try to deliver if not INBOX */
            if ((strlen(mailbox) <= NETMAXMBX) &&
                ...
                !deliver_safely(NIL, &st, mailbox, path, tmp))
                return NIL;
        case '%':
    ...
    if (mailboxfile(path, mailbox = "INBOX") && !path[0])
    {
        ...
        if (strcmp(ds->dtb->name, "dummy"))
            return (ibxpath(ds, &mailbox, path) && !lstat(path, &sbuf)) ? deliver_safely(ds, &st, mailbox, path, tmp) : fail("unable to resolve INBOX path", EX_CANTCREAT);
                   ...
                   char *tmp)
{
    ...
    { /* any keywords requested? */
        if (flagseen)
            sprintf(flags = tmp, "\\Seen %.1000s", keywords);
        else
            flags = keywords;
    }
    else if (flagseen)
