...
int main(int argc, char *argv[])
{
    ...
    char *pgmname = (argc && argv[0]) ? (((s = strrchr(argv[0], '/')) || (s = strrchr(argv[0], '\\'))) ? s + 1 : argv[0]) : "ipop3d";
...
#include "linkage.c"
    ...
    { /* set up MD5 challenge */
        ...
        while (auth && compare_cstring(auth->name, "CRAM-MD5"))
            auth = auth->next;
        ...
        if (auth && auth->server && !(auth->flags & AU_DISABLE))
            ...
            challenge[0] = '\0'; /* no MD5 authentication */
    }
   ...
   */
    PSOUT("+OK POP3 ");
    if (!challenge[0])
    { /* if no MD5 enable, output host name */
    ...
    PSOUT(" server ready");
    if (challenge[0])
    { /* if MD5 enable, output challenge here */
        PBOUT(' ');
        PSOUT(challenge);
    }
    CRLF;
        ...
        {             /* dispatch based on command */
            ...
            else if (!strcmp(s, "CAPA"))
            {
                ...
                case AUTHORIZATION: /* waiting to get logged in */
                    ...
                    {
                        if (challenge[0])
                        { /* can do it if have an MD5 challenge */
                            if (host)
                                fs_give((void **)&host);
                            if (user)
                                fs_give((void **)&user);
                            ...
                            if (!(t && *t && (s = strtok(t, " ")) && (t = strtok(NIL, "\012"))))
                                ...
                                syslog(LOG_INFO, "APOP user=%.80s host=%.80s no mailbox",
                                       user, tcp_clienthost());
                        }
                        else
                            PSOUT("-ERR Not supported\015\012");
                    }
                    /* (chuckle) */
                             ...
                             !strcmp(s, "USER"))
                    {
                        ...
                        { /* if user name given */
                            ...
                            PSOUT("-ERR Missing username argument\015\012");
                    }
                    else if (!mail_parameters(NIL, GET_DISABLEPLAINTEXT, NIL) &&
                             user && *user && !strcmp(s, "PASS"))
                        state = pass_login(t, argc, argv);
                    else
                        PSOUT("-ERR Unknown AUTHORIZATION state command\015\012");
                    break;
                    ...
                    else if (!strcmp(s, "LIST"))
                    {
                        ...
                        { /* argument do single message */
                                ...
                                !(flags[i] & DELE))
                            {
                                ...
                                PSOUT("-ERR No such message\015\012");
                        }
                        else
                        { /* entire mailbox */
                        ...
                        { /* must have an argument */
                                ...
                                !(flags[i] & DELE))
                            {
                                ...
                                sprintf(tmp, STATUS, elt->seen ? "R" : " ",
                                        elt->recent ? " " : "O");
                                if (k < 4)
                                    CRLF; /* don't write Status: if no header */
                                ...
                                {         /* abnormal - no blank line at end of header */
                            ...
                            PSOUT("-ERR Missing message number argument\015\012");
                    }
                            ...
                            !(flags[i] & DELE))
                        {
                            ...
                            if ((*s >= '0') && (*s <= '9'))
                            {
                                ...
                                { /* want any text lines? */
                                    ...
                                    if (k && (j -= blat(t, j, k, &stream->private.string)))
                                        CRLF;
                                }
                                PBOUT('.'); /* end of list */
                                CRLF;
                                ...
                                PSOUT("-ERR Bad line count argument\015\012");
                        }
                        else
                            PSOUT("-ERR Bad message number argument\015\012");
                    }

                    else if (!strcmp(s, "XTND"))
                        PSOUT("-ERR Sorry I can't do that\015\012");
                    else
                        PSOUT("-ERR Unknown TRANSACTION state command\015\012");
                    break;
                default:
        ...
        {                      /* only bother if messages need marking seen */
            *(s = tmp) = '\0'; /* clear sequence */
            for (i = 1; i <= nmsgs; ++i)
                if (flags[i] & SEEN)
                {
                    for (j = i + 1, k = 0; (j <= nmsgs) && (flags[j] & SEEN); ++j)
                        k = j;
                    if (k)
                        sprintf(s, ",%lu:%lu", i, k);
                    else
        ...
        {                      /* any messages to delete? */
            *(s = tmp) = '\0'; /* clear sequence */
            for (i = 1; i <= nmsgs; ++i)
                if (flags[i] & DELE)
                {
                    for (j = i + 1, k = 0; (j <= nmsgs) && (flags[j] & DELE); ++j)
                        k = j;
                    if (k)
                        sprintf(s, ",%lu:%lu", i, k);
                    else
...
void clkint()
{
    ...
    logout = "Autologout";
    if (critical)
        state = LOGOUT; /* badly hosed if in critical code */
    else
...
char *responder(void *challenge, unsigned long clen, unsigned long *rlen)
{
    ...
    if (t[-1] == '\015')
        --t;   /* remove CR */
    *t = '\0'; /* tie off buffer */
    return (resp[0] != '*') ? (char *)rfc822_base64(resp, t - resp, rlen ? rlen : &i) : NIL;
}
    ...
    else
    {
        ...
        for (i = 1; i < j; ++i)
            if (!(elt = mail_elt(stream, i))->deleted)
            {
                ...
                if (elt->seen)
                    il = nmsgs; /* and set up initial LAST */
            }
...
long blat(char *text, long lines, unsigned long size, STRING *st)
{
    ...
    {
        c = *text++;
        d = *text++; /* collect first two bytes */
        if (c == '.')
    ...
    {
        c = SNX(st);
        d = SNX(st); /* collect first two bytes */
        if (c == '.')
            PBOUT('.'); /* double string-leading dot if necessary */
        while (lines && --size)
        {                /* copy loop */
...
void rset()
{
    /* clear all flags */
    if (flags)
        memset((void *)flags, 0, (nmsgs + 1) * sizeof(short));
...
void mm_login(NETMBX *mb, char *username, char *password, long trial)
{
    ...
    if (pass)
    {
        ...
        memset(password, 0, 256); /* no password to send, abort login */
