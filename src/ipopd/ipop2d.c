...
int main(int argc, char *argv[])
{
    char *s, *t;
    char cmdbuf[TMPLEN];
    char *pgmname = (argc && argv[0]) ? (((s = strrchr(argv[0], '/')) || (s = strrchr(argv[0], '\\'))) ? s + 1 : argv[0]) : "ipop2d";
   ...
   */
    ...
    {                       /* command processing loop */
        ...
        {             /* dispatch based on command */
            ...
            if ((state == AUTH) && !strcmp(s, "HELO"))
                state = c_helo(t, argc, argv);
            else if ((state == MBOX || state == ITEM) && !strcmp(s, "FOLD"))
                state = c_fold(t);
            else if ((state == MBOX || state == ITEM) && !strcmp(s, "READ"))
                state = c_read(t);
            else if ((state == ITEM) && !strcmp(s, "RETR"))
                state = c_retr(t);
            else if ((state == NEXT) && !strcmp(s, "ACKS"))
                state = c_acks(t);
            else if ((state == NEXT) && !strcmp(s, "ACKD"))
                state = c_ackd(t);
            else if ((state == NEXT) && !strcmp(s, "NACK"))
                state = c_nack(t);
                     ...
                     !strcmp(s, "QUIT"))
            {
                ...
                if (t)
                    goodbye = "- Bogus argument given to QUIT\015\012";
                else
                { /* expunge the stream */
                    if (stream && nmsgs)
                        stream = mail_close_full(stream, CL_EXPUNGE);
                    stream = NIL; /* don't repeat it */
                }
            }
            else
            { /* some other or inappropriate command */
                server_init(NIL, NIL, NIL, SIG_IGN, SIG_IGN, SIG_IGN, SIG_IGN, SIG_IGN);
...
short c_fold(char *t)
{
    ...
    { /* don't permit proxy to leave IMAP */
        ...
        else if (!mail_valid_net_parse(t, &mb))
...
#endif
    printf("#%lu messages in %s\015\012", nmsgs, stream ? stream->mailbox : "<none>");
    return MBOX;
}
...
short c_read(char *t)
{
    ...
    if (elt)
        sprintf(status, "Status: %s%s\015\012",
...
short c_retr(char *t)
{
    ...
    { /* message size valid? */
        ...
        {           /* only if there is something */
            ...
            while (i)
            { /* blat the header */
                if (!(j = fwrite(t, sizeof(char), i, stdout)))
                    return DONE;
                if (i -= j)
                    t += j; /* advance to incomplete data */
            }
        }
        ...
        if ((t = mail_fetch_text(stream, msg[current], NIL, &i, FT_RETURNSTRINGSTRUCT)) != NULL)
            ...
            for (bs = &stream->private.string; i--;)
                if (putc(SNX(bs), stdout) == EOF)
                    return DONE;
        fputs("\015\012", stdout); /* trailer to coddle PCNFS' NFSMAIL */
    }
    else
        return DONE; /* otherwise go away */
    return NEXT;
}
