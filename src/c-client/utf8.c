...
long utf8_text(SIZEDTEXT *text, char *charset, SIZEDTEXT *ret, long flags)
{
    ...
    const CHARSET *cs = (charset && *charset) ? utf8_charset(charset) : utf8_infercharset(text);
    if (cs)
        return (text && ret) ? utf8_text_cs(text, cs, ret, cv, de) : LONGT;
    if (ret)
                     ...
                     unsigned long errch)
{
        ...
        (scs = (sc && *sc) ? utf8_charset(sc) : utf8_infercharset(src)))
    {
        ...
        /* otherwise do the conversion */
            ...
            ret = (utf8_text_cs(src, scs, &utf8, NIL, NIL) &&
...
{
    return (currmapcs && !compare_cstring(charset, currmapcs->name)) ? currmap : utf8_rmap_cs(utf8_charset(charset));
}
    ...
    { /* have a map? */
        ...
        case CT_1BYTE: /* 1 byte ASCII + table 0x80-0xff */
            for (tab = (unsigned short *)cs->tab, i = 128; i < 256; i++)
                if (tab[i & BITS7] != UBOGON)
                    rmap[tab[i & BITS7]] = (unsigned short)i;
            break;
        case CT_1BYTE8: /* 1 byte table 0x00 - 0xff */
            for (tab = (unsigned short *)cs->tab, i = 0; i < 256; i++)
                if (tab[i] != UBOGON)
                    rmap[tab[i]] = (unsigned short)i;
            break;
        case CT_EUC: /* 2 byte ASCII + utf8_eucparam base/CS2/CS3 */
        ...
        if (rmap[0x00a0] == NOCHAR)
            rmap[0x00a0] = rmap[0x0020];
    }
    ...
    if ((i = utf8_rmapsize(text, rmap, errch, iso2022jp)) != 0L)
    {
        ...
        for (i = text->size; i;)
            if ((u = utf8_get(&s, &i)) != UCS2_BOM)
            {
                ...
                if ((u & U8GM_NONBMP) || ((c = rmap[u]) == NOCHAR))
                    c = errch;
                switch (iso2022jp)
        ...
        if (iso2022jp == 2)
        {                   /* ISO-2022-JP string must end in Roman */
            *t++ = I2C_ESC; /* ESC ( J */
            *t++ = I2C_G0_94;
            *t++ = I2CS_94_JIS_ROMAN;
        }
                            ...
                            unsigned long errch, long iso2022jp)
{
    ...
    unsigned char *s = text->data;
    if (iso2022jp)
        iso2022jp = 1; /* start non-zero ISO-2022-JP state at 1 */
    for (i = text->size; i;)
        if ((u = utf8_get(&s, &i)) != UCS2_BOM)
        {
...
unsigned long utf8_get(unsigned char **s, unsigned long *i)
{
    ...
    /* no, is it surrogate? */
    else if ((ret >= UTF16_SURR) && (ret <= UTF16_MAXSURR))
        ret = U8G_SURROGA;
    /* or in non-Unicode ISO 10646 space? */
    else if (ret > UCS4_MAXUNICODE)
        ret = U8G_NOTUNIC;
    else
    {
...
unsigned long utf8_get_raw(unsigned char **s, unsigned long *i)
{
    ...
    int more = 0;
    do
    { /* make sure have source octets available */
        if (!j--)
	...
	if (((c1 >= 0x80) && (c1 <= 0x8f))) more = 3;
            ...
            { /* U+100000 - U+10ffff (and 110000 - 1fffff) */
                c &= 0x07;
                if ((c1 >= 0x80))
                    more = 3;
            }
            else if (c < 0xfc)
            { /* ISO 10646 200000 - 3ffffff */
                c &= 0x03;
                if ((c1 >= 0x80))
                    more = 4;
            }
            else if (c < 0xfe)
            { /* ISO 10646 4000000 - 7fffffff */
                c &= 0x01;
                if ((c1 >= 0x80))
                    more = 5;
            }
...
unsigned long ucs4_cs_get(CHARSET *cs, unsigned char **s, unsigned long *i)
{
    ...
    if (j--)
        ...
        return U8G_ENDSTRG; /* empty string */
    switch (cs->type)
    {             /* convert if type known */
    case CT_UTF8: /* variable UTF-8 encoded Unicode no table */
        return utf8_get(s, i);
    ...
    case CT_SJIS: /* 2 byte Shift-JIS encoded JIS no table */
        ...
        if (!(c & BIT8))
            ret = (c == JISROMAN_YEN) ? UCS2_YEN : c;
        /* half-width katakana? */
        else if ((c >= MIN_KANA_8) && (c < MAX_KANA_8))
            ret = c + KANA_8;
        else
        { /* Shift-JIS */
            if (j--)
    ...
    case CT_UCS2: /* 2 byte 16-bit Unicode no table */
        ...
        if (j--)
            ...
            return U8G_ENDSTRI; /* empty string */
        ret |= c;
        break;
    case CT_UCS4: /* 4 byte 32-bit Unicode no table */
        if (c & 0x80)
...
unsigned long *utf8_csvalidmap(char *charsets[])
{
	 ...
	 */
    for (i = 0xa720; i < 0xabff; ++i)
        ret[i] = 0x1;
    /* U+AC00 - U+D7FF Hangul Syllables */
    for (i = 0xd800; i < 0xf8ff; ++i)
        ret[i] = 0x1;
    /* U+F900 - U+FAFF CJK Compatibility Ideographs */
    for (i = 0xfb00; i < 0xfe2f; ++i)
        ret[i] = 0x1;
    ...
    for (csi = 1; ret && charsets && (s = charsets[csi - 1]); ++csi)
    {
        ...
        if (!compare_cstring(s, "ISO-2022-JP"))
            s = "EUC-JP";
        /* look up charset */
                   ...
                   ucs4de_t de)
{
    ...
    for (pass = 0, s = NIL, ret->size = 0; pass <= 1; pass++)
    {
        for (i = 0; i < text->size;)
        {
                        ...
                        {                     /* CS2 set up? */
                            if (p2->base_ten) /* yes, multibyte? */
                                c = ((i < text->size) && ((c = text->data[i++]) & BIT8) &&
                                     ((ku = (c1 & BITS7) - p2->base_ku) < p2->max_ku) &&
                                     ((ten = (c & BITS7) - p2->base_ten) < p2->max_ten))
                                        ? t2[(ku * p2->max_ten) + ten]
                                        : UBOGON;
                            else
                                c = ((c1 >= p2->base_ku) && (c1 < p2->max_ku)) ? c1 + ((unsigned long)p2->tab) : UBOGON;
                        }
                        else
                        {               /* CS2 not set up */
                        ...
                        {                     /* CS3 set up? */
                            if (p3->base_ten) /* yes, multibyte? */
                                c = ((i < text->size) && ((c = text->data[i++]) & BIT8) &&
                                     ((ku = (c1 & BITS7) - p3->base_ku) < p3->max_ku) &&
                                     ((ten = (c & BITS7) - p3->base_ten) < p3->max_ten))
                                        ? t3[(ku * p3->max_ten) + ten]
                                        : UBOGON;
                            else
                                c = ((c1 >= p3->base_ku) && (c1 < p3->max_ku)) ? c1 + ((unsigned long)p3->tab) : UBOGON;
                        }
                        else
                        {               /* CS3 not set up */
                            c = UBOGON; /* swallow byte, say bogon */
            ...
            if (pass)
                UTF8_WRITE_BMP(s, c, cv, de)
            else
                UTF8_COUNT_BMP(ret->size, c, cv, de);
        }
        if (!pass)
            (s = ret->data = (unsigned char *)
                     ...
                     ucs4de_t de)
{
    ...
    for (ret->size = i = 0; i < text->size;)
    {
        if ((c = text->data[i++]) & BIT8)
        {
            ...
            if ((c == 0x80) && (t1 == (unsigned short *)gb2312tab))
                c = UCS2_EURO;
            else
                c = ((i < text->size) && (c1 = text->data[i++]) &&
                     ((ku = c - p1->base_ku) < p1->max_ku) &&
                     ((ten = c1 - p1->base_ten) < p1->max_ten))
                        ? t1[(ku * p1->max_ten) + ten]
                        : UBOGON;
        }
        UTF8_COUNT_BMP(ret->size, c, cv, de)
    }
        ...
        if ((c = text->data[i++]) & BIT8)
        {
            if ((i >= text->size) || !(c1 = text->data[i++]))
                ...
                c = ((ku = c - p2->base_ku) < p2->max_ku &&
                     ((ten = c1 - p2->base_ten) < p2->max_ten))
                        ? t[(ku * (p1->max_ten + p2->max_ten)) + p1->max_ten + ten]
                        : UBOGON;
            else
                c = ((ku = c - p1->base_ku) < p1->max_ku &&
                     ((ten = c1 - p1->base_ten) < p1->max_ten))
                        ? t[(ku * (p1->max_ten + p2->max_ten)) + ten]
                        : UBOGON;
        }
        UTF8_COUNT_BMP(ret->size, c, cv, de)
    }
...
void utf8_text_2022(SIZEDTEXT *text, SIZEDTEXT *ret, ucs4cn_t cv, ucs4de_t de)
{
    ...
    for (pass = 0, s = NIL, ret->size = 0; pass <= 1; pass++)
    {
        ...
        gl = I2C_G0;
        gr = I2C_G1; /* left is G0, right is G1 */
        for (i = 0; i < text->size;)
        {
            c = text->data[i++];
            switch (state)
            ...
            case I2S_CHAR: /* character data */
                ...
                default:    /* ordinary character */
                    ...
                    case I2CS_BRITISH: /* British ASCII */
                                       /* Pound sterling sign */
                        if (c == BRITISH_POUNDSTERLING)
                            c = UCS2_POUNDSTERLING;
                        break;
                    ...
                    default: /* all other character sets */
                        ...
                        if ((gi & I2CS_MUL) && !(c & BIT8) && isgraph(c))
                        {
                            ...
                            c = co; /* unknown single byte, treat as 8859-1 */
                    }
                    /* convert if second pass */
                    if (pass)
                        UTF8_WRITE_BMP(s, c, cv, de)
    ...
    for (pass = 0, s = NIL, ret->size = 0; pass <= 1; pass++)
    {
        ...
        for (i = 0, state = NIL; i < text->size;)
        {
            ...
            case U7_UNICODE:        /* Unicode state */
                ...
                { /* decode Unicode */
                    ...
                    if ((c >= 'A') && (c <= 'Z'))
                        ...
                        state = U7_ASCII; /* end of modified BASE64 */
                }
                break;
            ...
            case U7_UNICODE: /* Unicode */
                ...
                if (e == 1)
                    break; /* done if first BASE64 state */
                if (!e1)
                ...
                if ((c >= UTF16_SURR) && (c <= UTF16_MAXSURR))
                {
                    ...
                    if (c < UTF16_SURRL)
                        ...
                        c = UTF16_BASE + ((surrh & UTF16_MASK) << UTF16_SHIFT) +
                            (c & UTF16_MASK);
...
void utf8_text_utf16(SIZEDTEXT *text, SIZEDTEXT *ret, ucs4cn_t cv, ucs4de_t de)
{
    ...
    for (ret->size = 0, t = text->data, i = text->size / 2; i; --i)
    {
        ...
        {
            /* invalid first surrogate */
            if ((c > UTF16_SURRHEND) || !i)
                c = UBOGON;
            else
            { /* get second surrogate */
                d = *t++ << 8;
                d |= *t++;
                ...
                if ((d < UTF16_SURRL) || (d > UTF16_SURRLEND))
                    ...
                    c = UTF16_BASE + ((c & UTF16_MASK) << UTF16_SHIFT) +
                        (d & UTF16_MASK);
            }
        }
        UTF8_COUNT(ret->size, c, cv, de);
...
{
    if (c <= UCS4_TMAPMAX)
        return ucs4_tmaptab[c];
    if (c < UCS4_TMAPHIMIN)
        return c;
    if (c <= UCS4_TMAPHIMAX)
        return c - UCS4_TMAPHIMAP;
    if (c < UCS4_TMAPDESERETMIN)
        return c;
    if (c <= UCS4_TMAPDESERETMAX)
...
long ucs4_width(unsigned long c)
{
    ...
    if ((c > UCS4_MAXUNICODE) || ((c & 0xfffe) == 0xfffe) ||
        ((c >= UTF16_SURR) && (c <= UTF16_MAXSURR)))
        ret = U4W_NOTUNCD;
    /* private-use */
    else if (c >= UCS4_PVTBASE)
        ret = U4W_PRIVATE;
    /* SSP are not printing characters */
    else if (c >= UCS4_SSPBASE)
        ret = U4W_SSPCHAR;
    /* unassigned planes */
    else if (c >= UCS4_UNABASE)
        ret = U4W_UNASSGN;
    /* SIP and reserved plane 3 are wide */
    else if (c >= UCS4_SIPBASE)
        ret = 2;
...
#error "UCS4_WIDLEN != UCS4_SIPBASE"
    ...
    /* BMP and SMP get value from table */
        ...
        switch (ret = (ucs4_widthtab[(c >> 2)] >> ((3 - (c & 0x3)) << 1)) & 0x3)
        {
        case 0: /* zero-width */
            if (c == 0x00ad)
                ret = 1; /* force U+00ad (SOFT HYPHEN) to width 1 */
        case 1:          /* single-width */
        case 2:          /* double-width */
            break;
...
unsigned long ucs4_decompose(unsigned long c, void **more)
{
    ...
    { /* want to chase more? */
        /* do sanity check */
        if ((m = (struct decomposemore *)*more) != NULL)
            switch (m->type)
            {
            ...
            fatal("no more block provided to ucs4_decompose!");
    }
    ...
    {                /* start decomposition */
        *more = NIL; /* initially set no more */
        ...
        if (c < UCS4_BMPLOMIN)
            ret = c;
        /* fix this someday */
        else if (c == UCS4_BMPLOMIN)
            ret = ucs4_dbmplotab[0];
        else if (c <= UCS4_BMPLOMAX)
        {
            ...
            if ((i = ucs4_dbmploixtab[c - UCS4_BMPLOMIN]) != 0L)
            {
                ...
                {
                    m = (struct decomposemore *)(*more = memset(fs_get(sizeof(struct decomposemore)), 0,
        ...
        else if (c < UCS4_BMPHALFFULLMIN)
            ret = c;
        else if (c <= UCS4_BMPHALFFULLMAX)
        {
            if (!(ret = ucs4_bmphalffulldecomptab[c - UCS4_BMPHALFFULLMIN]))
                ret = c;
        }
        /* SMP music */
        else if (c < UCS4_SMPMUSIC1MIN)
            ret = c;
        ...
        else if (c < UCS4_SMPMATHMIN)
            ret = c;
        else if (c <= UCS4_SMPMATHMAX)
        {
            if (!(ret = ucs4_smpmathdecomptab[c - UCS4_SMPMATHMIN]))
                ret = c;
        }
        /* CJK compatibility ideographs in SIP */
        else if (!(ret = ((c >= UCS4_SIPMIN) && (c <= UCS4_SIPMAX)) ? ucs4_sipdecomptab[c - UCS4_SIPMIN] : c))
            ret = c;
    }
    return ret;
}
...
unsigned long ucs4_decompose_recursive(unsigned long c, void **more)
{
    ...
    { /* want to chase more? */
        ...
        if ((mr = (RECURSIVEMORE *)*more) != NULL)
            ...
            fatal("no more block provided to ucs4_decompose_recursive!");
        if ((mr = mn) != NULL)
    ...
    {                /* start decomposition */
        *more = NIL; /* initially set no more */
        mr = NIL;
        do
        { /* repeatedly decompose this codepoint */
