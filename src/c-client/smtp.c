                           ...
                           unsigned long port, long options)
{
    ...
    /* maximum domain name is 64 characters */
        ...
        do
            if (strlen(*hostlist) < SMTPMAXDOMAIN)
            {
                ...
                { /* light tryssl flag if requested */
                    ...
                    /* else port argument overrides SMTP port */
                        ...
                        port = smtp_port ? smtp_port : SMTPTCPPORT;
                                  ...
                                  "*smtps", smtp_sslport ? smtp_sslport : SMTPSSLPORT)) != NULL)
                    {
                        ...
                        if (options & SOP_SECURE)
                            mb.secflag = T;
                        /* get name of local host to use */
                        s = compare_cstring("localhost", mb.host) ? net_localhost(netstream) : "localhost";

                        do
                            reply = smtp_reply(stream);
                        ...
                        else
                        {
                                ...
                                (smtp_send(stream, "STARTTLS", NIL) == SMTPGREET))
                            {
                                ...
                                if (!(stream->netstream->stream = (*stls)(stream->netstream->stream, mb.host,
                            ...
                            if (stream && ((mb.secflag || mb.user[0])))
                            {
                                ...
                                { /* use authenticator? */
                                    if ((long)mail_parameters(NIL, GET_TRUSTDNS, NIL))
                                    {
                                        /* remote name for authentication */
                                        strncpy(mb.host,
                                                (long)mail_parameters(NIL, GET_SASLUSESPTRNAME, NIL) ? net_remotehost(netstream) : net_host(netstream),
                                                NETMAXHOST - 1);
                                        mb.host[NETMAXHOST - 1] = '\0';
                                    }
                                    if (!smtp_auth(stream, &mb, tmp))
                                        stream = smtp_close(stream);
                                }
                                else
                                { /* no available authenticators? */
    ...
    { /* set stream options if have a stream */
        ...
        {
            ESMTP.dsn.want = T;
            if (options & SOP_DSN_NOTIFY_FAILURE)
                ESMTP.dsn.notify.failure = T;
            if (options & SOP_DSN_NOTIFY_DELAY)
                ESMTP.dsn.notify.delay = T;
            if (options & SOP_DSN_NOTIFY_SUCCESS)
                ESMTP.dsn.notify.success = T;
            if (options & SOP_DSN_RETURN_FULL)
                ESMTP.dsn.full = T;
        }
        if (options & SOP_8BITMIME)
            ESMTP.eightbit.want = T;
    }
    return stream;
}
...
SENDSTREAM *smtp_close(SENDSTREAM *stream)
{
    ...
    { /* send "QUIT" */
        ...
        if (ESMTP.dsn.envid)
            fs_give((void **)&ESMTP.dsn.envid);
        if (ESMTP.atrn.domains)
            fs_give((void **)&ESMTP.atrn.domains);
...
long smtp_mail(SENDSTREAM *stream, char *type, ENVELOPE *env, BODY *body)
{
    ...
    sprintf(smtpserver, "{%.200s/smtp%s}<none>",
            (long)mail_parameters(NIL, GET_TRUSTDNS, NIL) ? ((long)mail_parameters(NIL, GET_SASLUSESPTRNAME, NIL) ? net_remotehost(stream->netstream) : net_host(stream->netstream)) : stream->host,
    ...
    do
    {
        ...
        { /* need to retry with authentication? */
            ...
            if (!smtp_auth(stream, &mb, smtpserver))
                return NIL;
        ...
        case SMTPWANTAUTH2:
            if (ESMTP.auth)
                retry = T; /* yes, retry with authentication */
        ...
        if (!retry && env->to)
            retry = smtp_rcpt(stream, env->to, &error);
        if (!retry && env->cc)
            retry = smtp_rcpt(stream, env->cc, &error);
        if (!retry && env->bcc)
            retry = smtp_rcpt(stream, env->bcc, &error);
        if (!retry && error)
        ...
        { /* ignore group syntax */
            ...
            else
            {
                ...
                if (ESMTP.ok && ESMTP.dsn.ok && ESMTP.dsn.want)
                {
                    ...
                    strcat(tmp, " NOTIFY=");
                    s = tmp + strlen(tmp);
                    if (ESMTP.dsn.notify.failure)
                        strcat(s, "FAILURE,");
                    if (ESMTP.dsn.notify.delay)
                        strcat(s, "DELAY,");
                    if (ESMTP.dsn.notify.success)
                        strcat(s, "SUCCESS,");
                    /* tie off last comma */
                    if (*s)
                        s[strlen(s) - 1] = '\0';
...
long smtp_send(SENDSTREAM *stream, char *command, char *args)
{
    ...
    {
        do
            stream->replycode = smtp_reply(stream);
        while ((stream->replycode < 100) || (stream->reply[3] == '-'));
        ret = stream->replycode;
    }
    else
        ret = smtp_fake(stream, "SMTP connection broken (command)");
    fs_give((void **)&s);
    return ret;
}
...
long smtp_reply(SENDSTREAM *stream)
{
    ...
    if (stream->netstream && (stream->reply = net_getline(stream->netstream)))
    {
        ...
        reply = smtp_fake(stream, "SMTP connection broken (reply)");
    return reply;
}
    ...
    do
        if ((i = smtp_reply(stream)) == SMTPOK)
        {
            ...
            else if ((t = strtok_r(NIL, " ", &r)) && *t)
            {
                ...
                {
                    if (isdigit(*t))
                        ESMTP.size.limit = strtoul(t, &t, 10);
                    ESMTP.size.ok = T;
                }
                else if (!compare_cstring(s, "DELIVERBY"))
                {
                    if (isdigit(*t))
                        ESMTP.deliverby.minby = strtoul(t, &t, 10);
                ...
                else if (!compare_cstring(s, "ATRN"))
                {
                    ...
                    do
                        if ((j = mail_lookup_auth_name(t, flags)) &&
                            (--j < MAXAUTHENTICATORS))
                            ESMTP.auth |= (1 << j);
                    while ((t = strtok_r(NIL, " ", &r)) && *t);
            }
            /* EHLO options which do not take arguments */
            else if (!compare_cstring(s, "SIZE"))
                ESMTP.size.ok = T;
            else if (!compare_cstring(s, "8BITMIME"))
                ESMTP.eightbit.ok = T;
            else if (!compare_cstring(s, "DSN"))
                ESMTP.dsn.ok = T;
            else if (!compare_cstring(s, "ATRN"))
                ESMTP.atrn.ok = T;
            else if (!compare_cstring(s, "SEND"))
                ESMTP.service.send = T;
            else if (!compare_cstring(s, "SOML"))
                ESMTP.service.soml = T;
            else if (!compare_cstring(s, "SAML"))
                ESMTP.service.saml = T;
            else if (!compare_cstring(s, "EXPN"))
                ESMTP.service.expn = T;
            else if (!compare_cstring(s, "HELP"))
                ESMTP.service.help = T;
            else if (!compare_cstring(s, "TURN"))
                ESMTP.service.turn = T;
            else if (!compare_cstring(s, "ETRN"))
                ESMTP.service.etrn = T;
            else if (!compare_cstring(s, "STARTTLS"))
                ESMTP.service.starttls = T;
            else if (!compare_cstring(s, "RELAY"))
                ESMTP.service.relay = T;
            else if (!compare_cstring(s, "PIPELINING"))
                ESMTP.service.pipe = T;
            else if (!compare_cstring(s, "ENHANCEDSTATUSCODES"))
                ESMTP.service.ensc = T;
            else if (!compare_cstring(s, "BINARYMIME"))
                ESMTP.service.bmime = T;
            else if (!compare_cstring(s, "CHUNKING"))
                ESMTP.service.chunk = T;
        }
