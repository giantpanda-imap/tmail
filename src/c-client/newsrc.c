...
FILE *newsrc_create(MAILSTREAM *stream, int notify)
{
    ...
    if (!f)
        newsrc_error("Unable to create news state %.80s", newsrc, ERROR);
    else if (notify)
        newsrc_error("Creating news state %.80s", newsrc, WARN);
    return f;
}
    ...
    { /* have any messages? */
             ...
             i <= stream->nmsgs; ++i)
        {
            ...
            if ((elt = mail_elt(stream, i))->deleted)
            {
                ...
                if (!j)
                    j = k; /* if no range in progress, start one */
            }
            else if (j)
            { /* unread message, ending a range */
                /* calculate end of range */
                if ((k = elt->private.uid - 1) != 0L)
                {
                    /* dump range */
                    if (j == k)
                        sprintf(tmp, "%c%ld", c, j);
                    else
                        sprintf(tmp, "%c%ld-%ld", c, j, k);
                    if (fputs(tmp, f) == EOF)
                        return NIL;
...
void newsrc_lsub(MAILSTREAM *stream, char *pattern)
{
    ...
    { /* got file? */
        /* remote name? */
        if (*(lcl = strcpy(name, pattern)) == '{')
            lcl = strchr(lcl, '}') + 1;
        if (*lcl == '#')
            lcl += 6; /* namespace format name? */
        while (c != EOF)
        { /* yes, read newsrc */
            for (s = lcl; (s < (name + MAILTMPLEN - 1)) && ((c = getc(f)) != EOF) &&
                ...
                ;
            if (c == ':')
            {              /* found a subscribed newsgroup? */
                *s = '\0'; /* yes, tie off name */
                           /* report if match */
                if (pmatch_full(name, pattern, '.'))
                    mm_lsub(stream, '.', name, NIL);
                else
            ...
            *s = '\0'; /* tie off name */
            ...
            if (((c == ':') || (c == '!')) && !strcmp(tmp, group))
            {
                ...
                { /* already at that state? */
                    if (c == ':')
                        newsrc_error("Already subscribed to %.80s", group, WARN);
                    ret = LONGT; /* noop the update */
                }
                /* write the character */
                else if (!fseek(f, pos, 0))
                    ret = ((putc(state, f)) == EOF) ? NIL : LONGT;
                if (fclose(f) == EOF)
                    ret = NIL;
            ...
            if (!nl[0] && ((c == '\015') || (c == '\012')) && ((nl[0] = c) == '\015'))
            {
                ...
                if ((c = getc(f)) == '\012')
                    ...
                    ungetc(c, f); /* nope, push it back */
            }
        } while (c != EOF);

        if (f)
        { /* still haven't written it yet? */
            if (nl[0])
            {                    /* know its newline convention? */
                fseek(f, 0L, 2); /* yes, seek to end of file */
                ret = newsrc_newstate(f, group, state, nl);
            }
            else
            {              /* can't find a newline convention */
                fclose(f); /* punt the file */
                           /* can't win if read something */
                if (pos)
                    newsrc_error("Unknown newline convention in %.80s",
                                 newsrc, ERROR);
                /* file must have been empty, rewrite it */
                else
                    ret = newsrc_newstate(newsrc_create(stream, NIL), group, state, "\n");
            }
        }
    }
    /* create new file */
    else
        ret = newsrc_newstate(newsrc_create(stream, T), group, state, "\n");
...
long newsrc_read(char *group, MAILSTREAM *stream)
{
                    ...
                    if (stream->nmsgs)
                        while (f && (m <= stream->nmsgs))
                        {
                            ...
                            { /* better have a number */
                                for (i = 0, j = 0; isdigit(c); c = getc(f))
                                    i = i * 10 + (c - '0');
                                if (c == '-')
                                    for (c = getc(f); isdigit(c); c = getc(f))
                                        j = j * 10 + (c - '0'); /* collect second value if range */
                                if (!unseen && (mail_elt(stream, m)->private.uid < i))
                                    unseen = m;
                                ...
                                while ((m <= stream->nmsgs) && (elt = mail_elt(stream, m)) &&
                                       (j ? ((elt->private.uid >= i) && (elt->private.uid <= j)) : (elt->private.uid == i)) && m++)
    ...
    if (f)
    { /* still have file open? */
        sprintf(tmp, "No state for newsgroup %.80s found, reading as new", group);
        MM_LOG(tmp, WARN);
        fclose(f); /* close the file */
    }
    if (m <= stream->nmsgs)
    { /* any messages beyond newsrc range? */
        if (!unseen)
            unseen = m; /* then this must be the first unseen one */
        do
        {
...
long newsrc_write(char *group, MAILSTREAM *stream)
{
    ...
    FILE *f, *bf;
    nl[0] = nl[1] = nl[2] = '\0'; /* no newline known yet */
    if ((f = fopen(newsrc, "rb")) != NULL)
    { /* have existing newsrc file? */
        ...
        if (fclose(bf) == EOF) /* and backup file */
            return newsrc_error("Error closing backup news state %.80s",
                                newsrc, ERROR);
        if (d == EOF)
        { /* open for write if empty file */
            if ((f = newsrc_create(stream, NIL)) != NULL)
                bf = NIL;
            else
                return NIL;
        }
    ...
    { /* create new newsrc file */
        if ((f = newsrc_create(stream, T)) != NULL)
            bf = NIL;
        else
            return NIL; /* can't create newsrc */
    }
        ...
        *s = '\0'; /* tie off name */
        ...
        if (tmp[0] && ((c == ':') || (c == '!')))
        {
            ...
            { /* found correct group? */
                ...
                /* done with file */
                if (fclose(f) == EOF)
                    return newsrc_write_error(newsrc, bf, NIL);
                f = NIL;
            }
            ...
            *s = '\0'; /* tie off name */
            ...
            {                           /* found newsgroup? */
                ...
                { /* yes, skip leading whitespace */
                    ...
                    if (fseek(f, pos, SEEK_SET) < 0 || fread(s, (size_t)1, size, f) != size)
                      ...
                      unsigned long *recent, unsigned long *unseen)
{
    unsigned long i, j;
    while (*state)
    { /* until run out of state string */
        /* collect a number */
        for (i = 0; isdigit(*state); i = i * 10 + (*state++ - '0'))
            ;
        if (*state != '-')
            j = i; /* coerce single mesage into range */
        else
        { /* have a range */
            ...
            if (!j)
                j = i; /* guard against -0 */
            if (j < i)
                return; /* bogon if end less than start */
        }
        if (*state == ',')
            state++; /* skip past comma */
        else if (*state)
            return; /* otherwise it's a bogon */
        if (uid <= j)
        { /* covered by upper bound? */
            if (uid < i)
                ++*unseen; /* unseen if not covered by lower bound */
