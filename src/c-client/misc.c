...
long search(unsigned char *base, long basec, unsigned char *pat, long patc)
{
    ...
    {
        if (patc <= 0)
            return T;         /* empty pattern always succeeds */
...
unsigned long hash_index(HASHTAB *hashtab, char *key)
{
    ...
    for (ret = 0; (i = (unsigned int)*key++) != 0L; ret += i)
        ret *= HASHMULT;
...
int compare_string(unsigned char *s1, unsigned char *s2)
{
    int i;
    if (!s1)
        return s2 ? -1 : 0; /* empty string cases */
    else if (!s2)
        return 1;
    for (; *s1 && *s2; s1++, s2++)
        if ((i = (compare_ulong(*s1, *s2))) != 0)
            return i;
    if (*s1)
...
int compare_csizedtext(unsigned char *s1, SIZEDTEXT *s2)
{
    ...
    unsigned long j;
    if (!s1)
        return s2 ? -1 : 0; /* null string cases */
    else if (!s2)
        return 1;
    for (s = (char *)s2->data, j = s2->size; *s1 && j; ++s1, ++s, --j)
        if ((i = (compare_uchar(*s1, *s))) != 0)
            return i;
    if (*s1)
        return 1;      /* first string is longer */
