...
void *imap_parameters(long function, void *value)
{
    ...
    case GET_NAMESPACE:
        ...
        break;
    case GET_THREADERS:
        value = (void *)((IMAPLOCAL *)((MAILSTREAM *)value)->local)->cap.threader;
        break;
                    ...
                    char *contents)
{
    ...
    { /* want to do a scan? */
        ...
        { /* make sure permitted */
            ...
            args[3] = NIL;
            aref.type = ASTRING;
            aref.text = (void *)(ref ? ref : "");
            apat.type = LISTMAILBOX;
            apat.text = (void *)pat;
            acont.type = ASTRING;
            acont.text = (void *)contents;
    ...
    { /* easy if IMAP4 */
        ...
        if (LOCAL->cap.mbx_ref && mail_parameters(stream, GET_IMAPREFERRAL, NIL))
        {
            ...
            if (!compare_cstring(cmd, "LIST"))
                cmd = "RLIST";
            /* and convert LSUB -> RLSUB */
            else if (!compare_cstring(cmd, "LSUB"))
                cmd = "RLSUB";
        }
        imap_send(stream, cmd, args);
    }
...
long imap_subscribe(MAILSTREAM *stream, char *mailbox)
{
    ...
    long ret = ((stream && LOCAL && LOCAL->netstream) ||
                (stream = mail_open(NIL, mailbox, OP_HALFOPEN | OP_SILENT)))
                   ? imap_manage(stream, mailbox, LEVELIMAP4(stream) ? "Subscribe" : "Subscribe Mailbox", NIL)
                   : NIL;
...
long imap_unsubscribe(MAILSTREAM *stream, char *mailbox)
{
    ...
    long ret = ((stream && LOCAL && LOCAL->netstream) ||
                (stream = mail_open(NIL, mailbox, OP_HALFOPEN | OP_SILENT)))
                   ? imap_manage(stream, mailbox, LEVELIMAP4(stream) ? "Unsubscribe" : "Unsubscribe Mailbox", NIL)
                   : NIL;
    /* toss out temporary stream */
    if (st != stream)
        mail_close(stream);
         ...
         (stream = mail_open(NIL, mailbox, OP_HALFOPEN | OP_SILENT))))
    {
            ...
            ir && LOCAL->referral)
        {
            ...
            if ((code >= 0) && (mailbox = (*ir)(stream, LOCAL->referral, code)))
                ret = imap_manage(NIL, mailbox, command, (*command == 'R') ? (mailbox + strlen(mailbox) + 1) : NIL);
        }
...
MAILSTREAM *imap_open(MAILSTREAM *stream)
{
    ...
    { /* if stream opened earlier by us */
        ...
        if (LOCAL->netstream && (!stream->halfopen || LOCAL->cap.unselect))
        {
            ...
            { /* was stream still alive? */
                ...
                if (LOCAL->user)
                    sprintf(tmp + strlen(tmp), "/user=\"%s\"",
                            LOCAL->user);
                if (!stream->silent)
                    mm_log(tmp, (long)NIL);
                /* unselect if now want halfopen */
                if (stream->halfopen)
                    imap_send(stream, "UNSELECT", NIL);
            }
            else
     ...
     */
        ...
        if (LOCAL->netstream && strcmp(reply->key, "PREAUTH"))
        {
            ...
            { /* still in the land of the living? */
                if ((long)mail_parameters(NIL, GET_TRUSTDNS, NIL))
                {
                    ...
                    strncpy(mb.host, (long)mail_parameters(NIL, GET_SASLUSESPTRNAME, NIL) ? net_remotehost(LOCAL->netstream) : net_host(LOCAL->netstream), NETMAXHOST - 1);
                    mb.host[NETMAXHOST - 1] = '\0';
                }
                /* need new capabilities after login */
                LOCAL->gotcapability = NIL;
                if (!(stream->anonymous ? imap_anon(stream, tmp) : (LOCAL->cap.auth ? imap_auth(stream, &mb, tmp, usr) : imap_login(stream, &mb, tmp, usr))))
                {
        ...
        if (LOCAL->netstream && !LOCAL->gotcapability)
            imap_capability(stream);
    ...
    { /* still have a connection? */
        ...
        sprintf(tmp, "{%s", (long)mail_parameters(NIL, GET_TRUSTDNS, NIL) ? net_host(LOCAL->netstream) : mb.host);
        ...
        if (stream->rdonly)
            strcat(tmp, "/readonly");
        if (stream->anonymous)
            strcat(tmp, "/anonymous");
        else
        { /* record user name */
            if (!LOCAL->user && usr[0])
                LOCAL->user = cpystr(usr);
            if (LOCAL->user)
                sprintf(tmp + strlen(tmp), "/user=\"%s\"",
                        LOCAL->user);
        }
        ...
        { /* wants to open a mailbox? */
            ...
            args[1] = NIL;
            stream->nmsgs = 0;
            if (imap_OK(stream, reply = imap_send(stream, stream->rdonly ? "EXAMINE" : "SELECT", args)))
            {
...
long imap_anon(MAILSTREAM *stream, char *tmp)
{
    ...
    if (LOCAL->cap.authanon)
    {
        ...
        if (imap_challenge(stream, &i))
            imap_response(stream, s, strlen(s));
        /* get response */
...
long imap_login(MAILSTREAM *stream, NETMBX *mb, char *pwd, char *usr)
{
    ...
    { /* OK to try login */
        ...
        do
        {
            ...
            if (pwd[0])
            {                         /* send login command if have password */
...
void imap_close(MAILSTREAM *stream, long options)
{
    ...
    { /* send "LOGOUT" */
        ...
        if (LOCAL->threaddata)
            mail_free_threadnode(&LOCAL->threaddata);
        /* flush threaders */
        if ((thr = LOCAL->cap.threader) != NULL)
            while ((t = thr) != NULL)
            {
                fs_give((void **)&t->name);
                thr = t->next;
        ...
        if (LOCAL->user)
            fs_give((void **)&LOCAL->user);
        if (LOCAL->reply.line)
            fs_give((void **)&LOCAL->reply.line);
        if (LOCAL->reform)
            fs_give((void **)&LOCAL->reform);
        if (LOCAL->id)
            mail_free_idlist(&LOCAL->id);
        /* nuke the local data */
...
{ /* send "FETCH sequence FLAGS" */
    ...
    if (LOCAL->loser)
        sequence = imap_reform_sequence(stream, sequence,
                                        flags & FT_UID);
    aseq.type = SEQUENCE;
                         ...
                         long flags)
{
     ...
     */
        if (!imap_OK(stream, reply = imap_fetch(stream, seq, FT_NEEDENV + (body ? FT_NEEDBODY : NIL) + (flags & (FT_UID + FT_NOHDRS)))))
            mm_log(reply->text, ERROR);
             ...
             (body && !*b && LEVELIMAP2bis(stream))))
        {
            ...
            { /* have a lookahead list? */
                ...
                for (k = imap_fetchlookaheadlimit;
                     ...
                     set = set->next)
                {
                    i = (set->first == 0xffffffff) ? stream->nmsgs : min(set->first, stream->nmsgs);
                    if ((j = (set->last == 0xffffffff) ? stream->nmsgs : min(set->last, stream->nmsgs)) != 0L)
                    {
                        if (i > j)
     ...
     */
        if (!elt->private.uid && LEVELIMAP4(stream))
            strcpy(tmp, " UID");
        else
        ...
        if (!elt->day)
            strcat(tmp, " INTERNALDATE");
        if (!elt->rfc822_size)
            strcat(tmp, " RFC822.SIZE");
        if (tmp[0])
        {                            /* anything to do? */
                  ...
                  long flags)
{
    ...
    { /* no, do it the easy way */
        aseq.type = NUMBER;
        aseq.text = (void *)msgno;
    }
    aatt.type = ATOM; /* assume atomic attribute */
    alns.type = LIST;
    alns.text = (void *)lines;
    acls.type = BODYCLOSE;
    acls.text = (void *)partial;
    aflg.type = ATOM;
    aflg.text = (void *)"FLAGS";
    args[0] = &aseq;
    args[1] = &aatt;
    args[2] = args[3] = args[4] = NIL;
    auxargs[0] = &aseq;
    auxargs[1] = &aflg;
    auxargs[2] = NIL;
    ...
    { /* easy if IMAP4rev1 server */
        ...
        if (!strcmp(section, "HEADER") && (lines || (flags & FT_PREFETCHTEXT)))
        {
            ...
            { /* want specific header lines? */
                ...
                aatt.text = (void *)((flags & FT_NOT) ? "HEADER.FIELDS.NOT" : "HEADER.FIELDS");
                args[2] = &alns;
                args[3] = &acls;
            }
            /* must be prefetching */
            else
                aatt.text = (void *)((flags & FT_PEEK) ? "(BODY.PEEK[HEADER] BODY.PEEK[TEXT])" : "(BODY[HEADER] BODY[TEXT])");
        }
        else
        { /* simple case */
            aatt.type = (flags & FT_PEEK) ? BODYPEEK : BODYTEXT;
        ...
        if (first || last)
            sprintf(partial, "<%lu.%lu>", first, last ? last : -1);
    }
   ...
   */
    ...
    { /* IMAP4 (RFC 1730) compatibility */
        ...
        if (!strcmp(section, "HEADER"))
            aatt.text = (void *)((flags & FT_PREFETCHTEXT) ? ((flags & FT_PEEK) ? "(BODY.PEEK[0] RFC822.TEXT.PEEK)" : "(BODY[0] RFC822.TEXT)") : ((flags & FT_PEEK) ? "BODY.PEEK[0]" : "BODY[0]"));
        ...
        if (strstr(section, ".MIME") || strstr(section, ".TEXT"))
            noextend = "4";
        if (first || last)
            nopartial = "4";
        if (lines)
   ...
   */
    ...
    else if (LEVELIMAP2bis(stream))
    {
        ...
        if (!strcmp(section, "HEADER"))
        {
            aatt.text = (void *)((flags & FT_PREFETCHTEXT) ? "(RFC822.HEADER RFC822.TEXT)" : "RFC822.HEADER");
            if (flags & FT_PEEK)
                flags &= ~FT_PEEK;
            else
                nononpeek = "2bis";
   ...
   */
    ...
    { /* IMAP2 (RFC 1176/1064) compatibility */
        /* BODY[HEADER] */
        if (!strcmp(section, "HEADER"))
        {
            aatt.text = (void *)((flags & FT_PREFETCHTEXT) ? "(RFC822.HEADER RFC822.TEXT)" : "RFC822.HEADER");
        ...
        /* BODY[] becomes RFC822 */
            ...
            noextend = "2"; /* how did we get here? */
        if (flags & FT_PEEK)
            nopeek = "2";
        if (first || last)
            nopartial = "2";
        ...
        if (!(i = mail_elt(stream, msgno)->seen))
        {
            sprintf(tmp, nopeek ? /* only babble if \Seen not set */
                             "[NOTIMAP4] Simulating peeking fetch in IMAP%s"
                                : "[NOTIMAP4] Simulating non-peeking header fetch in IMAP%s",
                    t);
...
unsigned long imap_msgno(MAILSTREAM *stream, unsigned long uid)
{
   ...
   */
    ...
    {
        if (!(i = mail_elt(stream, msgno)->private.uid))
            holes = T;
        else if (i == uid)
            return msgno;
    }
...
long imap_search(MAILSTREAM *stream, char *charset, SEARCHPGM *pgm, long flags)
{
    ...
    { /* do server-based SEARCH */
        ...
        { /* optional charset argument requested */
            ...
            args[0] = &apgm; /* no charset argument */
            ...
            !strcmp(reply->key, "BAD"))
        {
            ...
            for (set = ss; set; set = set->next)
                if ((i = set->first) != 0L)
                {
                    ...
                    while (i <= j)
                        mail_elt(stream, i++)->private.filter = T;
                }
    ...
    {                   /* only if prefetching permitted */
        ...
        *s = '\0';      /* initially nothing */
        ...
        if (LOCAL->tmp[0])
        { /* anything to pre-fetch? */
            /* pre-fetch envelopes for the first imap_prefetch number of messages */
            if (!imap_OK(stream, reply =
                                     imap_fetch(stream, s = cpystr(LOCAL->tmp), FT_NEEDENV + ((flags & SE_NOHDRS) ? FT_NOHDRS : NIL) + ((flags & SE_NEEDBODY) ? FT_NEEDBODY : NIL))))
                mm_log(reply->text, ERROR);
                         ...
                         SORTPGM *pgm, long flags)
{
        ...
        (!spg || (LEVELWITHIN(stream) || !(spg->older || spg->younger))))
    {
        ...
        if (!strcmp(reply->key, "BAD"))
            return (flags & SE_NOLOCAL) ? NIL : imap_sort(stream, charset, spg, pgm, flags | SE_NOSERVER);
        /* server sorted OK? */
        else if (imap_OK(stream, reply))
        {
    ...
    else if (stream->scache)
        ret = mail_sort_msgs(stream, charset, spg, pgm, flags);
    else
    { /* try to be a bit more clever */
        char *s, *t;
        unsigned long len;
        MESSAGECACHE *elt;
        SORTCACHE **sc;
        ...
        for (sp = pgm; sp && !ftflags; sp = sp->next)
            switch (sp->function)
            {
            case SORTDATE:
            case SORTFROM:
                        ...
                        SEARCHPGM *spg, long flags)
{
    ...
    return (flags & SE_NOLOCAL) ? NIL : mail_thread_msgs(stream, type, charset, spg, flags | SE_NOSERVER, imap_sort);
}
                             ...
                             SEARCHPGM *spg, long flags)
{
    ...
    /* do locally if server barfs */
    if (!strcmp(reply->key, "BAD"))
        ret = (flags & SE_NOLOCAL) ? NIL : mail_thread_msgs(stream, type, charset, spg, flags | SE_NOSERVER, imap_sort);
    /* server threaded OK? */
    else if (imap_OK(stream, reply))
    {
...
long imap_ping(MAILSTREAM *stream)
{
    return (LOCAL->netstream && /* send "NOOP" */
            imap_OK(stream, imap_send(stream, "NOOP", NIL)))
               ? T
               : NIL;
}
...
long imap_expunge(MAILSTREAM *stream, char *sequence, long options)
{
    ...
    { /* wants selective expunging? */
        ...
        else if (mail_sequence(stream, sequence))
        {
            ...
            for (*s = '\0', i = 1; i <= stream->nmsgs; ++i)
                ...
                {
                    if (t[0])
                        *s++ = ','; /* prepend with comma if not first time */
                    sprintf(s, "%lu", mail_uid(stream, j = i));
                    s += strlen(s); /* point at end of string */
                                    /* search for possible end of range */
                    while ((i < stream->nmsgs) && mail_elt(stream, i + 1)->sequence)
                        i++;
                    if (i != j)
                    { /* output end of range */
    ...
    /* ordinary EXPUNGE */
        ...
        ret = imap_OK(stream, reply = imap_send(stream, "EXPUNGE", NIL));
    if (reply)
...
long imap_copy(MAILSTREAM *stream, char *sequence, char *mailbox, long flags)
{
    ...
    { /* success, delete messages if move */
        if (flags & CP_MOVE)
            imap_flag(stream, sequence, "\\Deleted",
...
long imap_append(MAILSTREAM *stream, char *mailbox, append_t af, void *data)
{
    ...
    if (mail_valid_net(mailbox, &imapdriver, NIL, tmp))
    {
        ...
        if ((stream && LOCAL && LOCAL->netstream) ||
            (stream = mail_open(NIL, mailbox, OP_HALFOPEN | OP_SILENT | (debug ? OP_DEBUG : NIL))))
        {
            /* note mailbox in case APPENDUID */
            LOCAL->appendmailbox = mailbox;
            /* use multi-append? */
            ...
            /* do succession of single appends */
                ...
                while ((*af)(stream, data, &map.flags, &map.date, &map.message) &&
                       map.message &&
                          ...
                          APPENDDATA *map, long options)
{
    ...
    {
        /* create a stream if given one no good */
        if (!(stream = mail_open(NIL, mailbox, OP_HALFOPEN | OP_SILENT | (options ? OP_DEBUG : NIL))))
        {
        ...
        if (!(mailbox = (ir && LOCAL->referral) ? (*ir)(stream, LOCAL->referral, REFAPPEND) : NIL))
            mm_log(reply->text, ERROR);
...
long imap_setquota(MAILSTREAM *stream, char *qroot, STRINGLIST *limits)
{
    ...
    { /* send "SETQUOTA" */
        ...
        aqrt.type = ASTRING;
        aqrt.text = (void *)qroot;
        alim.type = SNLIST;
        alim.text = (void *)limits;
        args[0] = &aqrt;
        args[1] = &alim;
        args[2] = NIL;
    ...
    { /* send "ID (params)" */
        ...
        if (len > 0)
        {
            ...
            *p = ')';
            ...
            mm_log("Empty or malformed ID list", ERROR);
    }
    else
        mm_log("ID capability not available on this IMAP server", ERROR);
    return ret;
}
...
IMAPPARSEDREPLY *imap_send(MAILSTREAM *stream, char *cmd, IMAPARG *args[])
{
    ...
    if (sc)            /* tell client sending a command */
        (*sc)(stream, cmd, ((compare_cstring(cmd, "FETCH") && compare_cstring(cmd, "STORE") && compare_cstring(cmd, "SEARCH")) ? NIL : SC_EXPUNGEDEFERRED));
        ...
        while ((arg = *arglst++) != NULL)
        {
            *s++ = ' '; /* delimit argument with space */
            ...
            case LITERAL: /* literal, as a stringstruct */
                if ((reply = imap_send_literal(stream, tag, &s, arg->text)) != NULL)
                    return reply;
                break;
            ...
            case SEQUENCE: /* sequence */
                ...
                else
                {
                    ...
                    do
                    { /* break up into multiple commands */
                        if (i <= MAXCOMMAND)
                        { /* final part? */
                            reply = imap_send(stream, cmd, args);
                        ...
                        { /* still needs to be split further */
                                   ...
                                   SIZEDTEXT *as, long wildok, char *limit)
{
    ...
    for (j = 0; j < as->size; j++)
        ...
        default: /* all other characters */
            ...
            { /* must not be 8bit */
                if (c <= ' ')
                    qflag = T; /* must quote if a CTL */
                break;
            }
        case '\0': /* not a CHAR */
        case '\012':
        case '\015': /* not a TEXT-CHAR */
        case '"':
    ...
    {                   /* only if prefetching permitted */
        ...
        *t = '\0';      /* initially nothing */
        ...
        { /* anything to pre-fetch? */
            /* pre-fetch envelopes for the first imap_prefetch number of messages */
            if (!imap_OK(stream, reply =
                                     imap_fetch(stream, t = cpystr(LOCAL->tmp), FT_NEEDENV + ((flags & SE_NOHDRS) ? FT_NOHDRS : NIL) + ((flags & SE_NEEDBODY) ? FT_NEEDBODY : NIL))))
                mm_log(reply->text, ERROR);
                                ...
                                char **s, SEARCHPGM *pgm, char *limit)
{
    ...
    for (t = "ALL"; *t; *(*s)++ = *t++)
        ;
    if (!pgm)
        return NIL;           /* done if NIL searchpgm */
    ...
    /* message flags */
    if (pgm->answered)
        for (t = " ANSWERED"; *t; *(*s)++ = *t++)
            ;
    if (pgm->unanswered)
        for (t = " UNANSWERED"; *t; *(*s)++ = *t++)
            ;
    if (pgm->deleted)
        for (t = " DELETED"; *t; *(*s)++ = *t++)
            ;
    if (pgm->undeleted)
        for (t = " UNDELETED"; *t; *(*s)++ = *t++)
            ;
    if (pgm->draft)
        for (t = " DRAFT"; *t; *(*s)++ = *t++)
            ;
    ...
    /* sent date ranges */
    if (pgm->sentbefore)
        imap_send_sdate(s, "SENTBEFORE", pgm->sentbefore);
    if (pgm->senton)
        imap_send_sdate(s, "SENTON", pgm->senton);
    if (pgm->sentsince)
        imap_send_sdate(s, "SENTSINCE", pgm->sentsince);
    /* internal date ranges */
    if (pgm->before)
        imap_send_sdate(s, "BEFORE", pgm->before);
    if (pgm->on)
        imap_send_sdate(s, "ON", pgm->on);
    if (pgm->since)
        imap_send_sdate(s, "SINCE", pgm->since);
    if (pgm->older)
    {
    ...
    if ((pgm->return_path &&
        ...
        (pgm->references &&
         (reply = imap_send_slist(stream, tag, base, s, " HEADER References ",
                                  pgm->references, limit))))
        return reply;

    /* all other headers */
    if ((hdr = pgm->header) != NULL)
        do
        {
    ...
    *s = imap_send_spgm_trim(base, *s, prefix);
    ...
    { /* insert "OR" in front of incomplete set */
        ...
        *s += 3; /* point to end of buffer */
        ...
        if ((reply = imap_send_literal(stream, tag, s, &st)) != NULL)
            return reply;
        *(*s)++ = ')'; /* close glue */
...
IMAPPARSEDREPLY *imap_sout(MAILSTREAM *stream, char *tag, char *base, char **s)
{
    ...
    *(*s)++ = '\012';
    **s = '\0';
    reply = net_sout(LOCAL->netstream, base, *s - base) ? imap_reply(stream, tag) : imap_fake(stream, tag, "[CLOSED] IMAP connection broken (command)");
...
IMAPPARSEDREPLY *imap_reply(MAILSTREAM *stream, char *tag)
{
    ...
    { /* parse reply from server */
        if ((reply = imap_parse_reply(stream, net_getline(LOCAL->netstream))) != NULL)
        {
            ...
            { /* tagged data */
                if (tag && !compare_cstring(tag, reply->tag))
                    return reply;
                /* report bogon */
...
long imap_OK(MAILSTREAM *stream, IMAPPARSEDREPLY *reply)
{
    ...
    { /* BAD - operation rejected */
        ...
        /* bad protocol received */
            ...
            sprintf(LOCAL->tmp, "Unexpected IMAP response: %.80s %.80s",
...
void imap_parse_unsolicited(MAILSTREAM *stream, IMAPPARSEDREPLY *reply)
{
    ...
    if (isdigit(*reply->key))
    {
                 ...
                 msgno && (msgno <= stream->nmsgs))
        {
            ...
            while ((prop = (strtok_r(t, " )", &r))) && (t = strtok_r(NIL, "\n", &r)))
            {
                ...
                if (!strcmp(ucase(prop), "FLAGS"))
                    imap_parse_flags(stream, elt, &t);
                    ...
                    else if (prop[4] == '[')
                    {
                        ...
                        { /* parse text from server */
                            ...
                            /* otherwise register it in the cache */
                                ...
                                imap_cache(stream, msgno, md.what, stl, &text);
                        }
                ...
                if (e && *e)
                    env = *e; /* note envelope if we got one */
            }
            if (prop)
            {
        ...
        do
            if (*s != '\\')
            {
                for (i = 0; (i < NUSERFLAGS) && stream->user_flags[i] &&
                            compare_cstring(s, stream->user_flags[i]);
                     i++)
                    ;
                if (i > NUSERFLAGS)
                {
    ...
    else if (!strcmp(reply->key, "SORT"))
    {
        ...
        {
            do
                if ((i = atol(t)) && (LOCAL->filter ? mail_elt(stream, i)->searched : T))
        ...
        /* also return via callback if requested */
        if (sr)
            (*sr)(stream, LOCAL->sortdata, LOCAL->sortsize);
    }
             ...
             (s = strchr(reply->text, ')')) && (s[1] == ' '))
    {
        ...
        *s++ = '\0'; /* tie off attribute list */
        ...
        if ((t = strtok_r(reply->text + 1, " ", &r)) != NULL)
            do
            {
                if (!compare_cstring(t, "\\NoInferiors"))
                    i |= LATT_NOINFERIORS;
                else if (!compare_cstring(t, "\\NoSelect"))
                    i |= LATT_NOSELECT;
                else if (!compare_cstring(t, "\\Marked"))
                    i |= LATT_MARKED;
                else if (!compare_cstring(t, "\\Unmarked"))
                    i |= LATT_UNMARKED;
                else if (!compare_cstring(t, "\\HasChildren"))
                    i |= LATT_HASCHILDREN;
                else if (!compare_cstring(t, "\\All"))
                    i |= LATT_ALL;
                else if (!compare_cstring(t, "\\Archive"))
                    i |= LATT_ARCHIVE;
                else if (!compare_cstring(t, "\\Drafts"))
                    i |= LATT_DRAFTS;
                else if (!compare_cstring(t, "\\Flagged"))
                    i |= LATT_FLAGGED;
                else if (!compare_cstring(t, "\\Junk"))
                    i |= LATT_JUNK;
                else if (!compare_cstring(t, "\\Sent"))
                    i |= LATT_SENT;
                else if (!compare_cstring(t, "\\Trash"))
                    i |= LATT_TRASH;
        ...
        if ((t = imap_parse_astring(stream, &s, reply, &j)) != NULL)
        {
            ...
            if (LOCAL->prefix && ((strlen(LOCAL->prefix) + j) < IMAPTMPLEN))
                ...
                s = t; /* otherwise just mailbox name */
                       /* pass data to main program */
            if (reply->key[1] == 'S')
                mm_lsub(stream, delimiter, s, i);
            else
                mm_list(stream, delimiter, s, i);
    ...
    else if (!strcmp(reply->key, "NAMESPACE"))
    {
        if (LOCAL->namespace)
        {
            ...
            LOCAL->namespace = (NAMESPACE **)fs_get(3 * sizeof(NAMESPACE *));
        if ((s = reply->text) != NULL)
             ...
             (t = imap_parse_astring(stream, &s, reply, NIL)))
    {
        ...
        if (s && (*s++ == ' '))
        {
            ...
            ACLLIST *ac = al;
            do
                if ((ac->identifier = imap_parse_astring(stream, &s, reply, NIL)) &&
                    s && (*s++ == ' '))
    ...
    {
        if (compare_cstring(s, "NIL"))
            LOCAL->id = imap_parse_idlist(s);
    }
...
void imap_parse_response(MAILSTREAM *stream, char *text, long errflg, long ntfy)
{
        ...
        ((i = t - s) < IMAPTMPLEN))
    {
        ...
        if ((s = strchr(strncpy(t = LOCAL->tmp, s, i), ' ')) != NULL)
            *s++ = '\0';
        if (s)
        {               /* have argument? */
            ntfy = NIL; /* suppress mm_notify if normal SELECT data */
            if (!compare_cstring(t, "CAPABILITY"))
                     ...
                     (t[i - 1] == ')'))
            {
                ...
                if ((s = strtok_r(s + 1, " ", &r)) != NULL)
                    do
                    {
                        if (*s == '\\')
                        { /* system flags */
                            if (!compare_cstring(s, "\\Seen"))
                                stream->perm_seen = T;
                            ...
                            else if (!compare_cstring(s, "\\Draft"))
                                stream->perm_draft = T;
                            else if (!strcmp(s, "\\*"))
                                stream->kwd_create = T;
                        }
            ...
            {             /* all other response code events */
        ...
        { /* no arguments */
            ...
            else if (!compare_cstring(t, "READ-ONLY"))
                stream->rdonly = T;
    ...
    if (ntfy && !stream->silent)
        mm_notify(stream, text ? text : "", errflg);
}
        ...
        case '(':
            ...
            while (**txtptr == '(')
            {
                ...
                nam = (NAMESPACE *)memset(fs_get(sizeof(NAMESPACE)), 0,
                                          sizeof(NAMESPACE));
                if (!ret)
                    ret = nam; /* if first time note first namespace */
                ...
                { /* append new parameter to tail */
                    if (nam->param)
                        ...
                        nam->param = par = mail_newbody_parameter();
    ...
    {              /* see a thread? */
        ...
        { /* parse thread */
            ...
            { /* thread branch */
                ...
                /* add to parent */
                if (parent)
                    parent = parent->next = cur;
                else
                { /* no parent, create dummy */
                    if (last)
                        last = last->branch = mail_newthreadnode(NIL);
                    /* new tree */
                    else
                        ret = last = mail_newthreadnode(NIL);
                    /* add to dummy parent */
                          ...
                          strtoul(*txtptr, (char **)txtptr, 10)))
            {
                ...
                /* no parent, start new thread */
                    ...
                    last = last->branch = parent = cur;
                /* create new tree */
                else
                    ret = last = parent = cur;
            }
            else
            { /* anything else is a bogon */
                char tmp[MAILTMPLEN];
                         ...
                         unsigned char **txtptr, IMAPPARSEDREPLY *reply)
{
    ...
    while (c == ' ')
        c = *++*txtptr;
    if (c)
        ++*txtptr; /* skip past first character */
    switch (c)
    ...
    case '(': /* if envelope S-expression */
        ...
        {              /* recursion dies on small stack machines */
            ...
            { /* end of group? */
                ...
                if (ingroup && !(adr->personal || adr->adl || adr->host))
                    --ingroup;
                else
                {
                    if (ingroup)
                    {                       /* in a group? */
                        ...
                        mm_notify(stream, "End of group encountered when not in group",
                                  WARN);
                    stream->unhealthy = T;
            ...
            { /* start of group? */
                if (adr->personal || adr->adl)
                {
                    ...
                    ++ingroup; /* in a group now */
            }
            if (adr)
            { /* good address */
                if (!ret)
                    ret = adr; /* if first time note first adr */
                               /* if previous link new block to it */
                if (prev)
                    prev->next = adr;
                      ...
                      unsigned char **txtptr)
{
    ...
    { /* parse list of flags */
        ...
        **txtptr = '\0'; /* tie off flag */
        ...
        {
            if (!compare_cstring(flag, "\\Seen"))
                elt->seen = T;
            else if (!compare_cstring(flag, "\\Deleted"))
                elt->deleted = T;
            else if (!compare_cstring(flag, "\\Flagged"))
                elt->flagged = T;
            else if (!compare_cstring(flag, "\\Answered"))
                elt->answered = T;
            else if (!compare_cstring(flag, "\\Recent"))
                elt->recent = T;
            else if (!compare_cstring(flag, "\\Draft"))
                elt->draft = T;
        }
        /* otherwise user flag */
        else
...
unsigned long imap_parse_user_flag(MAILSTREAM *stream, char *flag)
{
    ...
    for (i = 0; i < NUSERFLAGS; ++i)
        if (stream->user_flags[i])
            if (!compare_cstring(flag, stream->user_flags[i]))
                return (1 << i);
    ...
    case '"':  /* if quoted string */
        ...
        for (c = **txtptr; c != '"'; ++i, c = *++*txtptr)
        {
            ...
            { /* NUL not permitted either */
                ...
                stream->unhealthy = T;
                if (len)
                    *len = 0; /* punt, since may be at end of string */
                return NIL;
            }
        }
        ++*txtptr; /* bump past delimiter */
    ...
    case '{': /* if literal string */
        ...
        { /* must slurp into free storage */
            ...
            *string = '\0'; /* init in case getbuffer fails */
                            /* get the literal */
            if (rp)
                for (k = 0; (j = min((long)MAILTMPLEN, (long)i)) != 0L; i -= j)
                {
                ...
                STRINGLIST *stl, SIZEDTEXT *text)
{
    ...
    { /* nested, find non-contents specifier */
        for (t = seg; *t && !((*t == '.') && (isalpha(t[1]) || !atol(t + 1))); t++)
            ;
        if (*t)
            *t++ = '\0'; /* tie off section from data specifier */
    ...
    case '(': /* body structure list */
        ...
        {                               /* multipart body? */
            ...
            { /* disposition */
                ...
                if (LOCAL->cap.extlevel < BODYEXTDSP)
                    LOCAL->cap.extlevel = BODYEXTDSP;
            }
            if (**txtptr == ' ')
            { /* language */
            ...
            { /* location */
                ...
                if (LOCAL->cap.extlevel < BODYEXTLOC)
                    LOCAL->cap.extlevel = BODYEXTLOC;
            }
            while (**txtptr == ' ')
        ...
        { /* not multipart, parse type name */
            ...
            { /* extension data - md5 */
                ...
                if (LOCAL->cap.extlevel < BODYEXTMD5)
                    LOCAL->cap.extlevel = BODYEXTMD5;
            }
            if (**txtptr == ' ')
            { /* disposition */
                imap_parse_disposition(stream, body, txtptr, reply);
                                ...
                                IMAPPARSEDREPLY *reply)
{
    ...
    /* language is a list */
    if (*++*txtptr == '(')
        ret = imap_parse_stringlist(stream, txtptr, reply);
                          ...
                          IMAPPARSEDREPLY *reply)
{
    ...
    case '{':      /* get size of literal */
        ...
        if ((i = strtoul(*txtptr, (char **)txtptr, 10)) != 0L)
            do
...
void imap_parse_capabilities(MAILSTREAM *stream, char *t)
{
    ...
    for (t = strtok_r(t, " ", &r); t; t = strtok_r(NIL, " ", &r))
    {
        ...
        else if (!compare_cstring(t, "IMAP2"))
            LOCAL->cap.rfc1176 = T;
        else if (!compare_cstring(t, "IMAP2bis"))
            LOCAL->cap.imap2bis = LOCAL->cap.rfc1176 = T;
        else if (!compare_cstring(t, "ACL"))
            LOCAL->cap.acl = T;
        else if (!compare_cstring(t, "QUOTA"))
            LOCAL->cap.quota = T;
        else if (!compare_cstring(t, "LITERAL+"))
            LOCAL->cap.litplus = T;
        else if (!compare_cstring(t, "IDLE"))
            LOCAL->cap.idle = T;
        else if (!compare_cstring(t, "MAILBOX-REFERRALS"))
            LOCAL->cap.mbx_ref = T;
        else if (!compare_cstring(t, "LOGIN-REFERRALS"))
            LOCAL->cap.log_ref = T;
        else if (!compare_cstring(t, "NAMESPACE"))
            LOCAL->cap.namespace = T;
        else if (!compare_cstring(t, "UIDPLUS"))
            LOCAL->cap.uidplus = T;
        else if (!compare_cstring(t, "STARTTLS"))
            LOCAL->cap.starttls = T;
        else if (!compare_cstring(t, "LOGINDISABLED"))
            LOCAL->cap.logindisabled = T;
        else if (!compare_cstring(t, "ID"))
            LOCAL->cap.id = T;
        else if (!compare_cstring(t, "CHILDREN"))
            LOCAL->cap.children = T;
        else if (!compare_cstring(t, "MULTIAPPEND"))
            LOCAL->cap.multiappend = T;
        else if (!compare_cstring(t, "BINARY"))
            LOCAL->cap.binary = T;
        else if (!compare_cstring(t, "UNSELECT"))
            LOCAL->cap.unselect = T;
        else if (!compare_cstring(t, "SASL-IR"))
            LOCAL->cap.sasl_ir = T;
        else if (!compare_cstring(t, "SCAN"))
            LOCAL->cap.scan = T;
        else if (!compare_cstring(t, "URLAUTH"))
            LOCAL->cap.urlauth = T;
        else if (!compare_cstring(t, "CATENATE"))
            LOCAL->cap.catenate = T;
        else if (!compare_cstring(t, "CONDSTORE"))
            LOCAL->cap.condstore = T;
        else if (!compare_cstring(t, "ESEARCH"))
            LOCAL->cap.esearch = T;
        else if (!compare_cstring(t, "X-GM-EXT-1"))
            LOCAL->cap.x_gm_ext1 = T;
        else if (((t[0] == 'S') || (t[0] == 's')) &&
                 ((t[1] == 'O') || (t[1] == 'o')) &&
                 ((t[2] == 'R') || (t[2] == 'r')) &&
                 ((t[3] == 'T') || (t[3] == 't')))
            ...
            *s++ = '\0'; /* separate token from value */
            ...
            else if (!compare_cstring(t, "AUTH"))
            {
                if ((i = mail_lookup_auth_name(s, LOCAL->authflags)) &&
                    (--i < MAXAUTHENTICATORS))
                    LOCAL->cap.auth |= (1 << i);
                else if (!compare_cstring(s, "ANONYMOUS"))
                    LOCAL->cap.authanon = T;
            }
        }
...
IMAPPARSEDREPLY *imap_fetch(MAILSTREAM *stream, char *sequence, long flags)
{
    ...
    aarg.type = ATOM;
    aenv.type = ATOM;
    aenv.text = (void *)"ENVELOPE";
    ahhr.type = ATOM;
    ahhr.text = (void *)hdrheader[LOCAL->cap.extlevel];
    axtr.type = ATOM;
    axtr.text = (void *)imap_extrahdrs;
    ahtr.type = ATOM;
    ahtr.text = (void *)hdrtrailer;
    abdy.type = ATOM;
    abdy.text = (void *)"BODYSTRUCTURE";
    atrl.type = ATOM;
    atrl.text = (void *)"INTERNALDATE RFC822.SIZE FLAGS)";
    if (LEVELIMAP4(stream))
    ...
    /* easy if IMAP2 */
        ...
        aarg.text = (void *)(flags & FT_NEEDENV) ? ((flags & FT_NEEDBODY) ? "(RFC822.HEADER BODY INTERNALDATE RFC822.SIZE FLAGS)" : "(RFC822.HEADER INTERNALDATE RFC822.SIZE FLAGS)") : "FAST";
...
char *imap_reform_sequence(MAILSTREAM *stream, char *sequence, long flags)
{
    ...
    for (s = sequence; (t = strpbrk(s, ",:")) != NULL;)
        switch (*t++)
        {
        case ',':                      /* single message */
...
char *imap_host(MAILSTREAM *stream)
{
    ...
    return (LOCAL && LOCAL->netstream) ? net_host(LOCAL->netstream) : ".NO-IMAP-CONNECTION.";
}
