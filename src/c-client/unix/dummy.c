...
DRIVER *dummy_valid(char *name)
{
    ...
    if (rname && *rname && (*rname != '{') && (s = mailboxfile(tmp, rname)))
    {
        ...
        else if (!compare_cstring(rname, "INBOX"))
            return &dummydriver;
    }
    if (rname)
        fs_give((void **)&rname);
...
void dummy_scan(MAILSTREAM *stream, char *ref, char *pat, char *contents)
{
    ...
    else if (dummy_canonicalize(test, ref, pat))
    {
        ...
        if ((s = strpbrk(test, "%*")) != NULL)
        {
            ...
            strcpy(file, test); /* use just that name then */
...
void dummy_lsub(MAILSTREAM *stream, char *ref, char *pat)
{
    ...
    /* get canonical form of name */
    if (dummy_canonicalize(test, ref, pat) && (s = sm_read(tmpx, &sdb)))
        do
            if (*s != '{')
            {
                ...
                else if (pmatch_full(s, test, '/'))
                    mm_lsub(stream, '/', s, NIL);
                else
                    while (showuppers && (t = strrchr(s, '/')))
                    {
                     ...
                     long level)
{
    ...
    { /* do nothing if can't open directory */
        ...
        if (!dir || dir[(len = strlen(dir)) - 1] == '/')
            while ((d = readdir(dp)) != NULL)
                if ((!(dt && (*dt)(d->d_name))) &&
                    ((d->d_name[0] != '.') ||
                     (((long)mail_parameters(NIL, GET_HIDEDOTFILES, NIL)) ? NIL : (d->d_name[1] && (((d->d_name[1] != '.') || d->d_name[2]))))) &&
                        ...
                        strcpy(path + len - 1, d->d_name) && !stat(path, &sbuf))
                    {
                        ...
                        case S_IFREG: /* ordinary name */
                            ...
                            if (pmatch_full(tmp, pat, '/') && compare_cstring(tmp, "INBOX"))
                                dummy_listed(stream, '/', tmp, LATT_NOINFERIORS + ((sbuf.st_size && (sbuf.st_atime < sbuf.st_ctime)) ? LATT_MARKED : LATT_UNMARKED), contents);
                            break;
                        }
                    }
                }
...
{
    scancontents_t sc = dtb ? (scancontents_t)(*dtb->parameters)(GET_SCANCONTENTS, NIL) : NIL;
                  ...
                  long attributes, char *contents)
{
    ...
    { /* if not \NoInferiors */
        ...
        for (nochild = T; nochild && (dr = readdir(dp));)
            if ((!(dt && (*dt)(dr->d_name))) &&
                ((dr->d_name[0] != '.') ||
                 (((long)mail_parameters(NIL, GET_HIDEDOTFILES, NIL)) ? NIL : (dr->d_name[1] && ((dr->d_name[1] != '.') || dr->d_name[2])))))
                nochild = NIL;
...
long dummy_create_path(MAILSTREAM *stream, char *path, long dirmode)
{
    ...
    int mask = umask(0);
    if (wantdir)
        *t = '\0'; /* flush trailing delimiter for directory */
...
long dummy_delete(MAILSTREAM *stream, char *mailbox)
{
    struct stat sbuf;
    char *s, tmp[MAILTMPLEN];
    if (!strncmp(mailbox, "#md/", 4) || !strncmp(mailbox, "#mc/", 4) || is_valid_maildir(&mailbox))
    {
    ...
    if ((s = strrchr(tmp, '/')) && !s[1])
        *s = '\0';
    if (stat(tmp, &sbuf) || ((sbuf.st_mode & S_IFMT) == S_IFDIR) ? rmdir(tmp) : unlink(tmp))
    {
...
long dummy_rename(MAILSTREAM *stream, char *old, char *newname)
{
    ...
    if (!dummy_file(oldname, rold) || !(s = dummy_file(mbx, rnewname)) ||
        stat(oldname, &sbuf) || ((s = strrchr(s, '/')) && !s[1] && ((sbuf.st_mode & S_IFMT) != S_IFDIR)))
    {
...
long dummy_append(MAILSTREAM *stream, char *mailbox, append_t af, void *data)
{
    ...
    {                     /* found file? */
        ...
        if (sbuf.st_size)
            ts = NIL; /* non-empty file? */
    }
    if (ts)
        return (*ts->dtb->append)(stream, mailbox, af, data);
...
long dummy_canonicalize(char *tmp, char *ref, char *pat)
{
    ...
    { /* preliminary reference check */
        if (*ref == '{')
            return NIL; /* remote reference not allowed */
        else if (!*ref)
            ref = NIL; /* treat empty reference as no reference */
    }
    switch (*pat)
    {
    case '#': /* namespace name */
        if (mailboxfile(tmp, pat))
            strcpy(tmp, pat);
        else
    ...
    default: /* apply reference for all other names */
        if (!ref)
            strcpy(tmp, pat); /* just copy if no namespace */
        else if ((*ref != '#') || mailboxfile(tmp, ref))
        {
            /* wants root of name? */
            if (*pat == '/')
                strcpy(strchr(strcpy(tmp, ref), '/'), pat);
