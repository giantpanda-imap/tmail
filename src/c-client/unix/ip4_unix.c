...
char *ip_sockaddrtostring(struct sockaddr *sadr, char *buf)
{
    return (sadr->sa_family == PF_INET) ? inet_ntoa(SADR4ADR(sadr)) : "NON-IPv4";
}
                             ...
                             unsigned short port, size_t *len)
{
    ...
    switch (family)
    {             /* build socket address based upon family */
    case AF_INET: /* IPv4 */
                    ...
                    void **next, void **cleanup)
{
    ...
    { /* first lookup? */
            ...
            (he = gethostbyname(lcase(strcpy(tmp, name)))))
        {
            ...
            if (canonical)
                *canonical = cpystr((char *)he->h_name);
            if (next)
                *next = (void *)adl;
        }
        else
