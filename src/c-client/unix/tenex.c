...
int tenex_isvalid(char *name, char *tmp)
{
    ...
    else if ((errno == ENOENT) && !compare_cstring(name, "INBOX"))
        errno = -1;
...
void *tenex_parameters(long function, void *value)
{
    ...
    case GET_INBOXPATH:
        if (value)
            ret = tenex_file((char *)value, "INBOX");
        break;
    }
    return ret;
}
...
long tenex_text(MAILSTREAM *stream, unsigned long msgno, STRING *bs, long flags)
{
    ...
    { /* normal form, previous text cached? */
            ...
            while (r && (++i <= sysibx->nmsgs))
            {
                ...
                if ((j = hdrlen + txtlen) != 0L)
                {
                    ...
                    sprintf(LOCAL->buf + strlen(LOCAL->buf),
                            ",%lu;0000000000%02o\n", j, (unsigned)((fSEEN * elt->seen) + (fDELETED * elt->deleted) + (fFLAGGED * elt->flagged) + (fANSWERED * elt->answered) + (fDRAFT * elt->draft)));
                    /* copy message */
...
long tenex_append(MAILSTREAM *stream, char *mailbox, append_t af, void *data)
{
    ...
    /* get first message */
    if (!MM_APPEND(af)(stream, data, &flags, &date, &message))
        return NIL;
...
unsigned long tenex_size(MAILSTREAM *stream, unsigned long m)
{
    ...
    return ((m < stream->nmsgs) ? mail_elt(stream, m + 1)->private.special.offset : LOCAL->filesize) -
...
char *tenex_file(char *dst, char *name)
{
    ...
    return (s && !*s) ? mailboxfile(dst, tenex_isvalid("~/INBOX", tmp) ? "~/INBOX" : "mail.txt") : s;
}
...
long tenex_parse(MAILSTREAM *stream)
{
    ...
    { /* while there is stuff to parse */
        ...
        *t++ = '\0'; /* tie off fields */
        ...
        while (j)
            if (((i = 29 - find_rightmost_bit(&j)) < NUSERFLAGS) &&
                stream->user_flags[i])
                elt->user_flags |= 1 << i;
        /* calculate system flags */
        if ((j = ((t[10] - '0') * 8) + t[11] - '0') & fSEEN)
            elt->seen = T;
        if (j & fDELETED)
            elt->deleted = T;
        if (j & fFLAGGED)
...
void tenex_read_flags(MAILSTREAM *stream, MESSAGECACHE *elt)
{
    ...
    lseek(LOCAL->fd, (off_t)elt->private.special.offset + elt->private.special.text.size - 13, L_SET);
    /* read the new flags */
...
void tenex_update_status(MAILSTREAM *stream, unsigned long msgno, long syncflag)
{
    ...
    {                        /* readwrite */
        ...
        /* reverse bits (dontcha wish we had CIRC?) */
        while (j)
            k |= 1 << (29 - find_rightmost_bit(&j));
        /* print new flag string */
        sprintf(LOCAL->buf, "%010lo%02o", k, (unsigned)(fOLD + (fSEEN * elt->seen) + (fDELETED * elt->deleted) + (fFLAGGED * elt->flagged) + (fANSWERED * elt->answered) + (fDRAFT * elt->draft)));
