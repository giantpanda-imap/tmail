            ...
            while (r && (++i <= sysibx->nmsgs))
            {
                ...
                if ((j = hdrlen + txtlen) != 0L)
                {
                    ...
                    sprintf(LOCAL->buf + strlen(LOCAL->buf),
                            ",%lu;0000000000%02o\015\012", j, (unsigned)((fSEEN * elt->seen) + (fDELETED * elt->deleted) + (fFLAGGED * elt->flagged) + (fANSWERED * elt->answered) + (fDRAFT * elt->draft)));
                    /* copy message */
...
long mtx_append(MAILSTREAM *stream, char *mailbox, append_t af, void *data)
{
    ...
    { /* parse flags */
        ...
        { /* write message */
            ...
            /* get next message */
            if (i || !MM_APPEND(af)(stream, data, &flags, &date, &message))
                ret = NIL;
        }
    } while (ret && message);
    /* if error... */
...
char *mtx_file(char *dst, char *name)
{
    ...
    return (s && !*s) ? mailboxfile(dst, mtx_isvalid("~/INBOX", tmp) ? "~/INBOX" : "INBOX.MTX") : s;
}
...
void mtx_read_flags(MAILSTREAM *stream, MESSAGECACHE *elt)
{
    ...
    lseek(LOCAL->fd, (off_t)elt->private.special.offset + elt->private.special.text.size - 14, L_SET);
    /* read the new flags */
