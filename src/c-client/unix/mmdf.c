...
MAILSTREAM *mmdf_open(MAILSTREAM *stream)
{
    ...
    if (!stream)
        return user_flags(&mmdfproto);
    retry = stream->silent ? 1 : KODRETRY;
    if (stream->local)
        fatal("mmdf recycle stream");
    ...
    if (LOCAL && (LOCAL->ld >= 0) && !stream->lock)
    {
        ...
        { /* see if need to reparse */
            ...
            if ((LOCAL->ddirty || reparse) && mmdf_parse(stream, &lock, LOCK_EX))
            {
                ...
                if (LOCAL->ddirty)
                    mmdf_rewrite(stream, NIL, &lock, NIL);
                /* unlock mailbox */
                else
                    mmdf_unlock(LOCAL->fd, stream, &lock);
...
int mmdf_append_msgs(MAILSTREAM *stream, FILE *sf, FILE *df, SEARCHSET *set)
{
    ...
    while (fgets(tmp, MAILTMPLEN, sf))
    {
        ...
        for (c = '\n'; i && fgets(tmp, MAILTMPLEN, sf); c = tmp[j - 1])
        {
            ...
            if (i < (j = strlen(tmp)))
                fatal("mmdf_append_msgs overrun");
            i -= j; /* number of bytes left */
                    /* squish out ^A and CRs (note copies NUL) */
            for (x = tmp; (x = strpbrk(x, "\01\r")) != NULL; --j)
...
int mmdf_parse(MAILSTREAM *stream, DOTLOCK *lock, int op)
{
    ...
    if ((LOCAL->fd = mmdf_lock(stream->mailbox, (LOCAL->ld >= 0) ? O_RDWR : O_RDONLY,
                               (long)mail_parameters(NIL, GET_MBXPROTECTION, NIL),
                               lock, op)) < 0)
    {
        ...
        { /* read new data */
            ...
            { /* read MMDF header */
                ...
                {                    /* look for message body */
                    ...
                    if (t)
                        s = t = mmdf_mbxline(stream, &bs, &i);
                    else
                        t = s; /* this line read was suppressed */
                    if (ISMMDF(s))
                    {                   /* found terminator in header? */
            ...
            {                     /* preserve this message */
                      ...
                      mmdf_xstatus(stream, LOCAL->buf, elt, NIL, flag))))
                {
                    ...
                    if (f.curpos != f.protect)
                    {
                        ...
                        f.protect = (i <= stream->nmsgs) ? mail_elt(stream, i)->private.special.offset : (f.curpos + j + MMDFHDRLEN);
