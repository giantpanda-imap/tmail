...
MAILSTREAM *phile_open(MAILSTREAM *stream)
{
    ...
    if ((pw = getpwuid(sbuf.st_uid)) != NULL)
        ...
        sprintf(tmp, "User-Number-%ld", (long)sbuf.st_uid);
    ...
    if ((i = phile_type(buf->data, buf->size, &j)) != 0)
    {
        ...
        LOCAL->body->parameter->value =
            cpystr((i & PTYPEISO2022JP) ? "ISO-2022-JP" : (i & PTYPEISO2022KR) ? "ISO-2022-KR" : (i & PTYPEISO2022CN) ? "ISO-2022-CN" : (i & PTYPE8) ? "X-UNKNOWN" : "US-ASCII");
...
long phile_append(MAILSTREAM *stream, char *mailbox, append_t af, void *data)
{
    ...
    if (s && *s)
        ...
        sprintf(tmp, "Can't append - invalid name: %.80s", mailbox);
    mm_log(tmp, ERROR);
    return NIL;
}
