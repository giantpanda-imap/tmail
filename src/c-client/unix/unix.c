 ...
 * Last Edited:	June 13 2018 (Chappa)
 */
...
DRIVER *unix_valid(char *name)
{
    ...
    if ((t = dummy_file(file, name)) && !stat(t, &sbuf))
    {
        ...
        else if ((fd = open(file, O_RDONLY, NIL)) >= 0)
        {
            ...
            if (unix_isvalid_fd(fd))
                ret = &unixdriver;
            else
                errno = -1; /* invalid format */
...
void *unix_parameters(long function, void *value)
{
    ...
    case GET_INBOXPATH:
        if (value)
            ret = dummy_file((char *)value, "INBOX");
        break;
    case SET_FROMWIDGET:
...
{
    if (stream)
        dummy_scan(NIL, ref, pat, contents);
}
...
long unix_create(MAILSTREAM *stream, char *mailbox)
{
    ...
    else if (dummy_create_path(stream, s, get_dir_protection(mailbox)))
    {
        ...
        if (((s = strrchr(s, '/')) && !s[1]) ||
            mail_parameters(NIL, GET_USERHASNOLIFE, NIL))
            ret = T;
...
MAILSTREAM *unix_open(MAILSTREAM *stream)
{
    ...
    if (!stream)
        return user_flags(&unixproto);
    retry = stream->silent ? 1 : KODRETRY;
    if (stream->local)
        fatal("unix recycle stream");
    ...
    if (!stream->rdonly)
        while (retry)
        {
            ...
            {                   /* got the lock, nobody else can alter state */
                ...
                if (stream->silent)
                    i = 0; /* silent streams won't accept KOD */
                else
                  ...
                  unsigned long *length, long flags)
{
    ...
    *length = 0; /* default to empty */
    ...
    lseek(LOCAL->fd, elt->private.special.offset + elt->private.msg.header.offset, L_SET);
                     ...
                     unsigned long *length, long flags)
{
    ...
    /* go to text position */
    lseek(LOCAL->fd, elt->private.special.offset + elt->private.msg.text.offset, L_SET);
    if (flags & FT_INTERNAL)
    { /* initial data OK? */
    ...
    if (elt->private.uid != LOCAL->uid)
    {
        ...
        for (s = (char *)LOCAL->text.data; SIZE(&bs);)
            switch (c = SNX(&bs))
            {
...
void unix_flagmsg(MAILSTREAM *stream, MESSAGECACHE *elt)
{
    ...
    if (elt->valid)
        elt->private.dirty = LOCAL->dirty = T;
}
    ...
    if (LOCAL && (LOCAL->ld >= 0) && !stream->lock)
    {
        ...
        { /* see if need to reparse */
            ...
            if ((LOCAL->ddirty || reparse) && unix_parse(stream, &lock, LOCK_EX))
            {
                ...
                if (LOCAL->ddirty)
                    unix_rewrite(stream, NIL, &lock, NIL);
                /* unlock mailbox */
                else
                    unix_unlock(LOCAL->fd, stream, &lock);
...
long unix_expunge(MAILSTREAM *stream, char *sequence, long options)
{
    ...
    char *msg = NIL;
    /* parse and lock mailbox */
    if ((ret = (sequence ? ((options & EX_UID) ? mail_uid_sequence(stream, sequence) : mail_sequence(stream, sequence)) : LONGT) != 0L) &&
        ...
        unix_parse(stream, &lock, LOCK_EX))
    {
        ...
        for (i = 1; !LOCAL->dirty && (i <= stream->nmsgs); i++)
        {
            ...
            if (mail_elt(stream, i)->deleted)
                LOCAL->dirty = T;
        }
        if (!LOCAL->dirty)
        { /* not dirty and no expunged messages */
            unix_unlock(LOCAL->fd, stream, &lock);
        ...
        {
            if (i)
                sprintf(msg = LOCAL->buf, "Expunged %lu messages", i);
            else
                msg = "Mailbox checkpointed, but no messages expunged";
        }
        /* rewrite failed */
        else
            unix_unlock(LOCAL->fd, stream, &lock);
...
long unix_copy(MAILSTREAM *stream, char *sequence, char *mailbox, long options)
{
    ...
    copyuid_t cu = (copyuid_t)(mail_parameters(NIL, GET_USERHASNOLIFE, NIL) ? NIL : mail_parameters(NIL, GET_COPYUID, NIL));
    ...
    for (d = (DRIVER *)mail_parameters(NIL, GET_DRIVERS, NIL);
         ...
         d = d->next)
        ; /* see if mbox driver active */
    if (!((options & CP_UID) ? mail_uid_sequence(stream, sequence) : mail_sequence(stream, sequence)))
        return NIL;
    ...
    for (i = 1; ret && (i <= stream->nmsgs); i++)
        if ((elt = mail_elt(stream, i))->sequence)
        {
            ...
            if (write(fd, LOCAL->buf, elt->private.special.text.size) < 0)
                ret = NIL;
            else
            { /* internal header succeeded */
    ...
    { /* flush any sets we may have built */
        mail_free_searchset(&source);
        mail_free_searchset(&dest);
    }
    tp[1] = time(0); /* set mtime to now */
    if (ret)
        tp[0] = tp[1] - 1; /* set atime to now-1 if successful copy */
    else
        tp[0] = /* else preserve \Marked status */
            ((sbuf.st_ctime > sbuf.st_atime) || (sbuf.st_mtime > sbuf.st_atime)) ? sbuf.st_atime : tp[1];
    utime(file, tp);             /* set the times */
...
long unix_append(MAILSTREAM *stream, char *mailbox, append_t af, void *data)
{
    ...
    appenduid_t au = (appenduid_t)(mail_parameters(NIL, GET_USERHASNOLIFE, NIL) ? NIL : mail_parameters(NIL, GET_APPENDUID, NIL));
    ...
    /* get first message */
    if (!MM_APPEND(af)(tstream, data, &flags, &date, &message))
        return NIL;
    if (!(sf = tmpfile()))
    { /* must have scratch file */
    ...
    { /* parse date */
        ...
        { /* user wants to suppress time zones? */
            ...
            /* get next message */
            else if (MM_APPEND(af)(tstream, data, &flags, &date, &message))
                continue;
        }
                     ...
                     STRING *msg)
{
    ...
    if (fprintf(sf, "%ld %lu %s", f, SIZE(msg) + 1, date) < 0)
        return NIL;
...
int unix_append_msgs(MAILSTREAM *stream, FILE *sf, FILE *df, SEARCHSET *set)
{
    ...
    while (fgets(tmp, MAILTMPLEN, sf))
    {
        ...
        for (c = '\n'; i && fgets(tmp, MAILTMPLEN, sf); c = tmp[j - 1])
        {
            ...
            if (i < (j = strlen(tmp)))
                fatal("unix_append_msgs overrun");
            i -= j; /* number of bytes left */
                    /* squish out CRs (note also copies NUL) */
            for (x = tmp; (x = strchr(x, '\r')) != NULL; --j)
                memmove(x, x + 1, j - (x - tmp));
            if (!j)
                continue; /* do nothing if line emptied */
                          /* start of line? */
            if (c == '\n')
                switch (tmp[0])
                {
                case 'F': /* possible "From " (case counts here) */
                        ...
                        (tmp[3] == 'm') && (tmp[4] == ' '))
                    {
                        if (!unix_fromwidget)
                        {
                            ...
                            if (!ti)
                                break; /*  it looks like a valid header */
                        }              /* write the widget */
                        if (putc('>', df) == EOF)
                            return NIL;
                    }
                    break;
                case 'S':
                case 's': /* possible "Status:" */
                    if (hdrp && (j > 6) && ((tmp[1] == 't') || (tmp[1] == 'T')) &&
                        ((tmp[2] == 'a') || (tmp[2] == 'A')) &&
                        ((tmp[3] == 't') || (tmp[3] == 'T')) &&
                        ((tmp[4] == 'u') || (tmp[4] == 'U')) &&
                        ((tmp[5] == 's') || (tmp[5] == 'S')) && (tmp[6] == ':') &&
                        (fputs("X-Original-", df) == EOF))
                        return NIL;
                    break;
                case 'X':
                case 'x': /* possible X-??? header */
                    if (hdrp && (tmp[1] == '-') &&
                        /* possible X-UID: */
                        (((j > 5) && ((tmp[2] == 'U') || (tmp[2] == 'u')) &&
                          ((tmp[3] == 'I') || (tmp[3] == 'i')) &&
            ...
            /* just write the line */
            if (fwrite(tmp, 1, j, df) != j)
                return NIL;
        }
        if (i)
            return NIL; /* didn't read entire message */
        /* update set */
        if (stream)
...
int unix_lock(char *file, int flags, int mode, DOTLOCK *lock, int op)
{
    ...
    if (dotlock_lock(file, lock, -1))
    {
        ...
        if ((fd = open(file, flags, mode)) >= 0)
            ...
            dotlock_unlock(lock); /* open failed, free the dotlock */
    }
    ...
    else if ((fd = open(file, flags, mode)) >= 0)
    {
        ...
        if (dotlock_lock(file, lock, fd))
        {
            ...
            flock(fd, op); /* paranoid way failed, just flock() it */
    }
    (*bn)(BLOCK_NONE, NIL);
    return fd;
}
...
void unix_unlock(int fd, MAILSTREAM *stream, DOTLOCK *lock)
{
    ...
    { /* need to muck with times? */
        ...
        if (now && !utime(stream->mailbox, tp))
            LOCAL->filetime = tp[1];
    }
    flock(fd, LOCK_UN); /* release flock'ers */
    if (!stream)
        close(fd);        /* close the file if no stream */
...
int unix_parse(MAILSTREAM *stream, DOTLOCK *lock, int op)
{
    ...
    if ((LOCAL->fd = unix_lock(stream->mailbox, (LOCAL->ld >= 0) ? O_RDWR : O_RDONLY,
                               (long)mail_parameters(NIL, GET_MBXPROTECTION, NIL),
                               lock, op)) < 0)
    {
    ...
    else if ((i = sbuf.st_size - LOCAL->filesize) != 0L)
    {
        ...
        { /* read new data */
            ...
            t = NIL, zn = 0;
            if (i)
                VALID(s, t, ti, zn); /* see if valid From line */
            if (!ti)
            { /* someone pulled the rug from under us */
                ...
                { /* look for message body */
                    ...
                    if (i)
                        ...
                        case 'X': /* possible X-???: line */
                            if (s[1] == '-')
                            { /* must be immediately followed by hyphen */
                                ...
                                /* possible X-IMAP */
                                         ...
                                         (s[5] == 'P') && ((m = (s[6] == ':')) || ((s[6] == 'b') && (s[7] == 'a') && (s[8] == 's') && (s[9] == 'e') && (s[10] == ':'))))
                                {
                                    retain = NIL; /* don't retain continuation */
                        ...
                        {                        /* got new data? */
                            ...
                            { /* not a header line, add it to message */
                                ...
                                if ((i == 1) && (*s == '\n'))
                                {
                                    ...
                                    k = m = 0; /* file does not end with newline! */
            ...
            if (pseudoseen)
            { /* flush pseudo-message if present */
                /* decrement recent count */
                if (mail_elt(stream, 1)->recent)
                    recent--;
...
char *unix_mbxline(MAILSTREAM *stream, STRING *bs, unsigned long *size)
{
    ...
    { /* find newline */
        ...
        *size = 0; /* end of data, return empty */
    return ret;
}
                           ...
                           unsigned long uid, long flag)
{
    ...
    { /* need to write X-IMAPbase: header? */
        ...
        *s++ = ' ';
        ...
        while (t > stack)
            *s++ = *--t;
        for (n = 0; n < NUSERFLAGS; ++n)
            if ((t = stream->user_flags[n]) != NULL)
                for (*s++ = ' '; *t; *s++ = *t++)
                    ;
        *s++ = '\n';
    ...
    *s++ = ' ';
    ...
    if (flag && (!elt->recent || !LOCAL->appending))
        *s++ = 'O';
    *s++ = '\n';
    *s++ = 'X';
    *s++ = '-';
                  ...
                  long flags)
{
    ...
    if (nexp)
        *nexp = 0; /* initially nothing expunged */
     ...
     */
        ...
        f.protect = stream->nmsgs ? /* initial protection pointer */
                        mail_elt(stream, 1)->private.special.offset
                                  : 8192;
            ...
            {                     /* preserve this message */
                      ...
                      unix_xstatus(stream, LOCAL->buf, elt, NIL, flag))))
                {
                    ...
                    if (f.curpos != f.protect)
                    {
                        ...
                        f.protect = (i <= stream->nmsgs) ? mail_elt(stream, i)->private.special.offset : (f.curpos + j + 1);
                        unix_write(&f, s, j); /* write text */
                                              /* write trailing newline */
                        unix_write(&f, "\n", 1);
                    }
                    else
                    { /* tie off header and status */
                        unix_write(&f, NIL, NIL);
                        /* protection pointer moves to next message */
                        f.protect = (i <= stream->nmsgs) ? mail_elt(stream, i)->private.special.offset : size;
                        /* locate end of message text */
                        j = f.filepos + elt->private.msg.text.text.size;
                        /* trailing newline already there? */
                        if (f.protect == (j + 1))
                            f.curpos = f.filepos = f.protect;
                        else
                        { /* trailing newline missing, write it */
                ...
                { /* no need to rewrite this message */
...
long unix_extend(MAILSTREAM *stream, unsigned long size)
{
    ...
    { /* does the mailbox need to grow? */
        ...
        while (T)
        { /* until write successful or punt */
            lseek(LOCAL->fd, LOCAL->filesize, L_SET);
            if ((write(LOCAL->fd, LOCAL->buf, i) >= 0) && !fsync(LOCAL->fd))
                break;
            else
            {
...
void unix_write(UNIXFILE *f, char *buf, unsigned long size)
{
    ...
    {                           /* doing buffered write? */
        ...
        if ((j = i ? ((f->buflen - i) % OVERFLOWBUFLEN) : f->buflen) != 0L)
        {
            ...
            if (j -= k)
                return; /* all done if still have buffer free space */
     ...
     */
        ...
        if ((j = min(i, f->protect - f->filepos)) != 0L)
        {
            ...
            if ((k = f->filepos % OVERFLOWBUFLEN) && ((k = OVERFLOWBUFLEN - k) < j))
                ...
                k = 0; /* no partial chunk to write */
                       /* if at least a chunk free, write that too */
            if (j > OVERFLOWBUFLEN)
                k += j - (j % OVERFLOWBUFLEN);
            if (k)
            { /* write data if there is anything we can */
                unix_phys_write(f, f->buf, k);
                /* slide buffer */
                if (i -= k)
                    memmove(f->buf, f->buf + k, i);
     ...
     */
        if (size)
        { /* have more data that we need to buffer? */
            ...
            if (size)
            { /* still have data that we need to buffer? */
