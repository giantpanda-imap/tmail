...
void *env_parameters(long function, void *value)
{
    ...
    case GET_ANONYMOUSHOME:
        if (!anonymousHome)
            anonymousHome = cpystr(ANONYMOUSHOME);
                 ...
                 void *staint)
{
    ...
    { /* set up network and maybe SSL */
        ...
        if (((port = tcp_serverport()) >= 0))
        {
            ...
            { /* not service or SSL service port */
...
long server_login(char *user, char *pwd, char *authuser, int argc, char *argv[])
{
        ...
        (authuser && (strlen(authuser) >= NETMAXUSER)))
    {
        ...
        err = "excessive login failures";
    else if (disablePlaintext)
        err = "disabled";
    else if (!(authuser && *authuser))
        pw = valpwd(user, pwd, argc, argv);
    else if (valpwd(authuser, pwd, argc, argv))
        pw = pwuser(user);
              ...
              char *argv[])
{
    ...
    { /* must have passwd struct for non-UID 0 */
        ...
        if (user && auser && *auser && compare_cstring(auser, user))
        {
            ...
            if ((gr = getgrnam(ADMINGROUP)) && (t = gr->gr_mem))
                while (*t && !ret)
        ...
        { /* paranoid site, lock out other directories */
            ...
            else if (loginpw(pw, argc, argv))
                ret = env_init(user, NIL);
            else
                fatal("Login failed after chroot");
        }
        /* normal login */
        else if (((pw->pw_uid == geteuid()) || loginpw(pw, argc, argv)) &&
                 (ret = env_init(user, home)))
            chdir(myhomedir());
...
long env_init(char *user, char *home)
{
    ...
    if (!myLocalHost)
        mylocalhost();
    if (!myNewsrc)
        myNewsrc = cpystr(strcat(strcpy(tmp, myHomeDir), "/.newsrc"));
    if (!newsActive)
        newsActive = cpystr(ACTIVEFILE);
    if (!newsSpool)
        newsSpool = cpystr(NEWSSPOOL);
...
char *myusername_full(unsigned long *flags)
{
    ...
    { /* logged in? */
        if (flags)
            *flags = anonymous ? MU_ANONYMOUS : MU_LOGGEDIN;
        ret = myUserName; /* return user name */
    }
    else if (flags)
        *flags = MU_NOTLOGGEDIN;
    return ret;
}
    ...
    if (!name || !*name || (*name == '{') || (strlen(name) > NETMAXMBX) ||
        ...
        case '#': /* namespace name */
            ...
            if (((name[1] == 'f') || (name[1] == 'F')) &&
                ...
                ((name[3] == 'p') || (name[3] == 'P')) &&
                (name[4] == '/') && ftpHome)
                sprintf(dst, "%s/%s", ftpHome, name + 5);
            /* #public/ and #shared/ namespaces */
                     ...
                     (!anonymous && ((name[1] == 's') || (name[1] == 'S')) &&
                      ...
                      ((name[6] == 'd') || (name[6] == 'D')) &&
                      (name[7] == '/') && (s = sharedHome)))
                sprintf(dst, "%s/%s", s, compare_cstring(name + 8, "INBOX") ? name + 8 : "INBOX");
            else
                dst = NIL; /* unknown namespace */
            break;

        case '/': /* root access */
            if (anonymous)
                dst = NIL; /* anonymous forbidden to do this */
            else if (blackBox)
            { /* other user access if blackbox */
                if (restrictBox & RESTRICTOTHERUSER)
                    dst = NIL;
                ...
                else if ((s = strchr(name + 1, '/')) && !compare_cstring(s + 1, "INBOX"))
                {
                    ...
                    *s = '/'; /* in case caller cares */
                    ...
                    sprintf(dst, "%s/%s", blackBoxDir, name + 1);
            }
            else
            {
                ...
                if ((restrictBox & RESTRICTROOT) && strcmp(name, sysinbox()) &&
                    ...
                    strcpy(dst, name); /* unrestricted, copy root name */
            }
            break;
        case '~': /* other user access */
            ...
            /* ~/ equivalent to ordinary name */
                ...
                sprintf(dst, "%s/%s", mymailboxdir(), name + 1);
            /* other user forbidden if closed/restricted */
            else if (closedBox || (restrictBox & RESTRICTOTHERUSER))
                dst = NIL;
            else if (blackBox)
            { /* black box form of other user */
                ...
                if ((s = strchr(name, '/')) && compare_cstring(s + 1, "INBOX"))
                {
                    ...
                    *s = '/'; /* in case caller cares */
                    ...
                    sprintf(dst, "%s/%s", blackBoxDir, name);
            }
            else
            { /* clear box other user */
                /* copy user name */
        ...
        case 'i': /* possible INBOX */
            if (!compare_cstring(name + 1, "NBOX"))
            {
                ...
                if (anonymous || blackBox || closedBox)
                    ...
                    *dst = '\0'; /* otherwise driver selects the name */
                break;
            }
...
long dotlock_lock(char *file, DOTLOCK *base, int fd)
{
    ...
    { /* make sure not symlink */
        if (!(j = chk_notsymlink(base->lock, &sb)))
            return NIL;
    ...
    if (retry < 0)
    { /* still returning retry after locktimeout? */
        if (!(j = chk_notsymlink(base->lock, &sb)))
            return NIL;
        if ((j > 0) && ((time(0)) < (sb.st_ctime + locktimeout * 60)))
        {
        ...
        case EACCES:          /* protection failure? */
            ...
            { /* make command pipes */
                ...
                if ((pi[0] < FD_SETSIZE) && (pi[1] < FD_SETSIZE) && (pipe(po) >= 0))
                {
                    ...
                    {
                        if (!fork())
                        { /* make grandchild so it's inherited by init */
            ...
            if ((s = strrchr(base->lock, '/')) != NULL)
            {
                ...
                *s = '/'; /* restore lock name */
                if (mask != 1777)
                { /* default warning if not 1777 */
                    if (!disableLockWarning)
                        MM_LOG(tmp, WARN);
                    break;
                }
            }
...
long dotlock_unlock(DOTLOCK *base)
{
    ...
    {
        if (base->pipei >= 0)
        { /* if running through a pipe unlocker */
...
int lock_work(char *lock, void *sb, int op, long *pid)
{
    ...
    { /* until get a good lock */
        do
            switch ((int)chk_notsymlink(lock, &lsb))
            {
            case 1: /* exists just once */
                if (((fd = open(lock, O_RDWR, shlock_mode)) >= 0) ||
                    (errno != ENOENT) || (chk_notsymlink(lock, &lsb) >= 0))
                    break;
        ...
        if (!lstat(lock, &lsb) && ((lsb.st_mode & S_IFMT) != S_IFLNK) &&
            ...
            (lsb.st_ino == fsb.st_ino) && (fsb.st_nlink == 1))
            break;
...
long set_mbx_protections(char *mailbox, char *path)
{
    ...
    if (!stat(path, &sbuf) && ((sbuf.st_mode & S_IFMT) == S_IFDIR))
    {
        ...
        if (mode & 0600)
            mode |= 0100;
        if (mode & 060)
            mode |= 010; /* set group search if allow read or write */
        if (mode & 06)
            mode |= 01; /* set world search if allow read or write */
...
MAILSTREAM *user_flags(MAILSTREAM *stream)
{
    ...
    for (i = 0; i < NUSERFLAGS && userFlags[i]; ++i)
        if (!stream->user_flags[i])
            stream->user_flags[i] = cpystr(userFlags[i]);
    return stream;
}
                ...
                *k++ = '\0'; /* tie off two words */
                if (!compare_cstring(s, "set keywords") && !userFlags[0])
                {
                    ...
                    for (i = 0; k && i < NUSERFLAGS; ++i)
                        if (strlen(k) <= MAXUSERFLAG)
                        {
                            if (userFlags[i])
                                fs_give((void **)&userFlags[i]);
                ...
                { /* none of these valid in .mminit */
                    ...
                    { /* only valid if logged in */
                            ...
                            !compare_cstring(s, "set new-folder-format"))
                        {
                                ...
                                if ((d = mail_valid(NIL, "INBOX", NIL)) != NULL)
                                {
                                    if (!compare_cstring(d->name, "mbox"))
                                        d = (DRIVER *)mail_parameters(NIL, GET_DRIVER,
                                                                      (void *)"unix");
                                    else if (!compare_cstring(d->name, "dummy"))
                                        d = NIL;
                                }
                            ...
                            { /* canonicalize mbox to unix */
                                if (!compare_cstring(k, "mbox"))
                                    k = "unix";
                    ...
                    else if (!compare_cstring(s, "set mail-subdirectory"))
                    {
                        ...
                        mail_parameters(NIL, SET_FROMWIDGET,
                                        compare_cstring(k, "header-only") ? VOIDT : NIL);
				 ...
				 */
                            ...
                            {
                                if (!compare_cstring(k, "root"))
                                    restrictBox |= RESTRICTROOT;
                                else if (!compare_cstring(k, "otherusers"))
                                    restrictBox |= RESTRICTOTHERUSER;
                                else if (!compare_cstring(k, "all"))
                                    restrictBox = -1;
                            }
                        ...
                        else if (!compare_cstring(s, "set disable-automatic-shared-namespaces"))
