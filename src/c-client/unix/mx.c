...
int mx_isvalid(char *name, char *tmp)
{
    ...
    else if (!compare_cstring(name, "INBOX"))
        errno = NIL;
    return NIL;
}
...
int mx_namevalid(char *name)
{
    ...
    { /* make sure valid name */
        if (isdigit(*s))
            s++; /* digit, check this node further... */
        else if (*s == '/')
...
MAILSTREAM *mx_open(MAILSTREAM *stream)
{
    ...
    if (!stream)
        return user_flags(&mxproto);
    if (stream->local)
        fatal("mx recycle stream");
                ...
                long flags)
{
    ...
    *length = 0; /* default to empty */
    ...
    if (!elt->private.msg.header.text.data)
    {
        ...
        /* find end of header */
        if (elt->rfc822_size < 4)
            i = 0;
        else
            for (i = 4; (i < elt->rfc822_size) &&
                        !((LOCAL->buf[i - 4] == '\015') &&
                          (LOCAL->buf[i - 3] == '\012') &&
                          (LOCAL->buf[i - 2] == '\015') &&
                          (LOCAL->buf[i - 1] == '\012'));
            ...
            !sysibx->rdonly && (r = sysibx->nmsgs))
        {
            ...
            { /* for each message in sysinbox mailbox */
                    ...
                    (write(fd, s, j) == j) && !fsync(fd) && !close(fd))
                {
                    ...
                    elt->day = selt->day;
                    elt->month = selt->month;
                    elt->year = selt->year;
                    elt->hours = selt->hours;
                    elt->minutes = selt->minutes;
                    elt->seconds = selt->seconds;
                    elt->zhours = selt->zhours;
                    elt->zminutes = selt->zminutes;
    ...
    {                        /* lock the index */
        ...
        {                              /* for each message */
            ...
            if (elt->deleted && (sequence ? elt->sequence : T))
            {
                ...
                /* note uncached */
                LOCAL->cachedtexts -= ((elt->private.msg.header.text.data ? elt->private.msg.header.text.size : 0) +
                                       (elt->private.msg.text.text.data ? elt->private.msg.text.text.size : 0));
...
long mx_copy(MAILSTREAM *stream, char *sequence, char *mailbox, long options)
{
    ...
    /* copy the messages */
    if (!(ret = ((options & CP_UID) ? mail_uid_sequence(stream, sequence) : mail_sequence(stream, sequence))))
        ;
    ...
    else
    {
        ...
        else
        {
            ...
            for (i = 1, uid = uidv = 0; ret && (i <= stream->nmsgs); i++)
                if ((elt = mail_elt(stream, i))->sequence)
                {
                    if ((ret = ((fd = open(mx_fast_work(stream, elt), O_RDONLY, NIL)) >= 0)) != 0L)
                    {
    ...
    else
    {
        ...
        do
        {
            ...
            else if (date && !(ret = mail_parse_date(&elt, date)))
            {
                ...
                ret = mx_append_msg(astream, flags, date ? &elt : NIL, message, dst) &&
                   ...
                   STRING *st, SEARCHSET *set)
{
    ...
    close(fd); /* close the file */
    if (elt)
        mx_setdate(tmp, elt); /* set file date */
    /* swell the cache */
    mail_exists(stream, ++stream->nmsgs);
    /* copy flags */
...
void mx_unlockindex(MAILSTREAM *stream)
{
    ...
    if (LOCAL->fd >= 0)
    {
        ...
        for (i = 1; i <= stream->nmsgs; i++)
        {
            ...
            sprintf(s, "M%08lx;%08lx.%04x", elt->private.uid, elt->user_flags, (unsigned)((fSEEN * elt->seen) + (fDELETED * elt->deleted) + (fFLAGGED * elt->flagged) + (fANSWERED * elt->answered) + (fDRAFT * elt->draft)));
        }
