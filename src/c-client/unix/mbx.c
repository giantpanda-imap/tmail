                ...
                long flags)
{
        ...
        ((fd = open(tmp, (flags ? O_RDWR : O_RDONLY) | O_BINARY, NIL)) >= 0))
    {
        ...
        if (ret != fd)
            ...
            lseek(fd, 0, L_SET); /* else rewind to start */
        /* \Marked status? */
...
void *mbx_parameters(long function, void *value)
{
    ...
    case GET_INBOXPATH:
        if (value)
            ret = mbx_file((char *)value, "INBOX");
        break;
    case SET_ONETIMEEXPUNGEATPING:
        if (value)
            ((MBXLOCAL *)((MAILSTREAM *)value)->local)->expok = T;
    case GET_ONETIMEEXPUNGEATPING:
        if (value)
            ret = (void *)(((MBXLOCAL *)((MAILSTREAM *)value)->local)->expok ? VOIDT : NIL);
        break;
    }
    return ret;
}
...
long mbx_create(MAILSTREAM *stream, char *mailbox)
{
    ...
    else if (dummy_create_path(stream, s, get_dir_protection(mailbox)))
    {
        ...
        else
        {
            ...
            {
                t = (stream && stream->user_flags[i]) ? stream->user_flags[i] : ((t = default_user_flag(i)) ? t : "");
...
long mbx_rename(MAILSTREAM *stream, char *old, char *newname)
{
    ...
    if (ret && !compare_cstring(old, "INBOX"))
...
MAILSTREAM *mbx_open(MAILSTREAM *stream)
{
    ...
    stream->kwd_create = (stream->user_flags[NUSERFLAGS - 1] || stream->rdonly) ? NIL : T; /* can we create new user flags? */
...
void mbx_close(MAILSTREAM *stream, long options)
{
    ...
    { /* only if a file is open */
        ...
        if (options & CL_EXPUNGE)
            mbx_expunge(stream, NIL, NIL);
        else
        {                     /* otherwise do a checkpoint to purge */
...
void mbx_flags(MAILSTREAM *stream, char *sequence, long flags)
{
    ...
    if (mbx_ping(stream) && /* ping mailbox, get new status for messages */
        ((flags & FT_UID) ? mail_uid_sequence(stream, sequence) : mail_sequence(stream, sequence)))
...
long mbx_text(MAILSTREAM *stream, unsigned long msgno, STRING *bs, long flags)
{
    ...
    if (!LOCAL)
        return NIL; /* mbx_flaglock() could have aborted */
...
void mbx_flag(MAILSTREAM *stream, char *sequence, char *flag, long flags)
{
    ...
    if (!stream->rdonly && LOCAL && (LOCAL->fd >= 0) && (LOCAL->ld >= 0))
    {
        ...
        if (((LOCAL->ffuserflag < NUSERFLAGS) &&
             stream->user_flags[LOCAL->ffuserflag]) ||
            (oldpid != LOCAL->lastpid))
...
{
    if (LOCAL)
        LOCAL->expok = T; /* mark that a check is desired */
    if (mbx_ping(stream))
            ...
            while (r && (++i <= sysibx->nmsgs))
            {
                ...
                if ((j = hdrlen + txtlen) != 0L)
                {
                    ...
                    sprintf(LOCAL->buf + strlen(LOCAL->buf),
                            ",%lu;00000000%04x-00000000\015\012", j, (unsigned)((fSEEN * elt->seen) + (fDELETED * elt->deleted) + (fFLAGGED * elt->flagged) + (fANSWERED * elt->answered) + (fDRAFT * elt->draft)));
                    /* copy message */
                    if ((write(LOCAL->fd, LOCAL->buf, strlen(LOCAL->buf)) < 0) ||
                        (write(LOCAL->fd, hdr, hdrlen) < 0) ||
                        (write(LOCAL->fd, txt, txtlen) < 0))
                        r = 0;
                }
                fs_give((void **)&hdr);
            }
...
long mbx_copy(MAILSTREAM *stream, char *sequence, char *mailbox, long options)
{
    ...
    for (i = 1; ret && (i <= stream->nmsgs); i++)
        ...
        {
            lseek(LOCAL->fd, elt->private.special.offset + elt->private.special.text.size, L_SET);
            ...
            for (j = elt->user_flags, k = 0; j;)
                if ((s = stream->user_flags[find_rightmost_bit(&j)]) != NULL)
                    for (m = 0; (m < NUSERFLAGS) && (t = dstream->user_flags[m]); m++)
                        if (!compare_cstring(s, t) && (k |= 1 << m))
                            break;
            sprintf(LOCAL->buf + strlen(LOCAL->buf), ",%lu;%08lx%04x-%08lx\015\012",
                    elt->rfc822_size, k, (unsigned)((fSEEN * elt->seen) + (fDELETED * elt->deleted) + (fFLAGGED * elt->flagged) + (fANSWERED * elt->answered) + (fDRAFT * elt->draft)), cu ? ++dstream->uid_last : 0);
            /* write target header */
    ...
    if (ret)
        tp[0] = time(0) - 1; /* set atime to now-1 if successful copy */
    /* else preserve \Marked status */
    else
        tp[0] = (sbuf.st_ctime > sbuf.st_atime) ? sbuf.st_atime : time(0);
    ...
    else
    {
        ...
        for (ret = LONGT; ret && message;)
        {
            ...
            { /* parse date if given */
                ...
                internal_date(tmp); /* get current date in IMAP format */
                                    /* write header */
            ...
            { /* write message */
                size_t j;
                if (!message->cursize)
                    SETPOS(message, GETPOS(message));
                for (errno = 0; !errno && i &&
                                (j = fwrite(message->curpos, 1, message->cursize, df));
                     i -= j)
                {
                ...
                if (i || !MM_APPEND(af)(dstream, data, &flags, &date, &message))
                    ...
                    mail_append_set(dst, dstream->uid_last);
            }
        }
...
long mbx_parse(MAILSTREAM *stream)
{
    ...
    i = (isxdigit(LOCAL->buf[HDRSIZE - 10]) && isxdigit(LOCAL->buf[HDRSIZE - 9]) &&
         ...
         (LOCAL->buf[HDRSIZE - 2] == '\015') && (LOCAL->buf[HDRSIZE - 1] == '\012'))
            ? strtoul(LOCAL->buf + HDRSIZE - 8, NIL, 16)
            : 0;
    /* set flagcheck if lastpid changed */
    if (LOCAL->lastpid && (LOCAL->lastpid != i))
        LOCAL->flagcheck = T;
    ...
    { /* while there is stuff to parse */
        ...
        *t++ = '\0'; /* break up fields */
        ...
        {
            if (m)
                lastuid = m; /* expunge message, update last UID seen */
            else
            { /* no UID assigned? */
...
unsigned long mbx_read_flags(MAILSTREAM *stream, MESSAGECACHE *elt)
{
    ...
    lseek(LOCAL->fd, (off_t)elt->private.special.offset + elt->private.special.text.size - 24, L_SET);
    /* read the new flags */
...
void mbx_update_status(MAILSTREAM *stream, unsigned long msgno, long flags)
{
    ...
    {                            /* readwrite */
        ...
        sprintf(LOCAL->buf, "%08lx%04x-%08lx", elt->user_flags, (unsigned)(((elt->deleted && flags) ? fEXPUNGED : (strtoul(LOCAL->buf + 9, NIL, 16)) & fEXPUNGED) + (fSEEN * elt->seen) + (fDELETED * elt->deleted) + (fFLAGGED * elt->flagged) + (fANSWERED * elt->answered) + (fDRAFT * elt->draft) + fOLD), elt->private.uid);
        while (T)
        { /* get to that place in the file */
            lseek(LOCAL->fd, (off_t)elt->private.special.offset + elt->private.special.text.size - 23, L_SET);
            /* write new flags and UID */
            if (write(LOCAL->fd, LOCAL->buf, 21) > 0)
                break;
            MM_NOTIFY(stream, strerror(errno), WARN);
            MM_DISKERROR(stream, errno, T);
        }
                         ...
                         unsigned long *size, char **hdr)
{
    ...
    if (hdr)
        *hdr = NIL; /* assume no header returned */
    /* is header size known? */
    if ((*size = elt->private.msg.header.text.size) != 0L)
        return ret;
    /* paranoia check */
    ...
    if (hdr)
        *hdr = LOCAL->buf; /* possibly return header too */
    return ret;
}
        ...
        for (i = 1, delta = 0, pos = ppos = HDRSIZE; i <= stream->nmsgs;)
        {
            ...
            {        /* preserved message */
                ...
                { /* moved, note first byte to preserve */
                    ...
                    { /* read from source position */
                        ...
                        while (T)
                        {
                            ...
                            if (write(LOCAL->fd, LOCAL->buf, m) > 0)
                                break;
                            MM_NOTIFY(stream, strerror(errno), WARN);
                            MM_DISKERROR(stream, errno, T);
                        }
                ...
                /* preserved but no deleted messages yet */
                    ...
                    pos = elt->private.special.offset + k;
            }
        }
...
long mbx_flaglock(MAILSTREAM *stream)
{
    ...
    if (!stream->rdonly && LOCAL && (LOCAL->fd >= 0) && (LOCAL->ld < 0))
    {
        ...
        { /* don't do this if flagcheck already needed */
            ...
            {                            /* know previous time? */
                fstat(LOCAL->fd, &sbuf); /* get current write time */
                if (LOCAL->filetime < sbuf.st_mtime)
                    LOCAL->flagcheck = T;
