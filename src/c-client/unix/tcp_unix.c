...
TCPSTREAM *tcp_open(char *host, char *service, unsigned long port)
{
    ...
    { /* won */
        ...
        if ((stream->ictr = ctr) != 0)
            *(stream->iptr = stream->ibuf) = tmp[0];
...
TCPSTREAM *tcp_aopen(NETMBX *mb, char *service, char *usrbuf)
{
    ...
    if (!sshpath)
        sshpath = cpystr(SSHPATH);
#endif
#ifdef RSHPATH /* rsh path defined yet? */
    if (!rshpath)
        rshpath = cpystr(RSHPATH);
#endif
    if (*service == '*')
    { /* want ssh? */
        /* return immediately if ssh disabled */
        if (!(sshpath && (ti = sshtimeout)))
            return NIL;
    ...
    if (mb->host[0] == '[' && mb->host[i = (strlen(mb->host)) - 1] == ']')
    {
        ...
        if ((adr = ip_stringtoaddr(host, &len, &i)) != NULL)
            fs_give((void **)&adr);
        else
        {
            sprintf(tmp, "Bad format domain-literal: %.80s", host);
    ...
    if (*service == '*') /* build ssh command */
        ...
        sprintf(tmp, rshcommand, rshpath, host,
    ...
    if (i <= 0)
    { /* timeout or error? */
        sprintf(tmp, i ? "error in %s to IMAP server" : "%s to IMAP server timed out", (*service == '*') ? "ssh" : "rsh");
        mm_log(tmp, WARN);
        ...
        { /* until request satisfied */
            ...
            {              /* non-timeout result from select? */
                if (i > 0) /* read what we can */
                    while (((i = read(stream->tcpsi, s, (int)min(maxposint, size))) < 0) && (errno == EINTR))
                        ;
                if (i <= 0)
                { /* error seen? */
                    if (tcpdebug)
                    {
                        char tmp[MAILTMPLEN];
                        if (i)
                            sprintf(s = tmp, "TCP buffer read I/O error %d", errno);
                        else
                            s = "TCP buffer read end of file";
                        mm_log(s, TCPDEBUG);
                    }
        ...
        { /* non-timeout result from select? */
            ...
            { /* error seen? */
                if (tcpdebug)
                {
                    char *s, tmp[MAILTMPLEN];
                    if (i)
                        sprintf(s = tmp, "TCP data read I/O error %d", errno);
                    else
                        s = "TCP data read end of file";
                    mm_log(s, TCPDEBUG);
                }
                return tcp_abort(stream);
            }
...
void tcp_close(TCPSTREAM *stream)
{
    ...
    if (stream->remotehost)
        fs_give((void **)&stream->remotehost);
    if (stream->localhost)
...
char *tcp_remotehost(TCPSTREAM *stream)
{
    if (!stream->remotehost)
    {
        ...
        stream->remotehost = /* get socket's peer name */
            getpeername(stream->tcpsi, sadr, (void *)&sadrlen) ? cpystr(stream->host) : tcp_name(sadr, NIL);
        fs_give((void **)&sadr);
    }
...
char *tcp_clientaddr()
{
    if (!myClientAddr)
    {
        ...
        { /* get stdin's peer name */
            ...
            if (myClientPort < 0)
                myClientPort = ip_sockaddrtoport(sadr);
        }
        fs_give((void **)&sadr);
    }
    return myClientAddr;
...
char *tcp_serveraddr()
{
    if (!myServerAddr)
    {
        ...
        { /* get stdin's name */
            ...
            if (myServerPort < 0)
                myServerPort = ip_sockaddrtoport(sadr);
        }
        fs_give((void **)&sadr);
    }
    return myServerAddr;
...
char *tcp_canonical(char *name)
{
    ...
    /* get canonical name */
    if (!ip_nametoaddr(name, NIL, NIL, &ret, NIL, NIL))
        ret = cpystr(name);
    (*bn)(BLOCK_NONSENSITIVE, data);
...
long tcp_isclienthost(char *host)
{
    ...
    if (tcp_clienthost() && myClientAddr)
    {
             ...
             adr = ip_nametoaddr(NIL, &adrlen, &family, NIL, &next, &cleanup))
        {
            ...
            if (!strcmp(myClientAddr, ip_sockaddrtostring(sadr, buf)))
                ret = LONGT;
...
void tcp_end(void)
{
    ...
    if (sshpath)
        fs_give((void **)&sshpath);
    if (myClientAddr)
        fs_give((void **)&myClientAddr);
    if (myClientHost)
        fs_give((void **)&myClientHost);
    if (myServerAddr)
        fs_give((void **)&myServerAddr);
    if (myServerHost)
        fs_give((void **)&myServerHost);
}
