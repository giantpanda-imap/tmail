   ...
   */
     ...
     */
        ...
        if (!fstatvfs(fd, &vsbuf) && (vsbuf.f_basetype[1] == 'f') &&
            ...
            return 0;
#endif /* NOFSTATVFS */
        if (!ustat(sbuf.st_dev, &usbuf) && !++usbuf.f_tinode)
            return 0;
    }
...
static long master(MAILSTREAM *stream, append_t af, void *data)
{
    ...
    if (pipe(pipei) < 0)
        mm_log("Can't create input pipe", ERROR);
            ...
            case 'A': /* append callback */
                if ((*af)(NIL, data, &s, &t, &message))
                {
                    if (i = message ? SIZE(message) : 0)
                    {
                        ...
                        s = t = ""; /* no flags or date if no message */
                    ...
                    if (i)
                        do
                            if (putc(c = 0xff & SNX(message), po) == EOF)
                            {
            ...
            case 'D': /* mm_diskerror() */
                ...
                if (s && (*s++ == ' '))
                {
                    ...
                    if (s && (*s++ == ' '))
                    {
                        ...
                        { /* recoverable on slave-created stream */
...
long safe_rename(DRIVER *dtb, MAILSTREAM *stream, char *old, char *newname)
{
    long ret = master(stream, NIL, NIL);
    if (lockslavep)
        exit((*dtb->mbxren)(stream, old, newname));
    return ret;
}
...
long safe_status(DRIVER *dtb, MAILSTREAM *stream, char *mbx, long flags)
{
    long ret = master(stream, NIL, NIL);
    if (lockslavep)
        exit((*dtb->status)(stream, mbx, flags));
    return ret;
}
                        ...
                        unsigned long csiz, unsigned long fsiz)
{
    ...
    if (lockslavep)
        exit(scan_contents(dtb, name, contents, csiz, fsiz));
    return ret;
}
...
long safe_copy(DRIVER *dtb, MAILSTREAM *stream, char *seq, char *mbx, long flags)
{
    ...
    if (lockslavep)
    {
        ...
        if (pc)
            mail_parameters(stream, SET_MAILPROXYCOPY, (void *)slaveproxycopy);
        exit((*dtb->copy)(stream, seq, mbx, flags));
    }
    /* do any proxycopy in master */
    if (lockproxycopy && pc)
        return (*pc)(stream, seq, mbx, flags);
    return ret;
}
                  ...
                  STRING **message)
{
    ...
    *flags = *date = NIL;   /* assume no flags or date */
    ...
    case '+': /* have message, get size of flags */
        ...
        {
            if (c == EOF)
                sprintf(tmp, "Pipe broken after flag size %lu", n);
            sprintf(tmp, "Missing delimiter after flag size %lu: %c", n, c);
            slave_fatal(tmp);
        }
        if (n)
            *flags = ad->flags = slave_append_read(n, "flags");
        /* get size of date */
        for (n = 0; isdigit(c = getc(slavein)); n *= 10, n += (c - '0'))
            ;
        if (c != ' ')
        {
            if (c == EOF)
                sprintf(tmp, "Pipe broken after date size %lu", n);
            else
                sprintf(tmp, "Missing delimiter after date size %lu: %c", n, c);
            slave_fatal(tmp);
        }
        if (n)
            *date = ad->date = slave_append_read(n, "date");
        /* get size of message */
        for (n = 0; isdigit(c = getc(slavein)); n *= 10, n += (c - '0'))
            ;
        if (c != ' ')
        {
            if (c == EOF)
                sprintf(tmp, "Pipe broken after message size %lu", n);
