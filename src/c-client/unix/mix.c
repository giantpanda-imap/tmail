...
long mix_isvalid(char *name, char *meta)
{
        ...
        !stat(dir, &sbuf) && ((sbuf.st_mode & S_IFMT) == S_IFDIR))
    {
        ...
        if (!stat(meta, &sbuf) && ((sbuf.st_mode & S_IFMT) == S_IFREG))
            ...
            errno = NIL; /* directory but not mix */
    }
    return NIL;
}
...
void *mix_parameters(long function, void *value)
{
    ...
    case GET_INBOXPATH:
        if (value)
            ret = mailboxfile((char *)value, "~/INBOX");
        break;
    case GET_DIRFMTTEST:
    ...
    case SET_ONETIMEEXPUNGEATPING:
        if (value)
            ((MIXLOCAL *)((MAILSTREAM *)value)->local)->expok = T;
    case GET_ONETIMEEXPUNGEATPING:
        if (value)
            ret = (void *)(((MIXLOCAL *)((MAILSTREAM *)value)->local)->expok ? VOIDT : NIL);
        break;
    }
    return ret;
}
...
long mix_create(MAILSTREAM *stream, char *mailbox)
{
    ...
    { /* success, write initial metadata */
        fprintf(f, SEQFMT, now);
        fprintf(f, MTAFMT, now, (unsigned long)0, now);
        for (i = 0, c = 'K'; (i < NUSERFLAGS) &&
                             (t = (stream && stream->user_flags[i]) ? stream->user_flags[i] : default_user_flag(i)) && *t;
             ++i)
        {
...
long mix_delete(MAILSTREAM *stream, char *mailbox)
{
    ...
    /* delete metadata */
        ...
        sprintf(tmp, "Can't delete mailbox %.80s index: %80s",
    ...
    if (fd >= 0)
        close(fd);      /* close any descriptor on metadata */
...
void mix_abort(MAILSTREAM *stream)
{
    ...
    { /* only if a file is open */
        ...
        if (LOCAL->mfd >= 0)
            close(LOCAL->mfd);
        if (LOCAL->index)
            fs_give((void **)&LOCAL->index);
        if (LOCAL->status)
            fs_give((void **)&LOCAL->status);
        if (LOCAL->sortcache)
            fs_give((void **)&LOCAL->sortcache);
                 ...
                 long flags)
{
    ...
    {                           /* document the problem */
        ...
        if ((s = strpbrk(LOCAL->buf, "\015\012")) != NULL)
            *s = '\0';
...
long mix_text(MAILSTREAM *stream, unsigned long msgno, STRING *bs, long flags)
{
    ...
    if (!(flags & FT_PEEK) && !elt->seen)
    {
        ...
        if (idxf)
            fclose(idxf); /* release index and status file */
        if (statf)
            fclose(statf);
    }
...
void mix_flag(MAILSTREAM *stream, char *sequence, char *flag, long flags)
{
    ...
    if (((flags & ST_UID) ? mail_uid_sequence(stream, sequence) : mail_sequence(stream, sequence)) &&
    ...
    if (statf)
        fclose(statf); /* release status file if still open */
    if (idxf)
...
long mix_ping(MAILSTREAM *stream)
{
    ...
    if (mail_parameters(NIL, GET_EXPUNGEATPING, NIL))
        LOCAL->expok = T;
    /* process metadata/index/status */
    if ((statf = mix_parse(stream, &idxf, LONGT, (LOCAL->internal ? NIL : LONGT))) != NULL)
    {
    ...
    if (!ret)
        mix_abort(stream); /* murdelyze stream if ping fails */
    return ret;
}
...
long mix_expunge(MAILSTREAM *stream, char *sequence, long options)
{
    ...
    if (!(ret = burponly || !sequence ||
                ((options & EX_UID) ? mail_uid_sequence(stream, sequence) : mail_sequence(stream, sequence))) ||
        stream->rdonly)
        ;
    /* read index and open status exclusive */
    else if ((statf = mix_parse(stream, &idxf, LONGT, LOCAL->internal ? NIL : LONGT)) != NULL)
    {
        ...
        if (!burponly)
            for (i = 1; i <= stream->nmsgs;)
            {
                ...
                if (elt->deleted && (sequence ? elt->sequence : T))
                {
                    ...
                    ++i; /* otherwise advance to next message */
            }
            ...
            { /* if have message files */
                ...
                for (i = 1, cur = burp; ret && (i <= stream->nmsgs); i++)
                {
                    ...
                    if (cur && (elt->private.spare.data != cur->fileno))
                    {
                        ...
                        if (elt->private.spare.data < cur->fileno)
                            cur = burp;
                    ...
                    /* if found, add to set */
                    if (cur)
                        ret = mix_addset(&cur->tail, elt->private.special.offset,
                ...
                if (ret) /* if no errors, burp all files */
                    for (cur = burp; ret && cur; cur = cur->next)
                    {
                        ...
                        if (cur->set.last)
                            ret = mix_burp(stream, cur, &reclaimed);
                        /* empty, delete it unless new msg file */
                        else if (mix_file_data(LOCAL->buf, stream->mailbox, cur->fileno) &&
                                 ((cur->fileno == LOCAL->newmsg) ? truncate(LOCAL->buf, 0) : unlink(LOCAL->buf)))
                        {
                            sprintf(LOCAL->buf,
                ...
                { /* flush the burp list */
                    cur = burp->next;
                    if (burp->name)
                        fs_give((void **)&burp->name);
                    fs_give((void **)&burp);
                    burp = cur;
                }
            }
            else
                MM_LOG("No mix message files found during expunge", WARN);
            /* free directory list */
            if ((a = (void *)names) != NULL)
                fs_give((void **)&a);
        }
    ...
    if (statf)
        fclose(statf); /* close status if still open */
    if (idxf)
        fclose(idxf);   /* close index if still open */
    LOCAL->expok = NIL; /* cancel expok */
    if (ret)
    { /* only if success */
        char *s = NIL;
        if (nexp)
            sprintf(s = LOCAL->buf, "Expunged %lu messages", nexp);
        else if (reclaimed)
            sprintf(s = LOCAL->buf, "Reclaimed %lu bytes of expunged space", reclaimed);
        else if (!burponly)
            s = stream->rdonly ? "Expunge ignored on readonly mailbox" : "No messages deleted, so no update needed";
        if (s)
            MM_LOG(s, (long)NIL);
    }
    return ret;
...
long mix_burp(MAILSTREAM *stream, MIXBURP *burp, unsigned long *reclaimed)
{
             ...
             !(f = fdopen(fd, "r+b")))
    {
        ...
        MM_LOG(LOCAL->buf, ERROR);
        if (fd >= 0)
            close(fd); /* in case fdopen() failure */
    }
    else if (fstat(fd, &sbuf))
    { /* get file size */
        sprintf(LOCAL->buf, staterr, burp->name, strerror(errno));
        MM_LOG(LOCAL->buf, ERROR);
        fclose(f);
        ...
        for (set = &burp->set, rpos = wpos = 0; set; set = set->next)
        {
                 ...
                 size; size -= wsize)
            {
                ...
                { /* data to skip at start? */
                    ...
                    wsize = size; /* nothing to skip, say we wrote it all */
                rpos += wsize;
                wpos += wsize;
            }
        }
...
long mix_copy(MAILSTREAM *stream, char *sequence, char *mailbox, long options)
{
    ...
    else if (!(ret = ((options & CP_UID) ? mail_uid_sequence(stream, sequence) : mail_sequence(stream, sequence))))
        ;
                        ...
                        : NIL) != 0L)
    {
        ...
        if (local->indexseq > seq)
            seq = local->indexseq + 1;
        if (local->statusseq > seq)
            seq = local->statusseq + 1;
        ...
        if ((msgf = mix_data_open(astream, &fd, &size, newsize)) != NULL)
        {
            ...
            if (ret && (ret = !fflush(msgf)))
            {
                            ...
                            mix_index_update(astream, idxf, LONGT))) != 0L)
                {
                    ...
                    if (cu)
                        (*cu)(stream, mailbox, astream->uid_validity, source, dest);
...
long mix_append(MAILSTREAM *stream, char *mailbox, append_t af, void *data)
{
    ...
    if (!stream)
        stream = user_flags(&mixproto);
    if (!ret)
        switch (errno)
        {            /* if not valid mailbox */
        case ENOENT: /* no such file? */
            if ((ret = compare_cstring(mailbox, "INBOX") ? NIL : mix_create(NIL, "INBOX")) != 0L)
                break;
                    ...
                    STRING *msg, SEARCHSET *set, unsigned long seq)
{
    ...
    elt->year = delt->year;
    elt->month = delt->month;
    elt->day = delt->day;
    elt->hours = delt->hours;
    elt->minutes = delt->minutes;
    elt->seconds = delt->seconds;
    elt->zoccident = delt->zoccident;
    elt->zhours = delt->zhours;
    elt->zminutes = delt->zminutes;
    /*
   * Do NOT set elt->valid here!  mix_status_update() uses it to determine
   * whether a message should be marked as old.
   */
    if (sf & fSEEN)
        elt->seen = T; /* copy flags to status */
    if (sf & fDELETED)
        elt->deleted = T;
    if (sf & fFLAGGED)
        elt->flagged = T;
    ...
    else if ((s = mix_meta_slurp(stream, &i)) != NULL)
    {
        ...
        else if (j || (i > LOCAL->indexseq))
        {
            ...
            while ((s = mix_read_record(*idxf, LOCAL->buf, LOCAL->buflen, "index")) &&
                ...
                {
                case ':': /* message record */
                    if (!(isxdigit(*++s) && (uid = strtoul(s, &t, 16))))
                        msg = "UID";
                                    ...
                                    if ((*s++ == ':') && isxdigit(*s))
                                    {
                                        ...
                                        if (*s++ == ':')
                                        {
                                            ...
                                            while (nmsgs <= stream->nmsgs)
                                            {
                                                ...
                                                {
                                                    if (LOCAL->expok)
                                                        mail_expunged(stream, nmsgs);
                                                    else
                                                    { /* message expunged, but not yet for us */
                                                        ++nmsgs;
                                                        elt->private.ghost = T;
                                                    }
                                                }
            ...
            if (!s)
                return NIL; /* barfage from mix_read_record() */
    ...
    { /* do nothing if stream readonly */
        ...
        { /* need to do expansion check? */
            ...
            for (i = 1, size = 0; i <= stream->nmsgs; ++i)
                if (!mail_elt(stream, i)->private.ghost)
                    ++size;
            if (size)
        ...
        {                 /* if still good to go */
            ...
            if (ret)
                ftruncate(fileno(idxf), ftell(idxf));
        }
    }
    return ret;
}
    ...
    { /* do nothing if stream readonly */
        ...
        {                  /* if still good to go */
            ...
            for (i = 1; ret && (i <= stream->nmsgs); ++i)
            {
                ...
                if (!elt->private.mod)
                    elt->private.mod = LOCAL->statusseq;
                if (!elt->private.ghost) /* only write living messages */
            ...
            if (ret)
                ftruncate(fileno(statf), ftell(statf));
        }
    }
    return ret;
}
                    ...
                    unsigned long newsize)
{
    ...
    unsigned long curend = (elt && (elt->private.spare.data == LOCAL->newmsg)) ? elt->private.special.offset + elt->private.msg.header.offset +
                                                                                     elt->rfc822_size
                                                                               : 0;
                    ...
                    O_RDWR | (curend ? NIL : O_CREAT), NIL)) >= 0)
    {
        ...
        { /* short file or becoming too long */
            if (curend > sbuf.st_size)
            {
                char tmp[MAILTMPLEN];
            ...
            errno = NIL;
            while ((*fd = open(mix_file_data(LOCAL->buf, stream->mailbox,
    ...
    { /* have a data file? */
        ...
        if ((msgf = fdopen(*fd, "r+b")) != NULL)
            fseek(msgf, *size, SEEK_SET);
        else
            close(*fd); /* fdopen() failed? */
    }
                                    ...
                                    if ((*s++ == ':') && isxdigit(*s))
                                    {
                                        ...
                                        if (*s++ == ':')
                                        {
                                            ...
                                            if ((i = mail_msgno(stream, uid)) != 0L)
                                            {
                                                ...
                                                {
                                                    if (sc->from)
                                                        fseek(srtcf, fromlen + 2, SEEK_CUR);
                                                ...
                                                {
                                                    if (sc->subject)
                                                        fseek(srtcf, subjlen + 2, SEEK_CUR);
                                            ...
                                            /* UID not found, ignore this message */
                                                ...
                                                fseek(srtcf, ((fromlen ? fromlen + 2 : 0) + (tolen ? tolen + 2 : 0) + (cclen ? cclen + 2 : 0) + (subjlen ? subjlen + 2 : 0) + (msgidlen ? msgidlen + 2 : 0) + (reflen ? reflen + 2 : 0)),
                                                      SEEK_CUR);
                                            continue;
                                        }
                                        else
...
long mix_sortcache_update(MAILSTREAM *stream, FILE **sortcache)
{
    ...
    { /* ignore if no file */
        ...
        mailcache_t mc = (mailcache_t)mail_parameters(NIL, GET_CACHE, NIL);
        for (i = 1; (i <= stream->nmsgs) &&
                    !((SORTCACHE *)(*mc)(stream, i, CH_SORTCACHE))->dirty;
             ++i)
            ;
        if (i <= stream->nmsgs)
        {              /* only update if some entry is dirty */
            ...
            for (i = 1; ret && (i <= stream->nmsgs); ++i)
            {
                ...
                if (s->from)
                    fprintf(f, "F%s\015\012", s->from);
                if (s->to)
                    fprintf(f, "T%s\015\012", s->to);
                if (s->cc)
                    fprintf(f, "C%s\015\012", s->cc);
                if (s->subject)
                    fprintf(f, "S%s\015\012", s->subject);
                if (s->message_id)
                    fprintf(f, "M%s\015\012", s->message_id);
                if (j)
...
char *mix_read_record(FILE *f, char *buf, unsigned long buflen, char *type)
{
    ...
    while (fgets(buf, buflen - 1, f))
    {
        if ((s = strchr(buf, '\012')) != NULL)
        {
            ...
            *s = '\0'; /* tie off buffer */
            if (s != buf)
                return buf; /* return if non-empty buffer */
            sprintf(tmp, "Empty mix %s record", type);
            MM_LOG(tmp, WARN);
        }
        else if (buf[buflen - 2])
        { /* overlong record is bad news */
...
unsigned long mix_read_sequence(FILE *f)
{
    unsigned long ret;
    char *s, tmp[MAILTMPLEN];
    if (!mix_read_record(f, tmp, MAILTMPLEN - 1, "sequence"))
        return NIL;
    switch (tmp[0])
    {            /* examine record */
    case '\0':   /* end of file */
...
char *mix_dir(char *dst, char *name)
{
    ...
    if (!mailboxfile(dst, name))
        *dst = '\0';
    /* driver-selected INBOX  */
    else if (!*dst)
        mailboxfile(dst, "~/INBOX");
    /* tie off unnecessary trailing / */
    else if ((s = strrchr(dst, '/')) && !s[1])
        *s = '\0';
    return dst;
}
...
char *mix_file_data(char *dst, char *dir, unsigned long data)
{
    char tmp[MAILTMPLEN];
    if (data)
        sprintf(tmp, "%08lx", data);
    else
        tmp[0] = '\0'; /* compatibility with experimental version */
