/* ========================================================================
 * Copyright 2004-2014 Eduardo Chappa <chappa@gmx.com>
 *
...
/*
 * Program:	Maildir driver routines
 *
 * Author: Eduardo Chappa <chappa@gmx.com>
 *
 * Date:	   April 13, 2004
...
long maildir_dirfmttest(char *name)
{
    ...
    for (i = 0; mdstruct[i] && strcmp(name, mdstruct[i]); i++)
        ;
    return (i < EndDir) || !strcmp(name, MDDIR) || !strncmp(name, MDUIDLAST, strlen(MDUIDLAST)) || !strncmp(name, MDUIDTEMP, strlen(MDUIDTEMP)) ? LONGT : NIL;
}
...
void courier_realname(char *name, char *realname)
{
    ...
    {
        realname[i] = name[j];
        if (name[j] == '/' && name[j + 1] != '.' && name[j + 1] != '%' && name[j + 1] != '*')
            realname[++i] = '.';
    }
    if (realname[i - 1] == '.')
        i--;
...
int maildir_file_path(char *name, char *tmp, size_t sizeoftmp)
{
    ...
    if (strlen(myrootdir(rname)) +
            max(strlen(rname), strlen(maildirpath)) >
        sizeoftmp)
    {
        errno = ENAMETOOLONG;
...
DRIVER *maildir_valid(char *name)
{
    ...
    return maildir_valid_dir(tmpname)
               ? (IS_COURIER(name) ? &courierdriver : &maildirdriver)
               : NIL;
}
...
int maildir_eliminate_duplicate(char *name, struct direct ***flist, unsigned long *nfiles)
{
                ...
                unsigned long *nfiles, int *scand, int flag)
{
    ...
    *nfiles = 0;
    if ((stat(name, &sbuf) < 0) || (flag == CCLIENT && ((rv = maildir_eliminate_duplicate(name, flist, nfiles)) < 0)))
        return 0L;
...
MAILSTREAM *maildir_open(MAILSTREAM *stream)
{
    ...
    if (LOCAL->dir)
    {
        ...
        LOCAL->path[Cur] = cpystr(tmp);
        MDFLD(tmp, LOCAL->dir, New);
        LOCAL->path[New] = cpystr(tmp);
        MDFLD(tmp, LOCAL->dir, Tmp);
        LOCAL->path[Tmp] = cpystr(tmp);
...
void maildir_parse_folder(MAILSTREAM *stream, int full)
{
    ...
    if (stream->rdonly ||
        (LOCAL && ((maildir_initial_check(stream, Cur) == 0) || nfilesnew > 0L)))
    {
        ...
        { /* move new messages to cur */
            ...
            if (lstat(file, &sbuf) == 0)
                ...
                case S_IFREG:
                    ...
                    if ((mdstr = strstr(tmp, MDSEP(3))) || (mdstr = strstr(tmp, MDSEP(2))))
                        *(mdstr + 1) = '2';
                    else
                        strcat(tmp, MDSEP(2));
...
void maildir_close(MAILSTREAM *stream, long options)
{
    ...
    if (options & CL_EXPUNGE)
        maildir_expunge(stream, NIL, NIL);
    maildir_abort(stream);
    if (mdfpath)
        fs_give((void **)&mdfpath);
    if (mypid)
                                    ...
                                    DirNamesType dirtype)
{
    ...
    if (LOCAL->fd >= 0)
    {
        ...
        for (i = 0L, b = s, c = *b; b &&
                                    ((c < '\016' && ((c == '\012' && ++i) || (c == '\015' && *(b + 1) == '\012' && ++b && (i += 2)))) || b < t);
             i++, c = *++b)
            ;
        elt->rfc822_size = i;
        fs_give((void **)&s);
...
int maildir_update_elt_maildirp(MAILSTREAM *stream, unsigned long msgno)
{
    ...
    if (in_list && pos >= 0L && pos < nfiles && !strcmp(MDFILE(elt), names[pos]->d_name))
    {
        in_list = NIL;
        maildir_abort(stream);
    }
...
void maildir_list(MAILSTREAM *stream, char *ref, char *pat)
{
    ...
    if ((!pat || !*pat) && maildir_canonicalize(test, ref, "*") && maildir_valid_name(test))
    { /* there is a #md/ leading here */
        for (i = 3L; test[i] && test[i] != '/'; i++)
            ;
        if ((s = strchr(test + i + 1, '/')) != NULL)
            *++s = '\0';
        else
            test[0] = '\0';
    ...
    else if (maildir_canonicalize(test, ref, pat))
    {
        ...
        {   /* looking down levels? */
            ...
            if ((s = strpbrk(test, "%*")) != NULL)
            {
                ...
                strcpy(file, test + 4); /* use just that name then */
                                        /* find directory name */
...
long maildir_create(MAILSTREAM *stream, char *mailbox)
{
    ...
    create_dir = mailbox ? (mailbox[strlen(mailbox) - 1] ==
                            MDSEPARATOR(IS_COURIER(mailbox)))
                         : 0;
    maildir_file_path(mailbox, tmp, sizeof(tmp));
    strcpy(tmp, mailbox);
...
void maildir_flagmsg(MAILSTREAM *stream, MESSAGECACHE *elt)
{
    ...
    if (elt->valid)
    {
        for (try = 1; try > 0 && try < MAXTRY; try ++)
        {
            ...
            if ((s = strrchr(MDFILE(elt), FLAGSEP)))
                *s = '\0';
...
long maildir_expunge(MAILSTREAM *stream, char *sequence, long options)
{
    ...
    ret = sequence ? ((options & EX_UID) ? mail_uid_sequence(stream, sequence) : mail_sequence(stream, sequence)) : LONGT;
    if (ret == 0L)
        return 0L;
    for (i = 1L; i <= stream->nmsgs;)
    {
        ...
        if (elt->deleted && (sequence ? elt->sequence : T))
        {
            ...
            if (elt->private.spare.ptr)
                maildir_free_file((void **)&elt->private.spare.ptr);
            if (elt->recent)
                --recent;             /* if recent, note one less recent message */
    ...
    if ((options & CP_UID) ? mail_uid_sequence(stream, sequence) : mail_sequence(stream, sequence))
        for (i = 1L; i <= stream->nmsgs; i++)
            if ((elt = mail_elt(stream, i))->sequence)
            {
                ...
                if (((fd = open(path, O_RDONLY, NIL)) < 0) || ((!elt->rfc822_size &&
...
long maildir_append(MAILSTREAM *stream, char *mailbox, append_t af, void *data)
{
    ...
    do
    {
        ...
        do
        {
            ...
            if (stat(path1, &sbuf) < 0 && errno == ENOENT && stat(path2, &sbuf) < 0 && errno == ENOENT)
                done++;
            else
                fail++;
            if (fail == 1000)
...
long maildir_delete(MAILSTREAM *stream, char *mailbox)
{
   ...
   */
    ...
    while ((d = readdir(dirp)) != NULL)
    {
        if (strcmp(d->d_name, ".") && strcmp(d->d_name, "..") && (!strcmp(d->d_name, MDDIR) || !strncmp(d->d_name, MDUIDLAST, strlen(MDUIDLAST)) || !strncmp(d->d_name, MDUIDTEMP, strlen(MDUIDTEMP))))
        {
...
void maildir_list_work(MAILSTREAM *stream, char *dir, char *pat, long level)
{
    ...
    {
        if (dir)
            snprintf(name, sizeof(name), "%s%s/", MDPREFIX(CCLIENT), dir);
        else
            strcpy(name, pat);
        ...
        while ((d = readdir(dp)) != NULL)
            if (strcmp(d->d_name, ".") && strcmp(d->d_name, "..") && strcmp(d->d_name, MDNAME(Cur)) && strcmp(d->d_name, MDNAME(Tmp)) && strcmp(d->d_name, MDNAME(New)))
            {

                if (dir)
                    snprintf(tmp, sizeof(tmp), "%s%s", name, d->d_name);
                else
                    strcpy(tmp, d->d_name);
                ...
                if (pmatch_full(tmp, pat, '/'))
                {
                    ...
                    if (stat(tmp, &sbuf) == 0 && ((sbuf.st_mode & S_IFMT) == S_IFDIR))
                    {
                        if (dir)
                            snprintf(tmp, sizeof(tmp), "%s%s", name, d->d_name);
                        else
                            ...
                            unsigned long bottom, unsigned long top, unsigned long *pos)
{
    ...
    { /* 0 <= 0 < 1 */
        ...
        else if (same_maildir_file(msgname, names[top]->d_name))
        {
            rv = T;
            if (pos)
                *pos = top;
...
int maildir_dir_is_empty(char *mailbox)
{
    ...
    if (courier)
    {
        ...
        {
            snprintf(tmp, sizeof(tmp), "%s/%s", tmp2, d->d_name);
            if (!strncmp(tmp, tmp3, strlen(tmp3)) && tmp[strlen(tmp3)] == '.')
            {
                rv = 0;
                break;
            }
        ...
        {
            snprintf(tmp, sizeof(tmp), "%s/%s", tmp2, d->d_name);
            if (strcmp(d->d_name, ".") && strcmp(d->d_name, "..") && strcmp(d->d_name, MDNAME(Cur)) && strcmp(d->d_name, MDNAME(Tmp)) && strcmp(d->d_name, MDNAME(New)) && strcmp(d->d_name, MDDIR) && strcmp(d->d_name, MDUIDVALIDITY) && !(d->d_name[0] == '.' && stat(tmp, &sbuf) == 0 && S_ISREG(sbuf.st_mode)))
            {
                rv = 0;
                break;
            }
...
{
    return name->d_name[0] == '.' && (strlen(name->d_name) > 2 || (strlen(name->d_name) == 2 && name->d_name[1] != '.'));
}
...
COURIER_S *courier_list_dir(char *curdir)
{
   ...
   */
    ...
    if (scand > 0)
    {
        ...
        {
            td = realname[strlen(realname) - 1] == '.' && *names[i]->d_name == '.';
...
int maildir_can_assign_uid(MAILSTREAM *stream)
{
    ...
    if (!stream || stream->rdonly || !LOCAL || !LOCAL->dir || !(dir = opendir(LOCAL->dir)))
        return 0;
   ...
   */
    ...
    if (ownuid > 0)
    {
        ...
        if (ownuid != existuid)
        { /* lock files for different sessions */
            if (LOCAL->uidtempfile)
            {
                unlink(LOCAL->uidtempfile);
                fs_give((void **)&LOCAL->uidtempfile);
...
void maildir_uid_renew_tempfile(MAILSTREAM *stream)
{
    ...
    if (!stream || stream->rdonly || !LOCAL || !LOCAL->candouid || !LOCAL->dir || !LOCAL->uidtempfile)
        return;
