...
#undef SAFE_DELETE
#define SAFE_DELETE(dtb, stream, mbx) (dtb->flags & DR_LOCKING) ? safe_delete(dtb, stream, mbx) : (*dtb->mbxdel)(stream, mbx)
#undef SAFE_RENAME
#define SAFE_RENAME(dtb, stream, old, newname) (dtb->flags & DR_LOCKING) ? safe_rename(dtb, stream, old, newname) : (*dtb->mbxren)(stream, old, newname)
#undef SAFE_STATUS
#define SAFE_STATUS(dtb, stream, mbx, bits) (dtb->flags & DR_LOCKING) ? safe_status(dtb, stream, mbx, bits) : (*dtb->status)(stream, mbx, bits)
#undef SAFE_SCAN_CONTENTS
#define SAFE_SCAN_CONTENTS(dtb, name, contents, csiz, fsiz) \
    (!dtb || (dtb->flags & DR_LOCKING)) ? safe_scan_contents(dtb, name, contents, csiz, fsiz) : scan_contents(dtb, name, contents, csiz, fsiz)
#undef SAFE_COPY
#define SAFE_COPY(dtb, stream, seq, mbx, bits) (dtb->flags & DR_LOCKING) ? safe_copy(dtb, stream, seq, mbx, bits) : (*dtb->copy)(stream, seq, mbx, bits)
#undef SAFE_APPEND
#define SAFE_APPEND(dtb, stream, mbx, af, data) (dtb->flags & DR_LOCKING) ? safe_append(dtb, stream, mbx, af, data) : (*dtb->append)(stream, mbx, af, data)
