    ...
    else
    {
            ...
            { /* negotiate continuation with KDC */
                ...
                switch (smj = /* make sure continuation going OK */
                        gss_init_sec_context(&smn, GSS_C_NO_CREDENTIAL, &ctx,
                                             crname, GSS_C_NO_OID, GSS_C_INTEG_FLAG | GSS_C_MUTUAL_FLAG | GSS_C_REPLAY_FLAG, 0,
        ...
        case GSS_S_COMPLETE:
                ...
                (resp.length >= 4) && (*((char *)resp.value) & AUTH_GSSAPI_P_NONE))
            {
                ...
                buf.value = tmp;
                buf.length = strlen(user) + 4;
                /* successful negotiation */
                switch (smj = gss_wrap(&smn, ctx, NIL, qop, &buf, &conf, &resp))
                {
                case GSS_S_COMPLETE:
                    if ((*responder)(stream, resp.value, resp.length))
                        ret = T;
                    gss_release_buffer(&smn, &resp);
                    break;
                default:
            ...
            do
                ...
                case GSS_S_COMPLETE: /* end of message, can kinit? */
                        ...
                        (*ki)(mb->host, (char *)resp.value))
                    {
                        ...
                        (*responder)(stream, NIL, 0);
                case GSS_S_CONTINUE_NEEDED:
                    sprintf(tmp, kerberos_try_kinit(smn) ? "Kerberos error: %.80s (try running kinit) for %.80s" : "GSSAPI failure: %s for %.80s", (char *)resp.value, mb->host);
                    mm_log(tmp, WARN);
                    gss_release_buffer(&dsmn, &resp);
                }
            while (dsmj == GSS_S_CONTINUE_NEEDED);
...
char *auth_gssapi_server(authresponse_t responder, int argc, char *argv[])
{
    ...
    if ((gss_import_name(&smn, &buf, GSS_C_NT_HOSTBASED_SERVICE, &crname)) ==
        GSS_S_COMPLETE)
    {
        if ((smj = gss_acquire_cred(&smn, crname, 0, NIL, GSS_C_ACCEPT, &crd, NIL, NIL)) == GSS_S_COMPLETE)
        {
