                     ...
                     unsigned long *trial, char *user)
{
    ...
    if ((challenge = (*challenger)(stream, &clen)) != NULL)
    {
        ...
        { /* got password, build response */
            sprintf(pwd, "%.65s %.33s", user, hmac_md5(hshbuf, challenge, clen, pwd, strlen(pwd)));
            fs_give((void **)&challenge);
    ...
    if ((user = (*responder)(chal, cl = strlen(chal), NIL)) != NULL)
    {
        ...
        if ((hash = strrchr(user, ' ')) != NULL)
        {
            *hash++ = '\0'; /* tie off user */
            ...
            if ((authuser = strchr(user, '*')) != NULL)
                *authuser++ = '\0';
            /* get password */
            if ((p = auth_md5_pwd((authuser && *authuser) ? authuser : user)) != NULL)
            {
                pl = strlen(p);
                u = (md5try && !strcmp(hash, hmac_md5(hshbuf, chal, cl, p, pl))) ? user : NIL;
