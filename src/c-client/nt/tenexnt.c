...
long tenex_text(MAILSTREAM *stream, unsigned long msgno, STRING *bs, long flags)
{
  ...
  { /* if internal representation wanted */
    ...
    /* slurp the data */
    if (read(LOCAL->fd, LOCAL->buf, i) != (long)i)
      return NIL;
...
long tenex_append(MAILSTREAM *stream, char *mailbox, append_t af, void *data)
{
  ...
  if (((fd = open(file, O_BINARY | O_WRONLY | O_APPEND | O_CREAT, S_IREAD | S_IWRITE)) < 0) || !(df = fdopen(fd, "ab")))
  {
  ...
  { /* parse flags */
    ...
    { /* write message */
      ...
      /* get next message */
      if (size || !(*af)(stream, data, &flags, &date, &message))
        ret = NIL;
    }
  } while (ret && message);
  /* if error... */
...
void tenex_update_status(MAILSTREAM *stream, unsigned long msgno, long syncflag)
{
  ...
  {                      /* readwrite */
    ...
    { /* get to that place in the file */
      lseek(LOCAL->fd, (off_t)elt->private.special.offset + elt->private.special.text.size - 13, L_SET);
      /* write new flags */
      if (write(LOCAL->fd, LOCAL->buf, 12) > 0)
        break;
      mm_notify(stream, strerror(errno), WARN);
      mm_diskerror(stream, errno, T);
    }
