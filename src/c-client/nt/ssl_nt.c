...
static SSLSTREAM *ssl_start(TCPSTREAM *tstream, char *host, unsigned long flags)
{
  ...
  {                    /* failed? */
    ...
    case '*':   /* certificate failure */
      ...
      { /* no error callback, build error message */
...
static char *ssl_start_work(SSLSTREAM *stream, char *host, unsigned long flags)
{
                               ...
                               host)))
  {
    ...
    if (scq)
      return (*scq)(err, host, cert ? buf : "???") ? NIL : "";
...
static char *ssl_validate_cert(X509 *cert, char *host)
{
  ...
  if (!cert)
    ret = "No certificate from server";
  /* and that it has a name */
  else if (!(cname = X509_get_subject_name(cert)))
  ...
  if (s != NULL)
  {
    ...
    ret = ssl_compare_hostnames(host, s) ? NIL : "Server name does not match certificate";
...
static long ssl_compare_hostnames(unsigned char *s, unsigned char *pat)
{
  ...
  case '*': /* wildcard */
    if (pat[1])
    { /* there must be a pattern suffix */
      /* there is, scan base against it */
      do
        if (ssl_compare_hostnames(s, pat + 1))
          ret = LONGT;
      while (!ret && (*s != '.') && *s++);
    }
    break;
  case '\0': /* end of pattern */
    if (!*s)
      ret = LONGT; /* success if base is also at end */
    break;
  default: /* non-wildcard, recurse if match */
    if (!compare_uchar(*pat, *s))
      ret = ssl_compare_hostnames(s + 1, pat + 1);
    break;
  }
  return ret;
}
...
char *ssl_getline(SSLSTREAM *stream)
{
  ...
  { /* got a line needing continuation? */
    ...
    { /* stash final part of line on list */
      ...
      for (n = 0, stc = stl; stc; stc = stc->next)
        n += stc->text.size;
...
long ssl_getdata(SSLSTREAM *stream)
{
  ...
  time_t t = time(0);
  blocknotify_t bn = (blocknotify_t)mail_parameters(NIL, GET_BLOCKNOTIFY, NIL);
  if (!stream->con || ((sock = SSL_get_fd(stream->con)) < 0))
    return NIL;
    ...
    {            /* non-timeout result from select? */
      ...
      { /* error seen? */
        if (tcpdebug)
        {
          ...
          if (i)
            sprintf(s = tmp, "SSL data read I/O error %d SSL error %d",
                    errno, SSL_get_error(stream->con, i));
          else
            s = "SSL data read end of file";
          mm_log(s, TCPDEBUG);
        }
...
void ssl_server_init(char *server)
{
  ...
  /* create context */
#ifdef OPENSSL_1_1_0
  if (!(stream->context = SSL_CTX_new(start_tls ? TLS_server_method() : SSLv23_server_method())))
#else
  if (!(stream->context = SSL_CTX_new(start_tls ? TLSv1_server_method() : SSLv23_server_method())))
#endif /* OPENSSL_1_1_0 */
...
long ssl_server_input_wait(long seconds)
{
  ...
  if (((stream = sslstdio->sslstream)->ictr > 0) ||
      !stream->con || ((sock = SSL_get_fd(stream->con)) < 0))
    return LONGT;
...
static SSLSTREAM *ssl_start(TCPSTREAM *tstream, char *host, unsigned long flags)
{
  ...
  /* acquire credentials */
  if (sft->AcquireCredentialsHandle(NIL, UNISP_NAME, SECPKG_CRED_OUTBOUND, NIL, (flags & NET_TLSCLIENT) ? &tlscred : NIL, NIL, NIL, &stream->cred, &t) != SEC_E_OK)
    reason = "Acquire credentials handle failed";
  else
    while (!reason)
    { /* negotiate security context */
      /* initialize buffers */
      ibuf[0].cbBuffer = size;
      ibuf[0].pvBuffer = buf;
      ibuf[1].cbBuffer = 0;
      ibuf[1].pvBuffer = NIL;
      obuf[0].cbBuffer = 0;
      ...
      obufs.pBuffers = obuf;
      /* negotiate security */
      e = sft->InitializeSecurityContext(&stream->cred, size ? &stream->context : NIL, host, req, 0,
      ...
      if (!reason)
        ...
        case SEC_E_OK: /* success, any data to be regurgitated? */
          ...
          if (certNameToStr && !(flags & NET_NOVALIDATECERT))
          {
            ...
            /* get certificate */
            if ((sft->QueryContextAttributes(&stream->context, SECPKG_ATTR_REMOTE_CERT_CONTEXT, &cert) !=
                 SEC_E_OK) ||
                !cert)
            {
            ...
            { /* get certificate subject name */
              ...
              if (!(*certGetCertificateChain)(NIL, cert, NIL, cert->hCertStore, &chparam, NIL, NIL, &chain))
              ...
              { /* validate certificate chain */
                ...
                if (!(*certVerifyCertificateChainPolicy)(CERT_CHAIN_POLICY_SSL, chain, &polparam, &status))
                  reason = ssl_analyze_status(GetLastError(), tmp);
            ...
            { /* got an error? */
              /* application callback */
              if (scq)
                reason = (*scq)((*reason == '*') ? reason + 1 : reason,
                                host, certname)
                             ? NIL
                             : "";
              else if (*certname)
          ...
          if (reason ||
              (reason = ssl_analyze_status(sft->QueryContextAttributes(&stream->context, SECPKG_ATTR_STREAM_SIZES, &stream->sizes), buf)))
...
long ssl_getdata(SSLSTREAM *stream)
{
  ...
  { /* decrypted buffer empty? */
    ...
    if (!stream->tcpstream)
      return NIL;
    do
    { /* yes, make sure have data from TCP */
      if (stream->iextractr)
    ...
    } while ((status = ((DECRYPT_MESSAGE_FN)sft->Reserved4)(&stream->context, &msg, 0, NIL)) == SEC_E_INCOMPLETE_MESSAGE);
    switch (status)
    {
    case SEC_E_OK:          /* won */
...
long ssl_sout(SSLSTREAM *stream, char *string, unsigned long size)
{
  ...
  for (s = stream->ibuf, n = 0; size;)
  {
    ...
    if ((((ENCRYPT_MESSAGE_FN)sft->Reserved3)(&stream->context, 0, &msg, NIL) != SEC_E_OK) ||
...
static long ssl_abort(SSLSTREAM *stream)
{
  ...
  if (stream->ibuf)
    fs_give((void **)&stream->ibuf);
  if (stream->obuf)
    fs_give((void **)&stream->obuf);
  return NIL;
}
