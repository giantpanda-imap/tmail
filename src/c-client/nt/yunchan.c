...
int flock(int fd, int op)
{
  ...
  int ret = -1;
  blocknotify_t bn = (blocknotify_t)((op & LOCK_NB) ? NIL : mail_parameters(NIL, GET_BLOCKNOTIFY, NIL));
  if (hdl < 0)
    errno = EBADF; /* error in file descriptor */
  else
    switch (op & ~LOCK_NB)
                           ...
                           unsigned long srcl)
{
  ...
  { /* make a new buffer if needed */
    *dst = (char *)fs_get((*dstl = i) + 1);
    if (dstl)
      *dstl = i; /* return new buffer length to main program */
  }
...
FILE *create_tempfile(void)
{
  ...
  { /* if got temporary name... */
    ...
    if (ret = fopen(s, "w+b"))
      add_tmpfile(&win_tmp, ret, s);
    else
      fs_give((void **)&s); /* flush temporary string */
  }
  return ret;
}
