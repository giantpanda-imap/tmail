        ...
        isxdigit(hdr[22]) && (hdr[23] == '\015') && (hdr[24] == '\012'))
    {
      ...
      { /* lock if making a mini-stream */
        ...
        /* reread data now that locked */
                 ...
                 (read(fd, hdr + 1, HDRSIZE - 1) != (HDRSIZE - 1)))
          ret = -1;
        else
        {
...
MAILSTREAM *mbx_open(MAILSTREAM *stream)
{
  ...
  char tmp[MAILTMPLEN];
  if (!stream)
    return &mbxproto; /* return prototype for OP_PROTOTYPE call */
  if (stream->local)
  ...
  else
  {
    ...
    for (ret = LONGT; ret && message;)
    {
      ...
      { /* write message */
        ...
        /* get next message */
        if (i || !(*af)(dstream, data, &flags, &date, &message))
          ret = NIL;
        else if (au)
