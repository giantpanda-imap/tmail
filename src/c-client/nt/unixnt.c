...
long unix_rename(MAILSTREAM *stream, char *old, char *newname)
{
  ...
  if (!dummy_file(file, old) ||
      (newname && (!(s = dummy_file(tmp, newname)) ||
                   ((s = strrchr(s, '\\')) && !s[1]))))
    sprintf(tmp, newname ? "Can't rename mailbox %.80s to %.80s: invalid name" : "Can't delete mailbox %.80s: invalid name",
            old, newname);
...
long unix_expunge(MAILSTREAM *stream, char *sequence, long options)
{
  ...
  char *msg = NIL;
  /* parse and lock mailbox */
  if (ret = (sequence ? ((options & EX_UID) ? mail_uid_sequence(stream, sequence) : mail_sequence(stream, sequence)) : LONGT) &&
...
long unix_copy(MAILSTREAM *stream, char *sequence, char *mailbox, long options)
{
  ...
  if (ret)
    ...
    times.actime = /* else preserve \Marked status */
        ((sbuf.st_ctime > sbuf.st_atime) || (sbuf.st_mtime > sbuf.st_atime)) ? sbuf.st_atime : times.modtime;
  utime(file, &times);        /* set the times */
...
long unix_append(MAILSTREAM *stream, char *mailbox, append_t af, void *data)
{
  ...
  /* get first message */
  if (!(*af)(tstream, data, &flags, &date, &message))
    return NIL;
  if (!(sf = tmpfile()))
  ...
  { /* parse date */
    ...
    { /* user wants to suppress time zones? */
      ...
      /* get next message */
      else if ((*af)(tstream, data, &flags, &date, &message))
        continue;
    }
                     ...
                     STRING *msg)
{
  ...
  /* write metadata */
  if (fprintf(sf, "%ld %lu ", f, SIZE(msg) + 2) < 0)
    return NIL;
  for (s = date; *s; *s++)
    switch (*s)
    {
  ...
  { /* need to muck with times? */
    ...
    { /* readonly with recent messages */
      if ((sbuf.st_atime >= sbuf.st_mtime) ||
        ...
        times.actime = (times.modtime = (sbuf.st_mtime < now) ? sbuf.st_mtime : now) - 1;
      else
                  ...
                  long flags)
{
  ...
  if (!size)
  { /* no messages and no pseudo, make one now */
  ...
  if (ret = unix_extend(stream, size))
  {
      ...
      {                   /* preserve this message */
              ...
              unix_xstatus(stream, LOCAL->buf, elt, NIL, flag))))
        {
          ...
          if (f.curpos != f.protect)
          {
            ...
            f.protect = (i <= stream->nmsgs) ? mail_elt(stream, i)->private.special.offset : (f.curpos + j + 2);
    ...
    if (size != ((unsigned long)f.filepos))
...
long unix_extend(MAILSTREAM *stream, unsigned long size)
{
  unsigned long i = (size > ((unsigned long)LOCAL->filesize)) ? size - ((unsigned long)LOCAL->filesize) : 0;
  if (i)
