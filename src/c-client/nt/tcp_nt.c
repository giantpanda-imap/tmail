...
TCPSTREAM *tcp_open(char *host, char *service, unsigned long port)
{
  ...
  { /* got a socket, create TCP/IP stream */
                    ...
                    char *tmp, char *hst)
{
  ...
  else
  {
    ...
    if (ttmo_open)
      WSAEventSelect(sock, event = WSACreateEvent(), FD_CONNECT);
    else
      event = 0; /* no event */
    /* open connection */
    ...
    if (event)
      while (err == WSAEWOULDBLOCK)
        ...
        case WSA_WAIT_EVENT_0: /* got an event? */
          err = (WSAEnumNetworkEvents(sock, event, &events) == SOCKET_ERROR) ? WSAGetLastError() : events.iErrorCode[FD_CONNECT_BIT];
          break;
    ...
    switch (err)
    {       /* analyze result from connect and wait */
...
long tcp_getbuffer(TCPSTREAM *stream, unsigned long size, char *s)
{
  ...
  if (stream->tcpsi == INVALID_SOCKET)
    return NIL;
  ...
  if (size)
  {
    ...
    { /* until request satisfied */
      ...
      { /* socket case */
        ...
        if (i > 0)
          while (((i = recv(stream->tcpsi, s,
                            (int)min(maxposint, size), 0)) ==
                  SOCKET_ERROR) &&
                 ((errno = WSAGetLastError()) == WSAEINTR))
            ;
        else if (!i)
        { /* timeout, ignore if told to resume */
...
long tcp_sout(TCPSTREAM *stream, char *string, unsigned long size)
{
  ...
  tmo.tv_usec = 0;
  FD_ZERO(&fds); /* initialize selection vector */
  if (stream->tcpso == INVALID_SOCKET)
    return NIL;
...
{
  if (stream->tcpsi != stream->tcpso)
    tcp_close_socket(&stream->tcpso);
  else
    stream->tcpso = INVALID_SOCKET;
