...
static SSLSTREAM *ssl_start(TCPSTREAM *tstream, char *host, unsigned long flags)
{
  ...
  /* acquire credentials */
  if (AcquireCredentialsHandle(NIL, UNISP_NAME, SECPKG_CRED_OUTBOUND, NIL, (flags & NET_TLSCLIENT) ? &tlscred : NIL, NIL, NIL, &stream->cred, &t) != SEC_E_OK)
    ...
    { /* negotiate security context */
      ...
      obufs.pBuffers = obuf;
      /* negotiate security */
      e = InitializeSecurityContext(&stream->cred, size ? &stream->context : NIL, host, req, 0,
      ...
      if (!reason)
        ...
        case SEC_E_OK: /* success, any data to be regurgitated? */
          ...
          if (!(flags & NET_NOVALIDATECERT))
          {
            ...
            /* get certificate */
            if ((QueryContextAttributes(&stream->context, SECPKG_ATTR_REMOTE_CERT_CONTEXT, &cert) !=
                 SEC_E_OK) ||
                !cert)
            {
          ...
          if (reason ||
              (reason = ssl_analyze_status(QueryContextAttributes(&stream->context, SECPKG_ATTR_STREAM_SIZES, &stream->sizes), buf)))
...
long ssl_getdata(SSLSTREAM *stream)
{
  ...
  { /* decrypted buffer empty? */
    ...
    } while ((status = DecryptMessage(&stream->context, &msg, 0, NIL)) == SEC_E_INCOMPLETE_MESSAGE);
    switch (status)
    {
    case SEC_E_OK:          /* won */
...
long ssl_sout(SSLSTREAM *stream, char *string, unsigned long size)
{
  ...
  for (s = stream->ibuf, n = 0; size;)
  {
    ...
    if ((EncryptMessage(&stream->context, 0, &msg, NIL) != SEC_E_OK) ||
