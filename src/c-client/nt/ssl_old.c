...
static SSLSTREAM *ssl_start(TCPSTREAM *tstream, char *host, unsigned long flags)
{
  ...
  ULONG req = ISC_REQ_REPLAY_DETECT | ISC_REQ_SEQUENCE_DETECT |
              ...
              ISC_REQ_ALLOCATE_MEMORY | ISC_REQ_STREAM | ISC_REQ_EXTENDED_ERROR + ((flags & NET_NOVALIDATECERT) ? ISC_REQ_MANUAL_CRED_VALIDATION : ISC_REQ_MUTUAL_AUTH);
    ...
    { /* negotiate security context */
      ...
      if (!reason)
        ...
        case SEC_E_OK: /* success, any data to be regurgitated? */
          ...
          if (reason = ssl_analyze_status(sft->QueryContextAttributes(&stream->context, SECPKG_ATTR_STREAM_SIZES, &stream->sizes), buf))
