...
long pmatch_full(unsigned char *s, unsigned char *pat, unsigned char delim)
{
  ...
  case '%': /* non-recursive */
    ...
    if (!pat[1])
      return (delim && strchr(s, delim)) ? NIL : T;
    /* scan remainder of string until delimiter */
    do
      if (pmatch_full(s, pat + 1, delim))
        return T;
    while ((*s != delim) && *s++);
    break;
  case '*': /* match 0 or more characters */
    if (!pat[1])
      return T; /* * at end, unconditional match */
...
long dmatch(unsigned char *s, unsigned char *pat, unsigned char delim)
{
  ...
  case '%': /* non-recursive */
    if (!*s)
      return T; /* end of base means have a subset match */
    if (!*++pat)
      return NIL; /* % at end, no inferiors permitted */
    /* scan remainder of string until delimiter */
    do
      if (dmatch(s, pat, delim))
        return T;
    while ((*s != delim) && *s++);
    if (*s && !s[1])
      return T;                   /* ends with delimiter, must be subset */
  ...
  default: /* match this character */
    if (*s)
      return compare_uchar(*pat, *s) ? NIL : dmatch(s + 1, pat + 1, delim);
