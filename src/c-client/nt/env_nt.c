...
long server_login(char *user, char *pass, char *authuser, int argc, char *argv[])
{
  ...
  { /* still have available logins? */
    ...
    if (check_nt())
    { /* NT: authserver_login() call not supported */
      if (!pass)
        mm_log("Unsupported authentication mechanism", ERROR);
                ...
                LogonUser(user, ".", pass, LOGON32_LOGON_SERVICE,
                          LOGON32_PROVIDER_DEFAULT, &hdl)) &&
               ImpersonateLoggedOnUser(hdl))
        return env_init(user, NIL);
    }
    else
    { /* Win9x: done if from authserver_login() */
      if (!pass)
        server_nli = NIL;
...
char *win_homedir(char *user)
{
  ...
  /* Win9x default */
  if (!check_nt())
    sprintf(tmp, "%s\\My Documents", defaultDrive());
  ...
  /* last resort NT default */
    ...
    sprintf(tmp, "%s\\users\\default", defaultDrive());
  return cpystr(tmp);
}
...
char *mailboxdir(char *dst, char *dir, char *name)
{
  ...
  { /* if either argument provided */
    ...
    strcpy(dst, myhomedir()); /* no arguments, wants home directory */
    ...
    case '#': /* namespace names */
          ...
          ((name[4] == 'r') || (name[4] == 'R')) && (name[5] == '.'))
      {
        ...
        *dir++ = '\0'; /* tie off user name */
        ...
        dst = NIL; /* unknown namespace name */
      break;
...
char *lockdir(char *lock, char *first, char *last)
{
  ...
  char c, *s;
  if (first && *first)
  { /* first part must be non-NIL */
    /* copy first part */
    for (s = lock; c = *first++; *s++ = (c == '/') ? '\\' : c)
      ;
    if (last && *last)
    { /* copy last part if specified */
