...
void nntp_list(MAILSTREAM *stream, char *ref, char *pat)
{
    ...
    if (!*pat)
    {
        if (nntp_canonicalize(ref, "*", pattern, NIL))
        {
            ...
            if ((s = strchr(pattern, '}')) && (s = strchr(s + 1, '.')))
                *++s = '\0';
            else
                pattern[0] = '\0';
    ...
    {
        /* namespace format name? */
        if (*(lcl = strchr(strcpy(name, pattern), '}') + 1) == '#')
            lcl += 6;
            ...
            { /* tie off after newsgroup name */
                *t = '\0';
                ...
                if (pmatch_full(name, pattern, '.'))
                    ...
                    while (showuppers && (t = strrchr(lcl, '.')))
                    {
...
void nntp_lsub(MAILSTREAM *stream, char *ref, char *pat)
{
    ...
    if (nntp_canonicalize(ref, pat, mbx, NIL))
        newsrc_lsub(stream, mbx);
    if (*pat == '{')
...
long nntp_status(MAILSTREAM *stream, char *mbx, long flags)
{
    ...
    if (!(stream && LOCAL->nntpstream &&
        ...
        !(tstream = stream =
              mail_open(NIL, mbx, OP_HALFOPEN | OP_SILENT | ((flags & SA_MULNEWSRC) ? OP_MULNEWSRC : NIL))))
    ...
    if (nntp_send(LOCAL->nntpstream, "GROUP", name) == NNTPGOK)
    {
        ...
        else if (!(flags & (SA_RECENT | SA_UNSEEN)))
            status.messages = k;
                 ...
                 unsigned long rnmsgs, unsigned long nmsgs, char *tmp)
{
    ...
    if (rnmsgs > (nmsgs * 8)) /* small subrange? */
        ...
        switch ((int)nntp_send(LOCAL->nntpstream, "LISTGROUP", name))
        {
    ...
    if (EXTENSION.hdr) /* have HDR extension? */
        return (nntp_send(LOCAL->nntpstream, "HDR Date", tmp) == NNTPHEAD) ? LONGT : NIL;
...
MAILSTREAM *nntp_mopen(MAILSTREAM *stream)
{
    ...
    { /* open NNTP now if not already open */
        ...
        if (mb.secflag)
            strcat(tmp, "/secure");
        if (mb.user[0])
            sprintf(tmp + strlen(tmp), "/user=\"%s\"", mb.user);
        hostlist[1] = NIL;
        if (!(nstream = nntp_open(hostlist, NOP_READONLY |
                                                (stream->debug ? NOP_DEBUG : NIL))))
            return NIL;
    }
    ...
    else if (nntp_send(nstream, "GROUP", mbx) == NNTPGOK)
    {
        ...
        /* restrict article range if needed */
        if (nntp_range && (nmsgs > nntp_range))
    ...
    { /* want to use multiple .newsrc files? */
        ...
        *s++ = '-';            /* hyphen delimiter and host */
        lcase(strcpy(s, (long)mail_parameters(NIL, GET_NEWSRCCANONHOST, NIL) ? net_host(nstream->netstream) : mb.host));
        LOCAL->newsrc = cpystr(nq ? (*nq)(stream, tmp, newsrc) : tmp);
    }
    else
        LOCAL->newsrc = cpystr(newsrc);
    if (mb.user[0])
        LOCAL->user = cpystr(mb.user);
    stream->sequence++; /* bump sequence number */
    stream->rdonly = stream->perm_deleted = T;
    /* UIDs are always valid */
    stream->uid_validity = 0xbeefface;
    sprintf(tmp, "{%s:%lu/nntp", (long)mail_parameters(NIL, GET_TRUSTDNS, NIL) ? net_host(nstream->netstream) : mb.host,
    ...
    if (stream->rdonly)
        strcat(tmp, "/readonly");
    if (LOCAL->user)
        sprintf(tmp + strlen(tmp), "/user=\"%s\"", LOCAL->user);
    if (stream->halfopen)
        strcat(tmp, "}<no_mailbox>");
    else
        sprintf(tmp + strlen(tmp), "}#news.%s", mbx);
...
void nntp_mclose(MAILSTREAM *stream, long options)
{
    ...
    {                       /* only if a file is open */
        ...
        if (LOCAL->over_fmt)
            fs_give((void **)&LOCAL->over_fmt);
        if (LOCAL->name)
            fs_give((void **)&LOCAL->name);
        if (LOCAL->user)
...
void nntp_fetchfast(MAILSTREAM *stream, char *sequence, long flags)
{
    ...
    MESSAGECACHE *elt;
    /* get sequence */
    if (stream && LOCAL && ((flags & FT_UID) ? mail_uid_sequence(stream, sequence) : mail_sequence(stream, sequence)))
        for (i = 1; i <= stream->nmsgs; i++)
        {
                ...
                !(elt->day && elt->rfc822_size))
            {
                ...
                if (!*env || !elt->rfc822_size)
                {
                    ...
                    if (!*env)
                        rfc822_parse_msg(env, NIL, ht, hs, NIL, BADHOST,
                                         stream->dtb->flags);
    ...
    for (i = 1; i <= stream->nmsgs; i++)
        ...
        if ((elt = mail_elt(stream, i))->sequence && !elt->private.spare.ptr)
        {
            for (j = i + 1; /* no, find end of cache gap range */
                 ...
                 !elt->private.spare.ptr;
                 j++)
                ;
            /* make NNTP range */
            if (i == (j - 1))
                sprintf(tmp, "%lu", mail_uid(stream, i));
            else
                sprintf(tmp, "%lu-%lu", mail_uid(stream, i), mail_uid(stream, j - 1));
            ...
            if (nntp_over(stream, tmp))
            {
                       ...
                       strcmp(s, "."))
                {
                    ...
                    *t++ = '\0'; /* tie off string in case it was shortened */
                    ...
                    { /* shouldn't happen, snarl if it does */
                ...
                i = stream->nmsgs; /* OVER failed, punt cache load */
        }
        ...
        for (i = 1; i <= stream->nmsgs; i++)
            if ((elt = mail_elt(stream, i))->sequence)
            {
                ...
                { /* parse failed */
                    ...
                    if (!s)
                        elt->private.spare.ptr = cpystr("");
                }
                /* clean up overview data */
                if (ov.from)
                    mail_free_address(&ov.from);
                if (ov.subject)
                    fs_give((void **)&ov.subject);
            }
    return T;
}
...
long nntp_over(MAILSTREAM *stream, char *sequence)
{
    ...
    if (EXTENSION.over) /* have OVER extension? */
        return (nntp_send(LOCAL->nntpstream, "OVER", sequence) == NNTPOVER) ? LONGT : NIL;
                  ...
                  long flags)
{
    ...
    FILE *f;
    *size = 0;
    if ((flags & FT_UID) && !(msgno = mail_msgno(stream, msgno)))
        return "";
    /* have header text? */
    ...
    return elt->private.msg.header.text.data ? (char *)elt->private.msg.header.text.data : "";
}
...
FILE *nntp_article(MAILSTREAM *stream, char *msgid, unsigned long *size,
                   unsigned long *hsiz)
{
    return (nntp_send(LOCAL->nntpstream, "ARTICLE", msgid) == NNTPARTICLE) ? netmsg_slurp(LOCAL->nntpstream->netstream, size, hsiz) : NIL;
}
                     ...
                     OVERVIEW *ov)
{
    ...
    { /* message set searches */
        SEARCHSET *set;
        /* message sequences */
        if ((set = pgm->msgno) != NULL)
        { /* must be inside this sequence */
            while (set)
            { /* run down until find matching range */
                if (set->last ? ((msgno < set->first) || (msgno > set->last)) : msgno != set->first)
                    set = set->next;
                else
                    break;
            }
        ...
        { /* must be inside this sequence */
            ...
            while (set)
            { /* run down until find matching range */
                if (set->last ? ((uid < set->first) || (uid > set->last)) : uid != set->first)
                    set = set->next;
                else
                    break;
            }
    ...
    { /* only do this if real searching */
        ...
        if ((pgm->larger && (ov->optional.octets <= pgm->larger)) ||
            (pgm->smaller && (ov->optional.octets >= pgm->smaller)))
            return NIL;
        /* date ranges */
    ...
    for (or = pgm->or ; or ; or = or->next)
        if (!(nntp_search_msg(stream, msgno, or->first, ov) ||
              nntp_search_msg(stream, msgno, or->second, ov)))
            return NIL;
    for (not = pgm->not; not; not = not->next)
        if (nntp_search_msg(stream, msgno, not->pgm, ov))
            return NIL;
    return T;
}
                                ...
                                long flags)
{
    ...
    { /* messages need to be loaded in sortcache? */
        /* yes, build range */
        if (start != last)
            sprintf(tmp, "%lu-%lu", start, last);
        else
            sprintf(tmp, "%lu", start);
                ...
                *v++ = '\0'; /* tie off subject */
                ...
                if ((t = strchr(v, '\t')) != NULL)
                {
                    *t++ = '\0'; /* tie off from */
                    ...
                    {
                        *v++ = '\0'; /* tie off date */
                        if (mail_parse_date(&telt, t))
                            r->date = mail_longdate(&telt);
...
void nntp_check(MAILSTREAM *stream)
{
    ...
    if (LOCAL->dirty)
        newsrc_write(LOCAL->name, stream);
    LOCAL->dirty = NIL;
}
...
long nntp_copy(MAILSTREAM *stream, char *sequence, char *mailbox, long options)
{
    ...
    if (pc)
        return (*pc)(stream, sequence, mailbox, options);
                           ...
                           unsigned long port, long options)
{
    ...
    if (!(hostlist && *hostlist))
        ...
        { /* try to open connection */
            ...
            { /* light tryssl flag if requested */
                mb.trysslflag = (options & NOP_TRYSSL) ? T : NIL;
                /* default port */
                if (mb.port)
                    port = mb.port;
                else if (!port)
                    port = nntp_port ? nntp_port : NNTPTCPPORT;
                              ...
                              "*nntps", nntp_sslport ? nntp_sslport : NNTPSSLPORT)) != NULL)
                {
                    ...
                    stream->host = cpystr((long)mail_parameters(NIL, GET_TRUSTDNS, NIL) ? net_host(netstream) : mb.host);
    ...
    if (stream && !dv && stls && NNTP.ext.starttls &&
        ...
        (nntp_send_work(stream, "STARTTLS", NNTP.ext.multidomain ? mb.host : NIL) == NNTPTLSSTART))
    {
    ...
    { /* have a session? */
        ...
        { /* yes, have user name? */
            if ((long)mail_parameters(NIL, GET_TRUSTDNS, NIL))
            {
                ...
                strncpy(mb.host, (long)mail_parameters(NIL, GET_SASLUSESPTRNAME, NIL) ? net_remotehost(netstream) : net_host(netstream),
                        NETMAXHOST - 1);
        ...
        /* authenticate if no-post and not readonly */
                   ...
                   nntp_send_auth(stream, NIL)))
            stream = nntp_close(stream);
    }

    /* in case server demands MODE READER */
    if (stream)
        switch ((int)nntp_send_work(stream, "MODE", "READER"))
        {
        case NNTPGREET:
            NNTP.post = T;
            break;
        ...
        case NNTPWANTAUTH2: /* remote name for authentication */
            if ((long)mail_parameters(NIL, GET_TRUSTDNS, NIL))
            {
                strncpy(mb.host, (long)mail_parameters(NIL, GET_SASLUSESPTRNAME, NIL) ? net_remotehost(netstream) : net_host(netstream), NETMAXHOST - 1);
                mb.host[NETMAXHOST - 1] = '\0';
            }
            if (nntp_send_auth_work(stream, &mb, tmp, NIL))
    ...
    { /* looks like we have a stream? */
        ...
        if (!(NNTP.post || (options & NOP_READONLY)))
            stream = nntp_close(stream);
        else if (extok)
...
long nntp_extensions(SENDSTREAM *stream, long flags)
{
    ...
    if (stream->loser)
        return NIL; /* nothing at all for losers */
    ...
    while ((t = net_getline(stream->netstream)) && (t[1] || (*t != '.')))
    {
        ...
        if ((args = strchr(t, ' ')) != NULL)
            *args++ = '\0';
        if (!compare_cstring(t, "LISTGROUP"))
            NNTP.ext.listgroup = T;
        else if (!compare_cstring(t, "OVER"))
            NNTP.ext.over = T;
        else if (!compare_cstring(t, "HDR"))
            NNTP.ext.hdr = T;
        else if (!compare_cstring(t, "PAT"))
            NNTP.ext.pat = T;
        else if (!compare_cstring(t, "STARTTLS"))
            NNTP.ext.starttls = T;
        else if (!compare_cstring(t, "MULTIDOMAIN"))
            NNTP.ext.multidomain = T;
...
long nntp_mail(SENDSTREAM *stream, ENVELOPE *env, BODY *body)
{
   ...
   */
    ...
    sprintf(path, "Path: %s!%s\015\012", net_localhost(stream->netstream),
            env->sender ? env->sender->mailbox : (env->from ? env->from->mailbox : "not-for-mail"));
    /* here's another cretinism */
    if ((s = strstr(env->date, " (")) != NULL)
        *s = NIL;
    do
        if ((ret = nntp_send_work(stream, "POST", NIL)) == NNTPREADY)
    ...
    if (s)
        *s = ' '; /* put the comment in the date back */
    if (ret == NNTPOK)
        return LONGT;
    else if (ret < 400)
    { /* if not an error reply */
...
long nntp_send(SENDSTREAM *stream, char *command, char *args)
{
    ...
    case NNTPWANTAUTH2:
        ...
        { /* we're probably hosed, nuke the session */
            nntp_send(stream, "QUIT", NIL);
...
{
    long ret;
    char *s = (char *)fs_get(strlen(command) + (args ? strlen(args) + 1 : 0) + 3);
    if (!stream->netstream)
        ret = nntp_fake(stream, "NNTP connection lost");
    else
    { /* build the complete command */
        if (args)
            sprintf(s, "%s %s", command, args);
        else
            strcpy(s, command);
        if (stream->debug)
            mail_dlog(s, stream->sensitive);
        strcat(s, "\015\012");
        /* send the command */
        ret = net_soutr(stream->netstream, s) ? nntp_reply(stream) : nntp_fake(stream, "NNTP connection broken (command)");
    }
    fs_give((void **)&s);
    return ret;
}
...
long nntp_send_auth(SENDSTREAM *stream, long flags)
{
    ...
    char tmp[MAILTMPLEN];
    /* remote name for authentication */
    sprintf(tmp, "{%.200s/nntp", (long)mail_parameters(NIL, GET_TRUSTDNS, NIL) ? ((long)mail_parameters(NIL, GET_SASLUSESPTRNAME, NIL) ? net_remotehost(stream->netstream) : net_host(stream->netstream)) : stream->host);
...
long nntp_send_auth_work(SENDSTREAM *stream, NETMBX *mb, char *pwd, long flags)
{
            ...
            do
            {
                ...
                if (nntp_send(stream, "AUTHINFO SASL", at->name) == NNTPCHALLENGE)
                {
                    ...
                    if ((*at->client)(nntp_challenge, nntp_response, "nntp", mb, stream,
                                      &trial, usr))
                    {
                        if (stream->replycode == NNTPAUTHED)
                            ret = LONGT;
   ...
   */
             ...
             stream->netstream;)
        {
            ...
            if (pwd[0])
                switch ((int)nntp_send_work(stream, "AUTHINFO USER", usr))
                {
                case NNTPBADCMD: /* give up if unrecognized command */
                    mm_log(NNTP.ext.authuser ? stream->reply : "Can't do AUTHINFO USER to this server", ERROR);
                    trial = nntp_maxlogintrials;
                    break;
...
long nntp_reply(SENDSTREAM *stream)
{
    ...
    if (stream->debug)
        mm_dlog(stream->reply);
    /* handle continuation by recursion */
    if (stream->reply[3] == '-')
        return nntp_reply(stream);
