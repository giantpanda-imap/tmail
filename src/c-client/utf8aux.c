...
static void utf8_stringlist(STRINGLIST *st, char *charset)
{
    ...
    if (st)
        do
            if (utf8_text(&st->text, charset, &txt, U8T_CANONICAL))
            {
...
void utf8_searchpgm(SEARCHPGM *pgm, char *charset)
{
    ...
    { /* must have a search program */
        ...
        for (pl = pgm->not; pl; pl = pl->next)
            utf8_searchpgm(pl->pgm, charset);
...
long utf8_mime2text(SIZEDTEXT *src, SIZEDTEXT *dst, long flags)
{
    ...
    dst->data = (flags & U8T_CANONICAL) ? (unsigned char *)fs_get((size_t)dsize) : NIL;
            ...
            (te = mime2_text(t = e + 2, se)) && (ee == e + 1))
        {
            if (mime2_decode(e, t, te, &txt))
            {
                *ce = '\0'; /* temporarily tie off charset */
                if ((ls = strchr(cs, '*')) != NULL)
                    *ls = '\0';
                /* convert to UTF-8 as best we can */
                if (!utf8_text(&txt, cs, &rtxt, flags))
                    utf8_text(&txt, NIL, &rtxt, flags);
                if (dst->data)
                { /* make sure existing buffer fits */
                ...
                { /* make a new buffer */
                    while (dsize <= (dst->size + rtxt.size))
                        dsize += DSIZE;
                    memcpy(dst->data = (unsigned char *)fs_get(dsize), src->data,
                           dst->size = s - src->data);
                }
                for (i = 0; i < rtxt.size; i++)
                    dst->data[dst->size++] = rtxt.data[i];

                /* all done with converted text */
                if (rtxt.data != txt.data)
                    fs_give((void **)&rtxt.data);
                if (ls)
                    *ls = '*'; /* restore language tag delimiter */
            ...
            do
                if (dst->data)
                { /* stash ASCII characters until LWSP */
                    ...
                    dst->data[dst->size++] = (flags & U8T_CASECANON) ? (unsigned char)ucs4_titlecase(*s) : *s;
                }
    ...
    { /* nothing converted, return identity */
        dst->data = src->data;
        dst->size = src->size;
    }
    return T; /* success */
                  ...
                  SIZEDTEXT *txt)
{
    ...
    switch (*e)
    { /* dispatch based upon encoding */
    case 'Q':
    case 'q': /* sort-of QUOTED-PRINTABLE */
        txt->data = (unsigned char *)fs_get((size_t)(te - t) + 1);
        for (q = t, txt->size = 0; q < te; q++)
            switch (*q)
            {
                           ...
                           unsigned char **t)
{
    for (*t = s; **t != '?'; ++*t)
    {
        if ((*t < se) && isgraph(**t))
            ...
            return NIL; /* out of text or CTL or space */
    }
    return s;
}
...
unsigned char *utf8_to_mutf7(unsigned char *src)
{
    ...
    int nonascii = 0;
    while (*s)
    { /* pass one: count destination octets */
        if (*s & 0x80)
        {                   /* non-ASCII character? */
            j = MAXUNIUTF8; /* get single UCS-4 codepoint */
            if ((c = utf8_get(&s, &j)) & U8G_ERROR)
                return NIL;
        ...
        { /* ASCII character */
            ...
            if (*s == '&')
                i += 2; /* two octets if the escape */
            else
                ++i; /* otherwise just count another octet */
    ...
    for (s = src; *s;)
    { /* pass two: copy destination octets */
        if (*s & 0x80)
        {                   /* non-ASCII character? */
            j = MAXUNIUTF8; /* get single UCS-4 codepoint */
            if ((c = utf8_get(&s, &j)) & U8G_ERROR)
        ...
        { /* ASCII character */
            ...
            *t++ = *s; /* copy the character */
            if (*s == '&')
                *t++ = '-'; /* special sequence if the escape */
            ++s;            /* advance to next source octet */
        }
    }
    /* add pending Modified BASE64 size + shifts */
