...
DRIVER *pop3_valid(char *name)
{
    ...
    return (mail_valid_net_parse(name, &mb) &&
            ...
            !compare_cstring(mb.mailbox, "INBOX"))
               ? &pop3driver
               : NIL;
}
...
long pop3_status(MAILSTREAM *stream, char *mbx, long flags)
{
    ...
    long ret = NIL;
    MAILSTREAM *tstream =
        (stream && LOCAL->netstream && mail_usable_network_stream(stream, mbx)) ? stream : mail_open(NIL, mbx, OP_SILENT);
    if (tstream)
...
MAILSTREAM *pop3_open(MAILSTREAM *stream)
{
        ...
        pop3_reply(stream))
    {
        ...
        if (!pop3_auth(stream, &mb, tmp, usr))
        ...
        else if (pop3_send(stream, "STAT", NIL))
        {
            ...
            sprintf(tmp, "{%.200s:%lu/pop3",
                    (long)mail_parameters(NIL, GET_TRUSTDNS, NIL) ? net_host(LOCAL->netstream) : mb.host,
            ...
            if (!LOCAL->loser && LOCAL->cap.capa && pop3_send(stream, "LIST", NIL))
            {
                while ((s = net_getline(LOCAL->netstream)) && (*s != '.'))
                {
                    if ((i = strtoul(s, &t, 10)) && (i <= stream->nmsgs) &&
                        (j = strtoul(t, NIL, 10)))
                        mail_elt(stream, i)->rfc822_size = j;
                    fs_give((void **)&s);
                }
                /* flush final dot */
                if (s)
...
long pop3_capa(MAILSTREAM *stream, long flags)
{
    ...
    while ((t = net_getline(LOCAL->netstream)) && (t[1] || (*t != '.')))
    {
        ...
        if ((args = strchr(t, ' ')) != NULL)
            *args++ = '\0';
        if (!compare_cstring(t, "STLS"))
            LOCAL->cap.stls = T;
        else if (!compare_cstring(t, "PIPELINING"))
            LOCAL->cap.pipelining = T;
        else if (!compare_cstring(t, "RESP-CODES"))
            LOCAL->cap.respcodes = T;
        else if (!compare_cstring(t, "TOP"))
            LOCAL->cap.top = T;
        else if (!compare_cstring(t, "UIDL"))
            LOCAL->cap.uidl = T;
        else if (!compare_cstring(t, "USER"))
            LOCAL->cap.user = T;
        ...
        else if (!compare_cstring(t, "EXPIRE") && args)
        {
            ...
            LOCAL->cap.expire = /* get expiration time */
                (!compare_cstring(args, "NEVER")) ? 65535 : ((s && !compare_cstring(s, "USER")) ? -atoi(args) : atoi(args));
        }
        else if (!compare_cstring(t, "LOGIN-DELAY") && args)
        {
            ...
            /* get delay time */
            LOCAL->cap.delaysecs = (s && !compare_cstring(s, "USER")) ? -atoi(args) : atoi(args);
        }
...
long pop3_auth(MAILSTREAM *stream, NETMBX *mb, char *pwd, char *usr)
{
    ...
    { /* got any authenticators? */
        if ((long)mail_parameters(NIL, GET_TRUSTDNS, NIL))
        {
            ...
            strncpy(mb->host, (long)mail_parameters(NIL, GET_SASLUSESPTRNAME, NIL) ? net_remotehost(LOCAL->netstream) : net_host(LOCAL->netstream),
                    NETMAXHOST - 1);
    ...
    {              /* traditional login */
        ...
        do
        {
            ...
            if (pwd[0])
            { /* send login sequence if have password */
                if (pop3_send(stream, "USER", usr))
                {
                    LOCAL->sensitive = T; /* hide this command */
                    if (pop3_send(stream, "PASS", pwd))
                        ret = LONGT;
                    LOCAL->sensitive = NIL; /* unhide */
                }
                if (!ret)
    ...
    { /* make CRLFless BASE64 string */
        if (size)
        {
            ...
            *u = '\0';
            ...
            ret = net_sout(LOCAL->netstream, "\015\012", 2);
    }
    else
    { /* abort requested */
...
long pop3_text(MAILSTREAM *stream, unsigned long msgno, STRING *bs, long flags)
{
    ...
    if (!LOCAL->txt)
        return NIL; /* error if don't have a file */
    if (!(flags & FT_PEEK))
    { /* mark seen if needed */
        elt->seen = T;
...
long pop3_expunge(MAILSTREAM *stream, char *sequence, long options)
{
    ...
    { /* build selected sequence if needed */
...
long pop3_send(MAILSTREAM *stream, char *command, char *args)
{
    ...
    if (!LOCAL->netstream)
        ret = pop3_fake(stream, "POP3 connection lost");
    else
    { /* build the complete command */
        if (args)
            ...
            strcpy(s, command);
        if (stream->debug)
            mail_dlog(s, LOCAL->sensitive);
        strcat(s, "\015\012");
        /* send the command */
        ret = net_soutr(LOCAL->netstream, s) ? pop3_reply(stream) : pop3_fake(stream, "POP3 connection broken in command");
    }
    fs_give((void **)&s);
