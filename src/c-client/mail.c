...
void mail_link(DRIVER *driver)
{
    ...
    while (*d)
        d = &(*d)->next; /* find end of list of drivers */
...
void *mail_parameters(MAILSTREAM *stream, long function, void *value)
{
    ...
    case SET_NEWSRC: /* too late on open stream */
        if (stream && stream->dtb && (stream != ((*stream->dtb->open)(NIL))))
            ...
            ret = env_parameters(function, value);
        break;
    case GET_NAMESPACE:
    ...
    case SET_EXTERNALAUTHID:
        if (value)
        { /* setting external authentication ID */
            externalauthid = cpystr((char *)value);
            mail_parameters(NIL, UNHIDE_AUTHENTICATOR, "EXTERNAL");
        }
        else
        { /* clearing external authentication ID */
            if (externalauthid)
                fs_give((void **)&externalauthid);
    ...
    case SET_SNARFMAILBOXNAME:
        if (stream)
        { /* have a stream? */
            if (stream->snarf.name)
                fs_give((void **)&stream->snarf.name);
            stream->snarf.name = cpystr((char *)value);
        }
        else
            fatal("SET_SNARFMAILBOXNAME with no stream");
    case GET_SNARFMAILBOXNAME:
        if (stream)
            ret = (void *)stream->snarf.name;
        break;
    ...
    case GET_IDPARAMS:
        ret = (void *)idapp;
    default:
        if ((r = smtp_parameters(function, value)) != NULL)
            ret = r;
        if ((r = env_parameters(function, value)) != NULL)
            ret = r;
        if ((r = tcp_parameters(function, value)) != NULL)
            ret = r;
        if ((r = utf8_parameters(function, value)) != NULL)
            ret = r;
        if (stream && stream->dtb)
        { /* if have stream, do for its driver only */
            if ((r = (*stream->dtb->parameters)(function, value)) != NULL)
                ret = r;
        }
        /* else do all drivers */
        else
            for (d = maildrivers; d; d = d->next)
                if ((r = (d->parameters)(function, value)) != NULL)
                    ret = r;
        break;
    }
    return ret;
...
DRIVER *mail_valid(MAILSTREAM *stream, char *mailbox, char *purpose)
{
    ...
    if (!factory && purpose)
    { /* if want an error message */
        sprintf(tmp, "Can't %s %.80s: %s", purpose, mailbox, (*mailbox == '{') ? "invalid remote specification" : "no such mailbox");
        MM_LOG(tmp, ERROR);
    }
...
long mail_valid_net_parse_work(char *name, NETMBX *mb, char *service)
{
    ...
    if (*name++ != '{')
        return NIL;
    if (*name == '[')
    { /* if domain literal, find its ending */
        if (!((v = strpbrk(name, "]}")) && (*v++ == ']')))
            return NIL;
    }
    /* find end of host name */
    else if (!(v = strpbrk(name, "/:}")))
        return NIL;
    ...
    {                           /* any switches or port specification? */
        ...
        do
            switch (c)
            {         /* act based upon the character */
            case ':': /* port specification */
                if (mb->port || !(mb->port = strtoul(t, &t, 10)))
                    return NIL;
            ...
            case '/': /* switch */
                    ...
                    { /* quoted string? */
                        for (v = arg, i = 0, ++t; (c = *t++) != '"';)
                        {
                            ...
                            if (c == '\\')
                                c = *t++;
                            if (!c)
                                return NIL; /* can't quote NUL either */
                            arg[i++] = c;
                        }
                        c = *t++;      /* remember delimiter for later */
                ...
                { /* non-argument switch */
                    if (!compare_cstring(s, "anonymous"))
                        mb->anoflag = T;
                    else if (!compare_cstring(s, "debug"))
                        mb->dbgflag = T;
                    else if (!compare_cstring(s, "readonly"))
                        mb->readonlyflag = T;
                    else if (!compare_cstring(s, "secure"))
                        mb->secflag = T;
                    else if (!compare_cstring(s, "norsh"))
                        mb->norsh = T;
                    else if (!compare_cstring(s, "loser"))
                        mb->loser = T;
                    ...
                    else if (!compare_cstring(s, "tls1") && !mb->tls1_1 && !mb->tls1_2 && !mb->tls1_3 && !mb->dtls1 && !mb->dtls1_2)
                        mb->sslflag = mb->notlsflag = mb->tls1 = T;
                    else if (!compare_cstring(s, "tls1_1") && !mb->tls1 && !mb->tls1_2 && !mb->tls1_3 && !mb->dtls1 && !mb->dtls1_2)
                        mb->sslflag = mb->notlsflag = mb->tls1_1 = T;
                    else if (!compare_cstring(s, "tls1_2") && !mb->tls1 && !mb->tls1_1 && !mb->tls1_3 && !mb->dtls1 && !mb->dtls1_2)
                        mb->sslflag = mb->notlsflag = mb->tls1_2 = T;
                    else if (!compare_cstring(s, "tls1_3") && !mb->tls1 && !mb->tls1_1 && !mb->tls1_2 && !mb->dtls1 && !mb->dtls1_2)
                        mb->sslflag = mb->notlsflag = mb->tls1_3 = T;
                    else if (!compare_cstring(s, "dtls1") && !mb->tls1 && !mb->tls1_1 && !mb->tls1_2 && !mb->tls1_3 && !mb->dtls1_2)
                        mb->sslflag = mb->notlsflag = mb->dtls1 = T;
                    else if (!compare_cstring(s, "dtls1_2") && !mb->tls1 && !mb->tls1_1 && !mb->tls1_2 && !mb->tls1_3 && !mb->dtls1)
                    ...
                    /* service switches below here */
                        ...
                        return NIL; /* invalid non-argument switch */
                }
                break;
    ...
    if (!*mb->service)
        strcpy(mb->service, service);
    /* /norsh only valid if imap */
    if (mb->norsh && strcmp(mb->service, "imap"))
        return NIL;
    return T;
}
...
void mail_scan(MAILSTREAM *stream, char *ref, char *pat, char *contents)
{
    ...
    if (*pat == '{')
        ref = NIL; /* ignore reference if pattern is remote */
    if (stream)
...
long mail_subscribe(MAILSTREAM *stream, char *mailbox)
{
    ...
    return factory ? (factory->subscribe ? (*factory->subscribe)(stream, mailbox) : sm_subscribe(mailbox)) : NIL;
}
...
long mail_unsubscribe(MAILSTREAM *stream, char *mailbox)
{
    ...
    return (factory && factory->unsubscribe) ? (*factory->unsubscribe)(stream, mailbox) : sm_unsubscribe(mailbox);
}
...
long mail_create(MAILSTREAM *stream, char *mailbox)
{
    ...
    if (strlen(mailbox) >= (NETMAXHOST + (NETMAXUSER * 2) + NETMAXMBX + NETMAXSRV + 50))
    {
        sprintf(tmp, "Can't create %.80s: %s", mailbox, (*mailbox == '{') ? "invalid remote specification" : "no such mailbox");
        MM_LOG(tmp, ERROR);
        return NIL;
    }
        ...
        ((mailbox[6] == 'r') || (mailbox[6] == 'R')) && (mailbox[7] == '.'))
    {
        ...
        if (d)
            mailbox = ++s; /* skip past driver specification */
        else
        {
    ...
    /* use stream if one given or deterministic */
        ...
        d = stream->dtb;
    else if (mailbox[0] == '#' && (mailbox[1] == 'm' || mailbox[1] == 'M') && (mailbox[2] == 'd' || mailbox[2] == 'D' || mailbox[2] == 'c' || mailbox[2] == 'C') && mailbox[3] == '/' && mailbox[4] != '\0')
        return (*md->create)(stream, mailbox);
    else if ((*mailbox != '{') && (ts = default_proto(NIL)))
        d = ts->dtb;
    else
    { /* failed utterly */
...
char *mail_utf7_valid(char *mailbox)
{
    ...
    { /* make sure valid name */
        ...
        if (*s & 0x80)
            return "mailbox name with 8-bit octet";
        /* validate modified UTF-7 */
            ...
            while (*++s != '-')
                ...
                default:   /* must be alphanumeric */
                    if (!isalnum(*s))
                        return "invalid modified UTF-7 name";
                    break;
                }
    }
    return NIL; /* all OK */
...
MAILSTREAM *mail_open(MAILSTREAM *stream, char *name, long options)
{
    ...
    case '#': /* possible special hacks */
                 ...
                 !strcmp(mb.service, "pop3") && !mb.anoflag && !mb.readonlyflag)
        {
            if ((stream = mail_open(stream, mb.mailbox, options)) != NULL)
            {
                ...
                if (mb.port)
                    sprintf(tmp + strlen(tmp), ":%lu", mb.port);
                if (mb.user[0])
                    sprintf(tmp + strlen(tmp), "/user=%.64s", mb.user);
                if (mb.dbgflag)
                    strcat(tmp, "/debug");
                if (mb.secflag)
                    strcat(tmp, "/secure");
    ...
    default: /* not special hack (but could be # name */
        d = mail_valid(NIL, name, (options & OP_SILENT) ? (char *)NIL : "open mailbox");
    }
                           ...
                           long options)
{
   ...
   */
    ...
    { /* recycling requested? */
        ...
        { /* stream not recycleable, babble if net */
...
void mail_free_idlist(IDLIST **idlist)
{
    ...
    {
        if ((*idlist)->name)
            fs_give((void **)&(*idlist)->name);
        if ((*idlist)->value)
            fs_give((void **)&(*idlist)->value);
        if ((*idlist)->next)
            mail_free_idlist(&(*idlist)->next);
        fs_give((void **)idlist);
    }
}
                               ...
                               BODY **body, long flags)
{
    ...
    if (flags & FT_UID)
    { /* UID form of call */
        if ((msgno = mail_msgno(stream, msgno)) != 0L)
            flags &= ~FT_UID;
        else
            return NIL; /* must get UID/msgno map first */
    }
    elt = mail_elt(stream, msgno); /* get elt for real message number */
    if (stream->scache)
    { /* short caching */
        if (msgno != stream->msgno)
        { /* garbage collect if not same message */
    ...
    { /* get pointers to elt envelope and body */
    ...
    if (stream->dtb && ((body && !*b) || !*env || (*env)->incomplete))
    {
        ...
        if (body || !elt->rfc822_size)
        {
            ...
            if (!elt->rfc822_size)
                elt->rfc822_size = hdrsize + SIZE(&bs);
        ...
        { /* can save memory doing it this way */
...
static void markseen(MAILSTREAM *stream, MESSAGECACHE *elt, long flags)
{
    ...
    {
        if (stream->dtb->flagmsg)
        {                     /* driver wants per-message call? */
                         ...
                         unsigned long *len, long flags)
{
    ...
    unsigned long i, j;
    if (len)
        *len = 0; /* default return size */
    if (flags & FT_UID)
    { /* UID form of call */
        if ((msgno = mail_msgno(stream, msgno)) != 0L)
            flags &= ~FT_UID;
        else
            return ""; /* must get UID/msgno map first */
    }
    ...
    if (!stream->dtb)
        return ""; /* not in cache, must have live driver */
    if (stream->dtb->msgdata)
        return ((*stream->dtb->msgdata)(stream, msgno, "", 0, 0, NIL, flags) && t->data) ? mail_fetch_text_return(&md, t, len) : "";
                        ...
                        STRINGLIST *lines, unsigned long *len, long flags)
{
    ...
    if (len)
        *len = 0; /* default return size */
    if (section && (strlen(section) > (MAILTMPLEN - 20)))
        return "";
    if (flags & FT_UID)
    { /* UID form of call */
        if ((msgno = mail_msgno(stream, msgno)) != 0L)
    ...
    {
        if (lines)
            textcpy(t = &stream->text, &m->header.text);
        else
            t = &m->header.text;      /* in cache, and cache is valid */
        markseen(stream, elt, flags); /* mark message seen */
    }

    else if (stream->dtb)
    { /* not in cache, has live driver? */
        if (stream->dtb->msgdata)
        { /* has driver section fetch? */
        ...
        { /* nested body wanted? */
            ...
            else if ((*stream->dtb->text)(stream, msgno, &bs, flags & ~FT_INTERNAL))
            {
                if ((bs.dtb->next == mail_string_next) && !lines)
                {
                    ...
                    textcpyoffstring(t = &stream->text, &bs,
                                     b->nested.msg->header.offset,
                                     b->nested.msg->header.text.size);
            }
        }
        else
        { /* top-level header fetch */
            /* mark message seen */
            markseen(stream, elt, flags);
            if ((rt.data = (unsigned char *)(*stream->dtb->header)(stream, msgno, &rt.size, flags)) != NULL)
            {
                /* make a safe copy if need to filter */
                if (lines)
                    textcpy(t = &stream->text, &rt);
                else
                    t = &rt; /* top level header */
            }
        }
    }
    if (!t || !t->data)
        return ""; /* error if no string */
    /* filter headers if requested */
    if (lines)
        t->size = mail_filter((char *)t->data, t->size, lines, flags);
    if (len)
        *len = t->size;     /* return size if requested */
                      ...
                      unsigned long *len, long flags)
{
    ...
    if (!stream->dtb)
        return ""; /* not in cache, must have live driver */
    if (stream->dtb->msgdata)
        return ((*stream->dtb->msgdata)(stream, msgno, tmp, 0, 0, NIL, flags) && p->text.data) ? mail_fetch_text_return(&md, &p->text, len) : "";
    if (!(*stream->dtb->text)(stream, msgno, &bs, flags))
        return "";
    if (section && *section)
    { /* nested is more complex */
        SETPOS(&bs, p->offset);
        i = p->text.size; /* just want this much */
    }
    else
        i = SIZE(&bs); /* want entire text */
                      ...
                      unsigned long *len, long flags)
{
    ...
    if (len)
        *len = b->mime.text.size;
    if (!b->mime.text.size)
    { /* empty MIME header -- mark seen anyway */
        markseen(stream, mail_elt(stream, msgno), flags);
        return "";
    }
                      ...
                      unsigned long *len, long flags)
{
    ...
    if (!stream->dtb)
        return ""; /* not in cache, must have live driver */
    if (stream->dtb->msgdata)
        return ((*stream->dtb->msgdata)(stream, msgno, section, 0, 0, NIL, flags) && t->data) ? mail_fetch_text_return(&md, t, len) : "";
    if (len)
        *len = t->size;
    if (!t->size)
    { /* empty body part -- mark seen anyway */
        markseen(stream, mail_elt(stream, msgno), flags);
        return "";
    }
                       ...
                       unsigned long first, unsigned long last, long flags)
{
    ...
    unsigned long i;
    if (!mailgets)
        fatal("mail_partial_text() called without a mailgets!");
    if (section && (strlen(section) > (MAILTMPLEN - 20)))
        return NIL;
    if (flags & FT_UID)
    { /* UID form of call */
    ...
    { /* else get data from driver */
        ...
        if (!(*stream->dtb->text)(stream, msgno, &bs, flags))
            return NIL;
        if (section && *section)
        {                           /* nexted if more complex */
            SETPOS(&bs, p->offset); /* offset stringstruct to data */
            i = p->text.size;       /* maximum size of data */
        }
        else
            i = SIZE(&bs); /* just want this much */
    }
    if (i <= first)
        i = first = 0; /* first byte is beyond end of text */
    /* truncate as needed */
    else
    { /* offset and truncate */
                       ...
                       unsigned long first, unsigned long last, long flags)
{
    ...
    if (!mailgets)
        fatal("mail_partial_body() called without a mailgets!");
    if (flags & FT_UID)
    { /* UID form of call */
        if ((msgno = mail_msgno(stream, msgno)) != 0L)
            flags &= ~FT_UID;
                               ...
                               unsigned long *len, long flags)
{
    ...
    /* have to do the mailgets thing? */
        ...
        ret = (*mailgets)(mail_read, bs, i, md);
    /* special hack to avoid extra copy */
    else if (bs->dtb->next == mail_string_next)
        ret = bs->curpos;
    /* make string copy in memory */
    else
        ret = textcpyoffstring(&md->stream->text, bs, GETPOS(bs), i);
    return ret;
}
...
unsigned long mail_uid(MAILSTREAM *stream, unsigned long msgno)
{
    ...
    return uid ? uid : (stream->dtb && stream->dtb->uid) ? (*stream->dtb->uid)(stream, msgno) : 0;
}
...
unsigned long mail_msgno(MAILSTREAM *stream, unsigned long uid)
{
    ...
    {                           /* active stream? */
       ...
       */
            for (msgno = 1; msgno <= stream->nmsgs; msgno++)
                if ((*stream->dtb->uid)(stream, msgno) == uid)
                    return msgno;
        }
        /* binary search since have full map */
        else
            for (first = 1, last = stream->nmsgs, delta = (first <= last) ? 1 : 0;
                 delta &&
                 ...
                 (uid <= (lastuid = mail_elt(stream, last)->private.uid));)
            {
                ...
                {
                    if ((miduid = mail_elt(stream, middle = first + delta)->private.uid) == uid)
    ...
    { /* dead stream, do linear search for UID */
        for (msgno = 1; msgno <= stream->nmsgs; msgno++)
            if (mail_elt(stream, msgno)->private.uid == uid)
                return msgno;
    }
                       ...
                       long length)
{
    ...
    if (env && env->subject)
        strncpy(s, env->subject, (size_t)length);
    else
        *s = ' '; /* if no subject then just a space */
}
...
void mail_flag(MAILSTREAM *stream, char *sequence, char *flag, long flags)
{
    ...
    short nf;
    if (!stream->dtb)
        return; /* no-op if no stream */
    if ((stream->dtb->flagmsg || !stream->dtb->flag) &&
        ((flags & ST_UID) ? mail_uid_sequence(stream, sequence) : mail_sequence(stream, sequence)) &&
    ...
    /* call driver once */
    if (stream->dtb->flag)
        (*stream->dtb->flag)(stream, sequence, flag, flags);
}
                      ...
                      long flags)
{
    ...
    if (pgm && stream->dtb) /* must have a search program and driver */
        ret = (*(stream->dtb->search ? stream->dtb->search : mail_search_default))(stream, charset, pgm, flags);
                         ...
                         long flags)
{
    ...
    for (i = 1; i <= stream->nmsgs; ++i)
        ...
        {
            if (flags & SE_UID)
                mm_searched(stream, mail_uid(stream, i));
            else
            { /* mark as searched, notify mail program */
            ...
            for (i = 1; ret && (i <= n); i++) /* for each message */
                    ...
                    (s = mail_fetch_message(snarf, i, &len, FT_PEEK)) && len)
                {
                    ...
                    if (mailsnarfpreserve)
                    {
                        ...
                        ret = mail_append(stream, stream->mailbox, &bs);
                    ...
                    { /* did snarf succeed? */
                        ...
                        {
                            elt->valid = NIL; /* prepare for flag alteration */
                            if (snarf->dtb->flagmsg)
                                (*snarf->dtb->flagmsg)(snarf, elt);
                            /* flags now altered */
...
long mail_copy_full(MAILSTREAM *stream, char *sequence, char *mailbox,
                    long options)
{
    return stream->dtb ? SAFE_COPY(stream->dtb, stream, sequence, mailbox, options) : NIL;
}
...
void mail_gc(MAILSTREAM *stream, long gcflags)
{
    ...
    if (stream->dtb && stream->dtb->gc)
        (*stream->dtb->gc)(stream, gcflags);
...
void mail_gc_msg(MESSAGE *msg, long gcflags)
{
    ...
    { /* garbage collect texts */
        if (msg->full.text.data)
            fs_give((void **)&msg->full.text.data);
        if (msg->header.text.data)
        {
            mail_free_stringlist(&msg->lines);
            fs_give((void **)&msg->header.text.data);
        }
        if (msg->text.text.data)
            fs_give((void **)&msg->text.text.data);
        /* now GC all body components */
        if (msg->body)
            mail_gc_body(msg->body);
    }
}
...
long mail_parse_date(MESSAGECACHE *elt, unsigned char *s)
{
    ...
    case '-':
        ...
        {            /* first three were OK, possibly full name */
    ...
    if ((d > 31) || (m > 12))
        return NIL;
    if (y < 49)
        y += 2000; /* RFC 2282 rules for two digit years 00-49 */
    else if (y < 999)
        y += 1900; /* 2-digit years 50-99 and 3-digit years */
    /* reject prehistoric and far future years */
    if ((y < BASEYEAR) || (y > maxyear))
        return NIL;
    /* set values in elt */
    elt->day = d;
    elt->month = m;
...
void mail_expunged(MAILSTREAM *stream, unsigned long msgno)
{
    ...
    else
    {
        ...
        if (stream->msgno)
        { /* have stream pointers? */
            /* make sure the short cache is nuked */
            if (stream->scache)
                mail_gc(stream, GC_ENV | GC_TEXTS);
            else
                stream->msgno = 0; /* make sure invalidated in any case */
        }
    }
}
...
{
    if (!stream->lock)
        fatal("Unlock when not locked");
    else
        stream->lock = NIL; /* unlock stream */
}
...
void mail_debug(MAILSTREAM *stream)
{
    ...
    if (stream->dtb)
        (*stream->dtb->parameters)(ENABLE_DEBUG, stream);
}
...
void mail_nodebug(MAILSTREAM *stream)
{
    ...
    if (stream->dtb)
        (*stream->dtb->parameters)(DISABLE_DEBUG, stream);
}
...
long mail_uid_sequence(MAILSTREAM *stream, unsigned char *sequence)
{
    ...
    for (i = 1; i <= stream->nmsgs; i++)
        mail_elt(stream, i)->sequence = NIL;
    ...
    { /* while there is something to parse */
        ...
        case ':': /* sequence range */
            ...
            /* start UID valid, end is not */
                ...
                while ((x <= stream->nmsgs) && (mail_uid(stream, x) <= j))
                    mail_elt(stream, x++)->sequence = T;
            /* end UID valid, start is not */
            else if (y)
                for (x = 1; x <= y; x++)
                {
                    if (mail_uid(stream, x) >= i)
                        mail_elt(stream, x)->sequence = T;
                }
...
long mail_match_lines(STRINGLIST *lines, STRINGLIST *msglines, long flags)
{
    ...
    STRINGLIST *m;
    if (!msglines)
        return T; /* full header is in cache */
    /* need full header but filtered in cache */
    if ((flags & FT_NOT) || !lines)
        return NIL;
    do
    { /* make sure all present & accounted for */
        for (m = msglines; m; m = m->next)
            if (lines->text.size == m->text.size)
            {
                for (s = lines->text.data, t = m->text.data, i = lines->text.size;
                     i && !compare_uchar(*s, *t); s++, t++, i--)
                    ;
                if (!i)
                    break; /* this line matches */
            }
        if (!m)
            return NIL; /* didn't find in the list */
                          ...
                          long flags)
{
    ...
    { /* process header */
        ...
        *t = '\0';               /* tie off */
        ...
        { /* copy line and any continuation line */
                     ...
                     SEARCHPGM *pgm)
{
    ...
    if (pgm->msgno || pgm->uid)
    { /* message set searches */
        SEARCHSET *set;
        /* message sequences */
        if (pgm->msgno)
        { /* inside this message sequence set */
            for (set = pgm->msgno; set; set = set->next)
                if (set->last ? ((set->first <= set->last) ? ((msgno >= set->first) && (msgno <= set->last)) : ((msgno >= set->last) && (msgno <= set->first))) : msgno == set->first)
                    break;
            if (!set)
                return NIL; /* not found within sequence */
        }
        if (pgm->uid)
        { /* inside this unique identifier set */
            unsigned long uid = mail_uid(stream, msgno);
            for (set = pgm->uid; set; set = set->next)
                if (set->last ? ((set->first <= set->last) ? ((uid >= set->first) && (uid <= set->last)) : ((uid >= set->last) && (uid <= set->first))) : uid == set->first)
                    break;
            if (!set)
                return NIL; /* not found within sequence */
        }
    ...
    if (pgm->before || pgm->on || pgm->since)
    {
        ...
        if (pgm->before && (d >= pgm->before))
            return NIL;
        if (pgm->on && (d != pgm->on))
            return NIL;
        if (pgm->since && (d < pgm->since))
            return NIL;
    }
    if (pgm->older || pgm->younger)
    {
        unsigned long msgd = mail_longdate(elt);
        if (pgm->older && msgd > (now - pgm->older))
            return NIL;
        if (pgm->younger && msgd < (now - pgm->younger))
            return NIL;
    }
        ...
        pgm->references)
    {
        ...
        if (!env)
            return NIL; /* no envelope obtained */
        /* sent date ranges */
        if ((pgm->sentbefore || pgm->senton || pgm->sentsince) &&
    ...
    for (hdr = pgm->header; hdr; hdr = hdr->next)
    {
            ...
            strchr(t, ':'))
        {
            ...
            { /* anything matches empty search string */
                ...
                /* for each line */
                for (v = (char *)s.data, e = t + s.size; t < e;)
                    switch (*t)
                    {
                    default: /* non-continuation, skip leading field name */
                        ...
                        if ((t < e) && (*t == ':'))
                            t++;
                    case '\t':
                    case ' ': /* copy field data  */
                        while ((t < e) && (*t != '\015') && (*t != '\012'))
                            *v++ = *t++;
                        *v++ = '\n'; /* tie off line */
                        while (((*t == '\015') || (*t == '\012')) && (t < e))
                            t++;
                    }
                /* calculate true size */
            ...
            return NIL; /* no matching header text */
    }
    /* search strings */
    ...
    for (or = pgm->or ; or ; or = or->next)
        if (!(mail_search_msg(stream, msgno, section, or->first) ||
              mail_search_msg(stream, msgno, section, or->second)))
            return NIL;
    for (not = pgm->not; not; not = not->next)
        if (mail_search_msg(stream, msgno, section, not->pgm))
            return NIL;
    return T;
}
...
long mail_search_header(SIZEDTEXT *hdr, STRINGLIST *st)
{
    ...
    do
        if (h.size ? /* search non-empty string */
                !ssearch(h.data, h.size, st->text.data, st->text.size)
                   : st->text.size)
            ret = NIL;
                      ...
                      STRINGLIST *st, long flags)
{
    ...
    /* clear searching */
    for (s = stream->private.search.string; s; s = s->next)
        s->text.data = NIL;
                      ...
                      char *prefix, unsigned long section, long flags)
{
    ...
    if (!ret)
        ...
        case TYPEMESSAGE:
            ...
            {
                if (flags)
                { /* want to search nested message header? */
                ...
                if ((body = body->nested.msg->body) != NULL)
                    ret = (body->type == TYPEMULTIPART) ? mail_search_body(stream, msgno, body, (prefix ? prefix : ""),
                                                                           section - 1, flags)
    ...
    { /* run down criteria list */
        if (ssearch(s->data, s->size, (*sc)->text.data, (*sc)->text.size))
        {
            ...
            *sc = (*sc)->next; /* remove it from the list */
            ...
            sc = &(*sc)->next; /* move to next in list */
    }
    return *st ? NIL : LONGT;
}
...
long mail_search_addr(ADDRESS *adr, STRINGLIST *st)
{
    ...
    if (adr)
    {
        ...
        for (txt.size = 0, a = adr; a; a = a->next)
        {
            ...
            if ((tadr.personal = a->personal) != NULL)
                k += 3 + 2 * strlen(a->personal);
            if ((tadr.adl = a->adl) != NULL)
                k += 3 + 2 * strlen(a->adl);
            if ((tadr.host = a->host) != NULL)
                k += 3 + 2 * strlen(a->host);
            if (tadr.personal || tadr.adl)
                k += 2;
            if (k < (SENDBUFLEN - 10))
            { /* ignore ridiculous addresses */
                tmp[0] = '\0';
                       ...
                       GETS_DATA *md)
{
    ...
    { /* more to do, blat slop down */
        memmove(tmp, tmp + MAILTMPLEN - SEARCHSLOP, (size_t)SEARCHSLOP);
        do
        { /* read subsequent buffers one at a time */
    ...
    { /* only if criteria defined */
             ...
             criterion; (criterion = strtok_r(NIL, " ", &r)))
        {
            ...
            case 'A': /* possible ALL, ANSWERED */
                if (!strcmp(criterion + 1, "LL"))
                    f = T;
                else if (!strcmp(criterion + 1, "NSWERED"))
                    f = pgm->answered = T;
                break;
            case 'B': /* possible BCC, BEFORE, BODY */
                ...
                break;
            case 'C': /* possible CC */
                if (!strcmp(criterion + 1, "C"))
                    f = mail_criteria_string(&pgm->cc, &r);
                break;
            case 'D': /* possible DELETED */
                if (!strcmp(criterion + 1, "ELETED"))
                    f = pgm->deleted = T;
                break;
            case 'F': /* possible FLAGGED, FROM */
                if (!strcmp(criterion + 1, "LAGGED"))
                    f = pgm->flagged = T;
            ...
            case 'U': /* possible UN* */
                if (criterion[1] == 'N')
                {
                    if (!strcmp(criterion + 2, "ANSWERED"))
                        f = pgm->unanswered = T;
                    else if (!strcmp(criterion + 2, "DELETED"))
                        f = pgm->undeleted = T;
                    else if (!strcmp(criterion + 2, "FLAGGED"))
                        f = pgm->unflagged = T;
                    else if (!strcmp(criterion + 2, "KEYWORD"))
                        f = mail_criteria_string(&pgm->unkeyword, &r);
                    else if (!strcmp(criterion + 2, "SEEN"))
                        f = pgm->unseen = T;
                }
                break;
...
int mail_criteria_string(STRINGLIST **s, char **r)
{
    ...
    case '"': /* quoted string */
        if (strchr(c + 1, '"'))
            ...
            return NIL; /* falls through */
    default:            /* atomic string */
        if ((d = strtok_r(c, end, r)) != NULL)
            n = strlen(d);
        else
            return NIL;
        break;
...
SEARCHSET *mail_append_set(SEARCHSET *set, unsigned long msgno)
{
    ...
    { /* find tail */
        ...
        if (!set->first)
            ...
            (set = set->next = mail_newsearchset())->first = msgno;
    }
    return set;
}
                              ...
                              SORTPGM *pgm, long flags)
{
    ...
    { /* pass 2: sort cache */
        ...
        if (!pgm->abort)
            ret = mail_sort_cache(stream, pgm, sc, flags);
        fs_give((void **)&sc); /* don't need sort vector any more */
    }
    /* empty sort results */
    else
        ret = (unsigned long *)memset(fs_get(sizeof(unsigned long)), 0,
                                      sizeof(unsigned long));
    /* also return via callback if requested */
    if (mailsortresults)
        (*mailsortresults)(stream, ret, pgm->nmsgs);
...
SORTCACHE **mail_sort_loadcache(MAILSTREAM *stream, SORTPGM *pgm)
{
    ...
    for (i = 1; !pgm->abort && (i <= stream->nmsgs); i++)
        if ((elt = mail_elt(stream, i))->searched)
        {
            ...
            if (stream->scache)
                env = (i == stream->msgno) ? stream->env : NIL;
            else
                env = elt->private.msg.env;
            for (pg = pgm; pg; pg = pg->next)
                switch (pg->function)
                {
                ...
                case SORTDISPLAYFROM: /* sort by first from */
                    ...
                    {
                        if (env)
                            s->from = env->from && env->from->mailbox ? cpystr(env->from->mailbox) : NIL;
                ...
                case SORTTO: /* sort by first to */
                    ...
                    {
                        if (env)
                            s->to = env->to && env->to->mailbox ? cpystr(env->to->mailbox) : NIL;
                ...
                case SORTCC: /* sort by first cc */
                    ...
                    {
                        if (env)
                            s->cc = env->cc && env->cc->mailbox ? cpystr(env->cc->mailbox) : NIL;
...
unsigned int mail_strip_subject(char *t, char **ret)
{
    ...
    if ((src.size = strlen(t)) != 0)
    { /* have non-empty subject? */
        ...
        for (slen = dst.size; s; slen = strlen(s))
        {
            ...
            for (t = s; t;)
                ...
                case 'R': /* possible "re" */
                    ...
                    break;
                case 'f':
                case 'F': /* possible "fw" or "fwd" */
                    if (((s[1] == 'w') || (s[1] == 'W')) &&
                        (((s[2] == 'd') || (s[2] == 'D')) ? (t = mail_strip_subject_wsp(s + 3)) : (t = mail_strip_subject_wsp(s + 2))) &&
                        (t = mail_strip_subject_blob(t)) && (*t == ':'))
                    {
                        s = ++t;   /* found "fwd" */
                        refwd = T; /* definitely a re/fwd at this point */
                ...
                case '[': /* possible subj-blob */
                    if ((t = mail_strip_subject_blob(s)) && *t)
                        s = t;
                    else
                        t = NIL; /* found subj-middle */
                    break;
                ...
                (s[i = strlen(s) - 1] == ']'))
            {
                ...
                break; /* don't need to loop back to step 2 */
        }
        if (s != (t = *ret))
...
unsigned long mail_longdate(MESSAGECACHE *elt)
{
    ...
    unsigned long ret = (elt->day ? (elt->day - 1) : 0) + 30 * (m - 1) + ((m + (m > 8)) / 2)
#ifndef USEJULIANCALENDAR
#ifndef USEORTHODOXCALENDAR /* Gregorian calendar */
                        ...
                        - ((m < 3) ? !(yr % 4) && ((yr % 100) || (!(yr % 400)
                                   ...
                                   : 2)
#else /* Orthodox calendar */
                        + ((2 * (yr / 900)) - (2 * (BASEYEAR / 900))) + (((yr % 900) >= 200) - ((BASEYEAR % 900) >= 200)) + (((yr % 900) >= 600) - ((BASEYEAR % 900) >= 600)) - ((yr / 100) - (BASEYEAR / 100)) - ((m < 3) ? !(yr % 4) && ((yr % 100) || ((yr % 900) == 200) || ((yr % 900) == 600)) : 2)
#endif
#endif
                        + elt->year * 365 + (((unsigned long)(elt->year + (BASEYEAR % 4))) / 4);
    ret *= 24;
    ret += elt->hours; /* date value in hours */
    ret *= 60;
    ret += elt->minutes; /* date value in minutes */
    yr = (elt->zhours * 60) + elt->zminutes;
    if (elt->zoccident)
        ret += yr; /* occidental timezone, make UTC */
    else if (ret < yr)
        return 0; /* still 31-Dec-1969 in UTC */
    else
        ret -= yr; /* oriental timezone, make UTC */
    ret *= 60;
    ret += elt->seconds;
    return ret;
}
                             ...
                             SEARCHPGM *spg, long flags, sorter_t sorter)
{
    ...
    for (t = &mailthreadlist; t; t = t->next)
        if (!compare_cstring(type, t->name))
        {
            ...
            if (mailthreadresults)
                (*mailthreadresults)(stream, ret);
            return ret;
        }
                                       ...
                                       sorter_t sorter)
{
    ...
    if ((lst = (*sorter)(stream, charset, spg, &pgm, flags & ~(SE_FREE | SE_UID))) != NULL)
    {
        ...
        { /* create thread */
            /* note first subject */
            cur = top = thr = mail_newthreadnode((SORTCACHE *)(*mailcache)(stream, *ls++, CH_SORTCACHE));
            /* note its number */
            ...
            { /* build tree */
                ...
                /* start a child of the top */
                    ...
                    cur = cur->next = mail_newthreadnode(s);
                /* sibling of child */
                else
                    cur = cur->branch = mail_newthreadnode(s);
            ...
            for (j = 0, cur = thr; cur; cur = cur->branch)
                tc[j++] = cur;
            if (i != j)
                fatal("Threadnode cache confusion");
                                   ...
                                   SEARCHPGM *spg, long flags, sorter_t sorter)
{
    ...
    { /* for each message in sortcache */
        ...
        if ((st = (s = sc[i])->references) && st->text.data)
            ...
            con = NIL; /* else message has no ancestors */
                       /* Step 1B */
        if ((prc = PARENT((nxc = hash_lookup(ht, s->unique)))) &&
            (prc != con))
        {                        /* break links if have a different parent */
            SETPARENT(nxc, NIL); /* easy if direct child */
            if (nxc == CHILD(prc))
                SETCHILD(prc, SIBLING(nxc));
            else
            { /* otherwise hunt through sisters */
        ...
        if (!prc && !mail_thread_check_child(con, nxc))
        {
            SETPARENT(nxc, con); /* establish parent/child link */
            if (con)
            { /* if non-root parent, set parent's child */
                if (CHILD(con))
                { /* have a child already */
                    ...
                    SETCHILD(con, nxc); /* set as only child */
            }
        }
    }
    ...
    for (i = 0, prc = con = NIL; i < ht->size; i++)
        for (he = ht->table[i]; he; he = he->next)
            if (!PARENT((nxc = he->data)))
            {
                ...
                if (con)
                    con = SETSIBLING(con, nxc);
                else
                    prc = con = nxc; /* first parentless message */
            }
   ...
   */
    ...
    for (i = 0, cur = root; cur; cur = cur->branch)
        tc[i++] = cur;
    if (i > 1)
    { /* only if need to sort */
        qsort((void *)tc, i, sizeof(THREADNODE *), mail_thread_compare_date);
        /* relink siblings */
        for (j = 0, --i; j < i; j++)
            tc[j]->branch = tc[j + 1];
    ...
    for (cur = root, sis = NIL; cur; cur = msg)
    {
        ...
        { /* no dummies, is current re/fwd, table not? */
            if (cur->sc->refwd && !lst->sc->refwd)
            {
                ...
                { /* find youngest daughter of msg in table */
                    ...
                    lst->next = cur; /* no children, so make the eldest daughter */
            }

            else
            { /* no re/fwd, create a new dummy */
                msg = mail_newthreadnode(NIL);
                if (lst == root)
                {                       /* msg in table is root? */
                ...
                { /* find older sister of msg in table */
                ...
                {                   /* have an elder sister? */
                    ...
                    root = msg;     /* otherwise this is the new root */
        ...
        if (sis)
            sis->branch = msg; /* older sister gets this as younger sister */
        else
            root = msg; /* otherwise this is the new root */
    }
                           ...
                           unsigned long msgno)
{
    ...
    { /* just in case */
        ...
        if (!s->references &&
            !(s->references = mail_thread_parse_references(ov->references, T)) && stream->dtb && !strcmp(stream->dtb->name, "imap") && (elt = mail_elt(stream, msgno)) != NULL && (env = elt->private.msg.env) != NULL && env->in_reply_to && !(s->references = mail_thread_parse_references(env->in_reply_to, NIL)))
        {
...
container_t mail_thread_prune_dummy(container_t msg, container_t ane)
{
    ...
    if (ret)
        for (ane = ret; ane && (msg = SIBLING(ane)); ane = msg)
...
container_t mail_thread_prune_dummy_work(container_t msg, container_t ane)
{
    ...
    else if ((cur = PARENT(msg)) || !SIBLING(nxt))
    {
        ...
        SETCHILD(msg, nxt); /* in case child pruned */
...
{
    if (mother)
    { /* only if mother non-NIL */
        if (mother == daughter)
            return T;
        for (daughter = CHILD(daughter); daughter; daughter = SIBLING(daughter))
            if (mail_thread_check_child(mother, daughter))
                return T;
    }
    return NIL;
}
...
THREADNODE *mail_thread_c2node(MAILSTREAM *stream, container_t con, long flags)
{
    ...
    for (ret = cur = NIL; con; con = SIBLING(con))
    {
        ...
        if (ret)
            ...
            ret = cur = mail_newthreadnode(s);
        /* attach sequence or UID for non-dummy */
        if (s)
            cur->num = (flags & SE_UID) ? mail_uid(stream, s->num) : s->num;
        /* attach the children */
        if ((nxt = CHILD(con)) != NULL)
            cur->next = mail_thread_c2node(stream, nxt, flags);
    }
    return ret;
}
...
THREADNODE *mail_thread_sort(THREADNODE *thr, THREADNODE **tc)
{
    ...
    for (cur = thr; cur; cur = cur->branch)
        if (cur->next)
            cur->next = mail_thread_sort(cur->next, tc);
    /* Must do this in a separate pass since recursive call will clobber tc */
    /* load threadcache and count nodes to sort */
    for (i = 0, cur = thr; cur; cur = cur->branch)
        tc[i++] = cur;
    if (i > 1)
    { /* only if need to sort */
        qsort((void *)tc, i, sizeof(THREADNODE *), mail_thread_compare_date);
    ...
    *uf = 0; /* initially no user flags */
    ...
    { /* no-op if no flag string */
        ...
        { /* parse the flags */
            ...
            if (*t == '\\')
            { /* system flag? */
                if (!compare_cstring(t + 1, "SEEN"))
                    f |= fSEEN;
                else if (!compare_cstring(t + 1, "DELETED"))
                    f |= fDELETED;
                else if (!compare_cstring(t + 1, "FLAGGED"))
                    f |= fFLAGGED;
                else if (!compare_cstring(t + 1, "ANSWERED"))
                    f |= fANSWERED;
                else if (!compare_cstring(t + 1, "DRAFT"))
                    f |= fDRAFT;
                else
                {
            ...
            {                   /* keyword flag */
                for (i = j = 0; /* user flag, search through table */
                    ...
                    if (!compare_cstring(t, s))
                        *uf |= i = 1 << j;
                if (!i)
                { /* flag not found, can it be created? */
...
long mail_usable_network_stream(MAILSTREAM *stream, char *name)
{
    ...
    long ret = (stream && stream->dtb && !(stream->dtb->flags & DR_LOCAL) &&
                ...
                ((!compare_cstring(smb.host, trustdns ? (s = tcp_canonical(nmb.host)) : nmb.host) &&
                 ...
                 (!compare_cstring(omb.host, nmb.host) &&
                  ...
                  (nmb.anoflag == stream->anonymous) &&
                  (!nmb.user[0] || !strcmp(omb.user, nmb.user)))))
                   ? LONGT
                   : NIL;
    if (s)
        fs_give((void **)&s);
...
THREADNODE *mail_newthreadnode(SORTCACHE *sc)
{
    THREADNODE *thr = (THREADNODE *)memset(fs_get(sizeof(THREADNODE)), 0,
                                           sizeof(THREADNODE));
    if (sc)
        thr->sc = sc; /* initialize sortcache */
    return thr;
}
...
void mail_free_body_data(BODY *body)
{
    ...
    if (body->disposition.type)
        fs_give((void **)&body->disposition.type);
    if (body->disposition.parameter)
        mail_free_body_parameter(&body->disposition.parameter);
    if (body->language)
        mail_free_stringlist(&body->language);
    if (body->location)
        fs_give((void **)&body->location);
    if (body->mime.text.data)
        fs_give((void **)&body->mime.text.data);
    if (body->contents.text.data)
        fs_give((void **)&body->contents.text.data);
    if (body->md5)
        fs_give((void **)&body->md5);
...
void mail_free_body_parameter(PARAMETER **parameter)
{
    ...
    { /* only free if exists */
        if ((*parameter)->attribute)
            fs_give((void **)&(*parameter)->attribute);
        if ((*parameter)->value)
            fs_give((void **)&(*parameter)->value);
...
void mail_free_elt(MESSAGECACHE **elt)
{
    ...
    if (*elt && !--(*elt)->lockcount)
    {
        ...
        *elt = NIL; /* else simply drop pointer */
}
...
void mail_free_address(ADDRESS **address)
{
    ...
    { /* only free if exists */
        if ((*address)->personal)
            fs_give((void **)&(*address)->personal);
        if ((*address)->adl)
            fs_give((void **)&(*address)->adl);
        if ((*address)->mailbox)
            fs_give((void **)&(*address)->mailbox);
        if ((*address)->host)
            fs_give((void **)&(*address)->host);
        if ((*address)->error)
            fs_give((void **)&(*address)->error);
        if ((*address)->orcpt.type)
            fs_give((void **)&(*address)->orcpt.type);
        if ((*address)->orcpt.addr)
            fs_give((void **)&(*address)->orcpt.addr);
...
void mail_free_searchheader(SEARCHHEADER **hdr)
{
    ...
    { /* only free if exists */
        if ((*hdr)->line.data)
            fs_give((void **)&(*hdr)->line.data);
        if ((*hdr)->text.data)
            fs_give((void **)&(*hdr)->text.data);
...
void mail_free_acllist(ACLLIST **al)
{
    ...
    { /* only free if exists */
        if ((*al)->identifier)
            fs_give((void **)&(*al)->identifier);
        if ((*al)->rights)
            fs_give((void **)&(*al)->rights);
...
static void mail_free_annotation_values(ANNOTATION_VALUES **val)
{
    ...
    { /* only free if exists */
        if ((*val)->attr)
            fs_give((void **)&(*val)->attr);
        if ((*val)->value)
            fs_give((void **)&(*val)->value);
...
void auth_link(AUTHENTICATOR *auth)
{
    if (!auth->valid || (*auth->valid)())
    {
        ...
        while (*a)
            a = &(*a)->next; /* find end of list of authenticators */
                    ...
                    NETDRIVER *ssld, char *ssls, unsigned long sslp)
{
    ...
    /* use designated driver if given */
        ...
        stream = net_open_work(dv, mb->host, mb->service, port, mb->port,
                               flags);
    ...
    /* default to TCP driver */
        ...
        stream = net_open_work(&tcpdriver, mb->host, mb->service, port, mb->port,
                               flags);
    return stream;
}
...
NETSTREAM *net_aopen(NETDRIVER *dv, NETMBX *mb, char *service, char *user)
{
    ...
    void *tstream;
    if (!dv)
        dv = &tcpdriver; /* default to TCP driver */
