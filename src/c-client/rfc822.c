                           ...
                           unsigned long flags)
{
    ...
    if (!host)
        host = BADHOST; /* make sure that host is non-null */
    while (i && *s != '\n')
    {            /* until end of header */
	       ...
	       */
            ...
            case 'M': /* possible Message-ID: or MIME-Version: */
                ...
                else if (!strcmp(tmp + 1, "IME-VERSION"))
                {
                    ...
                    if ((t = rfc822_parse_phrase(d)) != NULL)
                        *t = '\0';
    ...
    /* now parse the body */
    if (body)
        rfc822_parse_content(body, bs, host, depth, flags);
}
                          ...
                          unsigned long flags)
{
    ...
    case TYPEMULTIPART: /* multiple parts */
        ...
        { /* examine data */
            if ((m = GETPOS(bs)) != 0L)
                m--; /* get position in front of character */
            switch (c)
            {            /* examine each line */
            ...
            case '\012': /* at start of a line, start with -- ? */
                ...
                if ((k = j - 2) != 0L)
                    for (s = s1; i-- && *s++ == (c = SNX(bs)) && --k;)
                        ;
                if (k)
                    break; /* strings didn't match if non-zero */
        ...
        if (part)
            part->body.mime.text.size = i +
                                        ((GETPOS(bs) > part->body.mime.offset) ? (GETPOS(bs) - part->body.mime.offset) : 0);
        ...
        for (part = body->nested.part; part; part = part->next)
        {
            ...
            if ((i = part->body.mime.text.size) != 0L)
            {
                ...
                while (i && ((c = CHR(bs)) != '\015') && (c != '\012'))
                {
                    ...
                    for (j = 0, c = ' '; c;)
                    {
                        ...
                        if (j > (k - 10))
                            fs_resize((void **)&s1, k += MAILTMPLEN);
                        switch (c1 = SNX(bs))
                        {
                        case '\015': /* return */
                            if (i && (CHR(bs) == '\012'))
            ...
            { /* zero-length part, use default subtype */
                ...
                case TYPETEXT: /* text content */
                    ...
                    if (!part->body.parameter)
                    {
                        ...
                        part->body.parameter->value =
                            cpystr((part->body.encoding == ENC7BIT) ? "US-ASCII" : "X-UNKNOWN");
                    }
                    break;
...
void rfc822_parse_content_header(BODY *body, char *name, char *s)
{
    ...
    if ((t = strchr(name, ' ')) != NULL)
        *t = '\0';
    switch (*name)
    {         /* see what kind of content */
    case 'I': /* possible Content-ID */
        if (!(strcmp(name + 1, "D") || body->id))
            body->id = cpystr(s);
        break;
    case 'D': /* possible Content-Description */
        ...
        {
            /* get type word */
            if (!(name = rfc822_parse_word(s, tspecials)))
                break;
        ...
        if (!(strcmp(name + 1, "ANGUAGE") || body->language))
        {
            ...
            while (s && (name = rfc822_parse_word(s, tspecials)))
            {
                ...
                *name = '\0'; /* tie off subtype */
                if (stl)
                    stl = stl->next = mail_newstringlist();
                else
                    stl = body->language = mail_newstringlist();
        ...
        if (!(strcmp(name + 1, "YPE") || body->subtype || body->parameter))
        {
            ...
            *name = '\0'; /* tie off type */
            ...
            for (i = 0, s = rfc822_cpy(s);
                 ...
                 compare_cstring(s, body_types[i]);
                 i++)
                ;
            if (i > TYPEMAX)
            {                           /* fell off end of loop? */
            ...
            { /* record body type index */
                ...
                if (body_types[body->type])
                    fs_give((void **)&s);
                else
                { /* major MIME body type unknown to us */
            ...
            *name = c;            /* restore delimiter */
                ...
                (name = rfc822_parse_word((s = ++name), tspecials)))
            {
                ...
                *name = '\0';      /* tie off subtype */
                ...
                if (s)
                    body->subtype = ucase(rfc822_cpy(s));
                *name = c;            /* restore delimiter */
                rfc822_skipws(&name); /* skip whitespace */
            }
            else if (!name)
            {                         /* no subtype, was a subtype delimiter? */
        ...
        else if (!strcmp(name + 1, "RANSFER-ENCODING"))
        {
            ...
            *name = '\0'; /* tie off encoding */
            ...
            for (i = 0, s = rfc822_cpy(s);
                 ...
                 compare_cstring(s, body_encodings[i]);
                 i++)
                ;
            if (i > ENCMAX)
            { /* fell off end of loop? */
            ...
            { /* record body encoding index */
                ...
                if (body_encodings[body->encoding])
                    fs_give((void **)&s);
                else
                {
...
void rfc822_parse_parameter(PARAMETER **par, char *text)
{
        ...
        *text = '\0';      /* tie off attribute name */
        ...
        { /* instantiate a new parameter */
            if (*par)
                ...
                param = *par = mail_newbody_parameter();
    ...
    /* string not present */
    if (!text)
        MM_LOG("Missing parameter", PARSE);
    else if (*text)
    { /* must be end of poop */
...
void rfc822_parse_adrlist(ADDRESS **lst, char *string, char *host)
{
    ...
    { /* loop until string exhausted */
        ...
        if (!*string)
            string = NIL; /* punt if ran out of string */
        /* got an address? */
                              ...
                              char *defaulthost, unsigned long depth)
{
    ADDRESS *adr;
    if (!*string)
        return NIL;        /* no string */
    rfc822_skipws(string); /* skip leading WS */
    if (!**string)
        return NIL; /* empty string */
    if ((adr = rfc822_parse_group(lst, last, string, defaulthost, depth)) != NULL)
        last = adr;
    /* got an address? */
    else if ((adr = rfc822_parse_mailbox(string, defaulthost)) != NULL)
    {
        if (!*lst)
            *lst = adr; /* yes, first time through? */
        else
            last->next = adr; /* no, append to the list */
    ...
    else if ((end = rfc822_parse_phrase(s)) != NULL)
    {
                 ...
                 (adr = (*pp)(s, end, defaulthost)))
        {
            *string = end;         /* update parse pointer */
            ...
            adr = rfc822_parse_addrspec(s, string, defaulthost);
    }
...
{
    while (*end == ' ')
        ++end; /* call rfc822_skipws() instead?? */
    switch (*end)
    {
    case '\0':
    case ',':
...
ADDRESS *rfc822_parse_routeaddr(char *string, char **ret, char *defaulthost)
{
    ...
    { /* got an A-D-L? */
        if (*t != ':')
        { /* make sure syntax good */
            sprintf(tmp, "Unterminated at-domain-list: %.80s%.80s", adl, t);
            MM_LOG(tmp, PARSE);
        }
        else
            string = ++t; /* continue parse from this point */
    }
...
ADDRESS *rfc822_parse_addrspec(char *string, char **ret, char *defaulthost)
{
    ...
    *t = c;            /* restore delimiter */
    ...
    while (*t == '.')
    {                 /* some cretin taking RFC 822 too seriously? */
...
char *rfc822_parse_domain(char *string, char **end)
{
    ...
    { /* domain literal? */
        if (!(*end = rfc822_parse_word(string + 1, "]\\")))
            ...
            MM_LOG("Unterminated domain literal", PARSE);
        else
        {
...
char *rfc822_parse_phrase(char *s)
{
    char *curpos;
    if (!s)
        return NIL; /* no-op if no string */
    /* find first word of phrase */
    curpos = rfc822_parse_word(s, NIL);
    if (!curpos)
        return NIL; /* no words means no phrase */
    if (!*curpos)
        return curpos; /* check if string ends with word */
...
char *rfc822_parse_word(char *s, const char *delimiters)
{
    ...
    while (T)
    { /* look for delimiter, return if none */
        ...
        if (!delimiters && (*st == I2C_ESC))
        {
            ...
            switch (*st)
            {               /* special hack for RFC 1468 (ISO-2022-JP) */
            ...
            case '"': /* quoted string */
                ...
                while (*++st != '"')
                    ...
                    case '\\':      /* quoted character */
                        if (!*++st)
                            return NIL; /* skip the next character */
...
ADDRESS *rfc822_cpy_adr(ADDRESS *adr)
{
    ...
    ADDRESS *prev = NIL;
    while (adr)
    {                          /* loop while there's still an MAP adr */
        dadr = mail_newaddr(); /* instantiate a new address */
        if (!ret)
            ret = dadr; /* note return */
        if (prev)
            prev->next = dadr; /* tie on to the end of any previous */
...
void rfc822_skipws(char **s)
{
    while (T)
        ...
        case '(': /* start of comment */
            if (rfc822_skip_comment(s, (long)NIL))
                break;
        default:
...
char *rfc822_skip_comment(char **s, long trim)
{
    ...
    do
        ...
        case '(': /* nested comment? */
            if (!rfc822_skip_comment(&s1, (long)NIL))
                return NIL;
...
{
    if ((buf->cur == buf->end) && !rfc822_output_flush(buf))
        return NIL;
...
long rfc822_output_full(RFC822BUFFER *buf, ENVELOPE *env, BODY *body, long ok8)
{
    ...
    if (r822of)
        return (*r822of)(buf, env, body, ok8);
    else if (r822o)
        return (*r822o)(buf->cur, env, body, buf->f, buf->s, ok8);
                          ...
                          const char *specials, long flags)
{
    ...
    return     /* write header */
        ...
        ((env->bcc && !(env->to || env->cc)) ? rfc822_output_string(buf, "To: undisclosed recipients: ;\015\012") : LONGT) &&
    ...
    return !adr ||
           ((resent ? rfc822_output_string(buf, resentprefix) : LONGT) &&
            ...
            rfc822_output_address_list(buf, adr,
                                       resent ? pretty + sizeof(RESENTPREFIX) - 1 : pretty, specials) &&
            rfc822_output_string(buf, "\015\012"));
}
                                ...
                                const char *specials)
{
    ...
    for (n = 0; adr; adr = adr->next)
    {
        ...
        { /* ordinary address? */
            if (!(pretty && n))
            {       /* suppress if pretty and in group */
                ...
                else if (!rfc822_output_address(buf, adr))
                    return NIL;
        ...
        if (pretty && adr->next && /* pretty printing? */
            ((pretty += ((buf->cur > base) ? buf->cur - base : (buf->end - base) + (buf->cur - buf->beg))) >= 78))
        {
            if (!(rfc822_output_string(buf, "\015\012") &&
                  rfc822_output_string(buf, RFC822CONT)))
                return NIL;
...
long rfc822_output_cat(RFC822BUFFER *buf, char *src, const char *specials)
{
    ...
    if (!*src || /* empty string or any specials present? */
        (specials ? (T && strpbrk(src, specials)) : (strpbrk(src, wspecials) || (*src == '.') || strstr(src, "..") || (src[strlen(src) - 1] == '.'))))
    {
...
long rfc822_output_parameter(RFC822BUFFER *buf, PARAMETER *param)
{
    while (param)
    {
        if (rfc822_output_string(buf, "; ") &&
            ...
            rfc822_output_char(buf, '=') &&
            rfc822_output_cat(buf, param->value, tspecials))
            param = param->next;
        else
            return NIL;
    }
...
long rfc822_output_body_header(RFC822BUFFER *buf, BODY *body)
{
    return /* type and subtype*/
        ...
        rfc822_output_char(buf, '/') &&
        rfc822_output_string(buf, body->subtype ? body->subtype : rfc822_default_subtype(body->type)) &&
        /* parameters (w/ US-ASCII default */
        (body->parameter ? rfc822_output_parameter(buf, body->parameter) : ((body->type != TYPETEXT) || (rfc822_output_string(buf, "; CHARSET=") && rfc822_output_string(buf, (body->encoding == ENC7BIT) ? "US-ASCII" : "X-UNKNOWN")))) &&
    ...
    if (body->type == TYPEMULTIPART)
    {
        ...
        for (part = body->nested.part; part; part = part->next)
            if (!(rfc822_output_string(buf, "--") &&
                  ...
                  rfc822_output_string(buf, "\015\012") &&
                  rfc822_output_text(buf, &part->body)))
                return NIL;
        ...
        case '=': /* quoting character */
            if (((unsigned long)(s - src)) < srcl)
                ...
                case '\015': /* non-significant line break */
                    if ((((unsigned long)(s - src)) < srcl) && (*s == '\012'))
                        s++;
                case '\012': /* bare LF */
                   ...
                   long ok8bit)
{
    ...
    /* call external RFC 2822 output generator */
    if (r822o)
        ret = (*r822o)(t, env, body, f, s, ok8bit);
    else
    {                             /* output generator not armed */
