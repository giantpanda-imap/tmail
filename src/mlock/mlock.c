...
int main(int argc, char *argv[])
{
    ...
    if (argc != 3)
        return die("invalid arguments", EX_USAGE);
    for (s = argv[1]; *s; s++)
        if (!isdigit(*s))
            return die("invalid fd", EX_USAGE);
    /* find directory */
    ...
    { /* until OK or out of tries */
        ...
        if ((ld = open(hitch, O_WRONLY | O_CREAT | O_EXCL, LOCKPROTECTION)) >= 0)
        {
            ...
            if (fstat(ld, &fsb))
                return die("hitch fstat failure", errno);
       ...
       */
                ...
                (fsb.st_dev != sb.st_dev) || (fsb.st_ino != sb.st_ino))
            {
                ...
                if (i == EPERM)
                { /* was it because links not allowed? */
	   ...
	   */
                    if ((ld = open(lock, O_WRONLY | O_CREAT | O_EXCL, LOCKPROTECTION)) >= 0)
                    {
                        ...
                        if (fstat(ld, &fsb))
                            return die("lock fstat failure", errno);
    ...
    if (!stat(lock, &sb) && (fsb.st_dev == sb.st_dev) &&
        ...
        syslog(LOG_NOTICE, "lock file %s/%s changed dev/inode", dir, lock);
    return EX_OK;
}
