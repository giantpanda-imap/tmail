...
void mm(MAILSTREAM *stream, long debug)
{
    ...
    while (stream)
    {
        ...
        case '?': /* ? command */
            ...
            puts("Undelete, Xit, #Namespace, Annotation, +, -, or <RETURN> for next message");
            break;
...
void status(MAILSTREAM *stream)
{
    ...
    if (stream)
    {
        if (stream->mailbox)
            ...
            puts("%No mailbox is open on this stream");
        ...
        if (!strcmp(stream->dtb->name, "imap"))
        {
            ...
            if (LEVELIMAP4(stream))
            {
                ...
                if (LEVELACL(stream))
                    puts(" Access Control lists (RFC 2086)");
                if (LEVELQUOTA(stream))
                    puts(" Quotas (RFC 2087)");
                if (LEVELLITERALPLUS(stream))
                    puts(" Non-synchronizing literals (RFC 2088)");
                if (LEVELIDLE(stream))
                    puts(" IDLE unsolicited update (RFC 2177)");
                if (LEVELMBX_REF(stream))
                    puts(" Mailbox referrals (RFC 2193)");
                if (LEVELLOG_REF(stream))
                    puts(" Login referrals (RFC 2221)");
                if (LEVELANONYMOUS(stream))
                    puts(" Anonymous access (RFC 2245)");
                if (LEVELNAMESPACE(stream))
                    puts(" Multiple namespaces (RFC 2342)");
                if (LEVELUIDPLUS(stream))
                    puts(" Extended UID behavior (RFC 2359)");
                ...
                if (LEVELUNSELECT(stream))
                    puts(" Mailbox unselect (RFC 3691)");
                if (LEVELURLAUTH(stream))
                    puts(" URL authenticated fetch (RFC 4467)");
                if (LEVELCATENATE(stream))
                    puts(" Catenation (RFC 4469)");
                if (LEVELCONDSTORE(stream))
                    puts(" Conditional STORE (RFC 4551)");
                if (LEVELESEARCH(stream))
                    puts(" Extended SEARCH (RFC 4731)");
                puts("Supported draft extensions:");
                if (LEVELSASLIR(stream))
                    puts(" SASL initial client response");
                if (LEVELSORT(stream))
                    puts(" Server-based sorting");
...
void mm_status(MAILSTREAM *stream, char *mailbox, MAILSTATUS *status)
{
    ...
    if (status->flags & SA_MESSAGES)
        printf(", %lu messages", status->messages);
    if (status->flags & SA_RECENT)
        printf(", %lu recent", status->recent);
    if (status->flags & SA_UNSEEN)
        printf(", %lu unseen", status->unseen);
    if (status->flags & SA_UIDVALIDITY)
        printf(", %lu UID validity",
               status->uidvalidity);
    if (status->flags & SA_UIDNEXT)
        printf(", %lu next UID", status->uidnext);
    printf("\n");
}
...
void mm_login(NETMBX *mb, char *user, char *pwd, long trial)
{
    ...
    if (*mb->user)
        sprintf(tmp + strlen(tmp), "/user=%s", strcpy(user, mb->user));
    if (*mb->authuser)
